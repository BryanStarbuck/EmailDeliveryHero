 ...
 */
export function DmarcReportsSnapshotView({ snapshot }: { snapshot?: DmarcReportsSnapshot }) {
  ...
  return (
    <div className="mt-3">
      <p className="mb-1 text-xs font-medium uppercase text-[var(--edh-muted)]">
        Aggregate breakdown — window {fmtWindow(snapshot.window)} · {snapshot.report_count} report
        {snapshot.report_count === 1 ? "" : "s"}
      </p>
      <table className="w-full text-sm">
        <tbody>
          <Row label="Reporters" value={snapshot.reporters.join(", ") || "—"} />
          <Row label="Messages" value={snapshot.total_messages.toLocaleString()} />
          <Row
            label="Dual-aligned"
      ...
      {snapshot.rows.length > 0 && (
        <div className="mt-3">
          ...
          >
            ...
            Per-source table ({snapshot.rows.length} source{snapshot.rows.length === 1 ? "" : "s"})
          </button>
          {open && (
                  ...
                  {snapshot.rows.map((r) => (
                    ...
                    >
                      ...
                      <td className="px-2 py-1">
                        {r.spf_evaluated}/{r.spf_aligned ? "aligned" : "not aligned"}
                      </td>
                      <td className="px-2 py-1">
                        {r.dkim_evaluated}/{r.dkim_aligned ? "aligned" : "not aligned"}
                      </td>
                      <td
...
function Row({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <tr className="border-t border-[var(--edh-border)]">
      <td className="py-1.5 pr-3 align-top text-xs font-semibold text-slate-600">{label}</td>
      <td className={`py-1.5 align-top text-xs ${mono ? "font-mono text-slate-700" : "text-slate-700"}`}>
        {value}
      </td>
    </tr>
  )
