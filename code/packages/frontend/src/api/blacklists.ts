...
export function useBlacklistRegistry() {
	return useQuery({
		...
		queryFn: async () =>
			(await api.get<BlacklistRegistryInfo>("/blacklists/zones")).data,
		staleTime: 5 * 60 * 1000,
	});
}
...
export function useBlacklistRuns() {
	return useQuery({
		queryKey: RUNS_KEY,
		queryFn: async () =>
			(await api.get<BlacklistRunResults[]>("/blacklists/results")).data,
	});
}
...
export function useBlacklistRun(domain: string, enabled = true) {
	return useQuery({
		...
		queryFn: async () =>
			(
				await api.get<BlacklistRunResults>(
					`/blacklists/results/${encodeURIComponent(domain)}`,
				)
			).data,
		retry: false,
...
export function useRunBlacklistsCheck() {
	...
	return useMutation({
		mutationFn: async (domainId: string) =>
			(
				await api.post<AuditResult>(
					`/audit/run/${encodeURIComponent(domainId)}/blacklists`,
				)
			).data,
		onSuccess: () => {
