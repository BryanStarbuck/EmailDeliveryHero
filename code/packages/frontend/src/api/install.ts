...
export async function fetchPreflight(
  ...
  return (await api.get<PreflightResult>("/install/preflight", { params: { manager, scope } })).data
}
...
export function useStartInstall() {
  return useMutation({
    mutationFn: async (ids: string[]) =>
      (await api.post<{ jobId: string; ids: string[] }>("/install/run", { ids })).data,
  })
}
...
export async function redetect(manager: ToolManager): Promise<ToolStatus[]> {
  return (await api.post<ToolStatus[]>("/install/detect", undefined, { params: { manager } })).data
}
