...
export function useContentSample(domainId: string | undefined) {
	return useQuery({
		...
		queryFn: async () =>
			(
				await api.get<ContentSampleResponse>(
					`/audit/content-sample/${domainId}`,
				)
			).data,
		enabled: !!domainId,
	});
...
export function useUploadContentSample(domainId: string) {
	...
	return useMutation({
		mutationFn: async (raw: string) =>
			(
				await api.put<{ sample: ContentSampleView }>(
					`/audit/content-sample/${domainId}`,
					{ raw },
				)
			).data,
...
export function useContentSampleRaw(
	...
	return useQuery({
		...
		queryFn: async () =>
			(await api.get<{ raw: string }>(`/audit/content-sample/${domainId}/raw`))
				.data,
...
export function useRescoreContent(domainId: string) {
	...
	return useMutation({
		mutationFn: async () =>
			(await api.post<AuditResult>(`/audit/content-sample/${domainId}/rescore`))
				.data,
		onSuccess: () => {
