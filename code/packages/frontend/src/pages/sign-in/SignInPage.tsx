...
function useAuthConfig() {
	return useQuery({
		...
		queryFn: async () =>
			(await api.get<AuthConfig>("/health/auth-config")).data,
		staleTime: 60_000,
	});
}
	...
	return (
		<div className="max-w-lg rounded-lg border border-[var(--edh-border)] bg-white p-5 text-sm">
			...
			<p className="mb-3 text-slate-600">
				The app is fully usable without signing in — you are using it as the{" "}
				<code>default</code> user. To enable Google Workspace sign-in, the
				server needs a Google OAuth client:
			</p>
			<ol className="list-decimal space-y-2 pl-5 text-slate-600">
				<li>
					Set <code>GOOGLE_CLIENT_ID</code> / <code>GOOGLE_CLIENT_SECRET</code>{" "}
					in the backend environment, <em>or</em> add them under the{" "}
					<code>email_delivery_hero.google</code> key in{" "}
	...
	return (
		<div className="flex min-h-screen flex-col items-center justify-center bg-slate-100 p-4">
			<div className="mb-6 text-center">
				<h1 className="text-2xl font-bold text-[var(--edh-primary)]">
					EmailDeliveryHero
				</h1>
				<p className="mt-1 text-sm text-[var(--edh-muted)]">
					Audit email deliverability — spam filters, blacklists, and the fixes
					to apply.
				</p>
			</div>
