...
export function ContentScoringPage() {
  ...
  const urlFindings = (result?.findings ?? []).filter((f) => f.id.startsWith("content.url"))
  const urlResults = result?.results?.["content.url"] as LinkUrlResults | undefined
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <LinkUrlReputationSection findings={urlFindings} results={urlResults} hasRun={!!result} />
    ...
    <div className="mt-4 rounded-lg border border-[var(--edh-border)] bg-white p-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className="text-sm text-slate-600">
          {score.passed ? "Below the spam threshold" : "At or above the spam threshold"} —
      ...
      <p className="mt-2 text-xs text-[var(--edh-muted)]">
        sample:{" "}
        <span className="font-medium text-slate-700">"{score.subject ?? "(no subject)"}"</span> from{" "}
        {score.from_header ?? "(unknown sender)"} · scored{" "}
...
function LinkChips({ link }: { link: UrlLinkResult }) {
  const chips: { text: string; tone: "ok" | "warn" | "bad" }[] = [
    link.isHttps ? { text: "https", tone: "ok" } : { text: "http", tone: "warn" },
  ]
  if (link.isShortener) chips.push({ text: "shortener", tone: "warn" })
  if (link.isIpLiteral) chips.push({ text: "IP-literal", tone: "bad" })
  if (link.homographOf) chips.push({ text: `homograph: ${link.homographOf}`, tone: "bad" })
  ...
  for (const listing of link.listings) {
    if (listing.listed) chips.push({ text: `listed: ${listing.zone}`, tone: "bad" })
  }
  return (
...
function LinkUrlReputationSection({
  ...
  const allClean = results !== undefined && tableRows.length === 0 && links.length > 0
    ...
    <section className="mt-4 rounded-lg border border-[var(--edh-border)] bg-white p-4">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {tableRows.map((f) => {
              ...
              return (
                <tr key={f.id} className="border-t border-[var(--edh-border)] align-top">
                  <td className="py-1.5 pr-2">
                    <SeverityBadge severity={f.severity} />
                  </td>
                  <td className="py-1.5 pr-2">
                    <div className="font-mono text-xs font-semibold">{label}</div>
                    <div className="font-mono text-[10px] text-slate-500">({checkId})</div>
                  </td>
                  <td className="break-all py-1.5 pr-2 font-mono text-xs">
                    {domainsForFinding(f, links)}
                  </td>
                  <td className="py-1.5 pr-2 text-xs text-slate-600">{f.evidence ?? f.detail}</td>
                  <td className="py-1.5 text-right">
                    <span className="inline-flex items-center gap-1">
                      ...
                      {f.remediation && <CopyFixButton text={f.remediation} label="Copy" />}
                    </span>
                  </td>
                </tr>
              )
      ...
      {availabilityNotes.length > 0 && (
        <div className="mt-2 grid gap-1">
          {availabilityNotes.map((f) => (
            ...
            >
              <span className="font-medium text-amber-800">{f.title}</span> — {f.detail}
            </div>
          ))}
        </div>
      )}
      ...
      {links.length > 0 && (
        <div className="mt-3">
          ...
          >
            <ChevronRight
              className={cn("h-4 w-4 transition-transform", showLinks && "rotate-90")}
            />
          ...
          {showLinks && (
            <ul className="mt-2 grid gap-1">
              {links.map((l) => (
                ...
                >
                  <span className="break-all font-mono text-xs text-slate-700">{l.url}</span>
      ...
      <p className="mt-2 text-xs text-[var(--edh-muted)]">
        ...
        {results.zonesQueried.length} URI zone{results.zonesQueried.length === 1 ? "" : "s"}
