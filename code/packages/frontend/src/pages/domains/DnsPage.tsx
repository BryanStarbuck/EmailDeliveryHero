...
const WORST_RANK: Record<Severity, number> = { ok: 0, info: 1, warning: 2, critical: 3 }
...
function familyTrends(
  ...
  for (const fam of current) {
    ...
    out[fam.def.key] =
      nowRank > beforeRank ? "regressed" : nowRank < beforeRank ? "improved" : "unchanged"
  }
  return out
}
                ...
                {h.reachable == null ? (
                  ...
                  <span
                    className={cn(
                      ...
                      h.reachable ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700",
                    )}
                  >
                    :25 {h.reachable ? "✓" : "✗"}
                  </span>
        ...
        <div className="space-y-2 text-sm">
          ...
          {zone.ns.length === 0 ? (
            <div>
              ...
              <span className="font-mono text-xs">(none of its own — served by parent zone)</span>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                ...
                <tbody>
                  {zone.ns.map((n) => (
                    <tr key={n.host} className="border-t border-[var(--edh-border)]">
                      ...
                      <td className="py-1 pr-2 font-mono">{n.net_group || "—"}</td>
                      <td
                      ...
                      <td className="py-1">
                        {n.lame ? <span className="text-red-600">yes ✗</span> : "no"}
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              <div className="mt-1 text-xs text-[var(--edh-muted)]">
                {zone.ns_count} server{zone.ns_count === 1 ? "" : "s"}, {zone.network_count} network
...
function RegRow({
  ...
  return (
    <tr className="border-t border-[var(--edh-border)] first:border-t-0">
      <td className="py-1 pr-3 align-top font-medium text-[var(--edh-muted)]">{label}</td>
...
const DANGER_STATUSES = new Set(["serverHold", "clientHold", "pendingDelete", "redemptionPeriod"])
...
function RegistrationSummary({
  ...
  return (
    <div className="border-t border-[var(--edh-border)] px-3 py-2">
      <div className="flex items-center justify-between">
        <span className="text-xs font-semibold text-slate-600">Domain Registration</span>
      ...
      <table className="mt-1 w-full text-xs">
        <tbody>
          ...
          <RegRow
            ...
            note={reg.age_days !== null && reg.age_days < 30 ? "new — spam prior" : undefined}
          />
          <RegRow
            label="Expires"
            value={
          ...
          <RegRow
            label="Auto-renew"
            value={reg.auto_renew === null ? "unknown" : reg.auto_renew ? "on" : "off"}
            tone={reg.auto_renew === null ? "unknown" : reg.auto_renew ? "ok" : "warn"}
          />
          {statuses.map((s, i) => (
            <RegRow
              key={s}
              label={i === 0 ? "Status" : ""}
              value={s}
              tone={DANGER_STATUSES.has(s) ? "bad" : LOCK_STATUSES.has(s) ? "ok" : "unknown"}
              note={DANGER_STATUSES.has(s) ? "danger" : LOCK_STATUSES.has(s) ? "locked" : undefined}
            />
          ))}
          <RegRow
            label="DNSSEC (reg)"
          ...
          <RegRow
            ...
            tone={reg.privacy_enabled === null ? "unknown" : reg.privacy_enabled ? "ok" : "unknown"}
          />
          <RegRow
            label="Parked"
            value={
            ...
            note={reg.parked || reg.parking_nameservers ? "raises spam scores" : undefined}
          />
        </tbody>
      </table>
          ...
          families
            ...
            .map((fam) => (
              <div key={fam.def.key} id={`fam-${fam.def.key}`} className="scroll-mt-4">
                ...
                {fam.def.key === "dns_health" && fam.findings.length > 0 && fam.failCount === 0 && (
                  <p className="flex items-center gap-2 border-t border-[var(--edh-border)] bg-emerald-50 px-3 py-2 text-sm text-emerald-800">
                    ...
                    Delegation OK — 2+ diverse NS, consistent SOA, no dangling records.
                  </p>
                )}
