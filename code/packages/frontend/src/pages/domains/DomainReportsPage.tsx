...
import type { DmarcReportAggregate, Finding, TlsRptReportAggregate } from "@/api/types"
...
export function DomainReportsPage() {
  ...
  const dmarcFindings = (view?.findings ?? []).filter((f) => f.id.startsWith("dmarc."))
  const tlsFindings = (view?.findings ?? []).filter((f) => f.id.startsWith("infra."))
  const neverIngested =
    view && view.dmarc.totalReportsStored === 0 && view.tlsrpt.totalReportsStored === 0
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="mb-4 flex items-center justify-between">
        ...
        >
          <Download className={ingest.isPending ? "h-4 w-4 animate-pulse" : "h-4 w-4"} />
          Ingest now
        </button>
      </div>
      ...
      <p className="mb-6 text-sm text-[var(--edh-muted)]">
        The report emails receivers sent back — DMARC aggregate (rua) and TLS-RPT — turned into
        problems and fixes.{" "}
        {view && (
          <>
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-dashed border-[var(--edh-border)] p-10 text-center">
          <p className="text-slate-600">Could not load the ingested reports for this domain.</p>
        </div>
      ) : neverIngested ? (
    ...
    <section className="overflow-hidden rounded-lg border border-[var(--edh-border)] bg-white">
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-[var(--edh-border)] bg-slate-50 px-4 py-3">
        ...
        <span className="text-xs text-[var(--edh-muted)]">
          ...
          {agg.policyPublished && (
            <>
              {" "}
              · p={agg.policyPublished.p}; adkim={agg.policyPublished.adkim}; aspf=
      ...
      {agg.reportCount === 0 ? (
        <p className="p-4 text-sm text-slate-600">
          ...
          {agg.totalReportsStored > 0 && <> ({agg.totalReportsStored} older report(s) stored)</>}.
        </p>
      ) : (
...
function TlsRptGroup({ agg, findings }: { agg: TlsRptReportAggregate; findings: Finding[] }) {
  return (
    <section className="overflow-hidden rounded-lg border border-[var(--edh-border)] bg-white">
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-[var(--edh-border)] bg-slate-50 px-4 py-3">
        ...
        <span className="text-xs text-[var(--edh-muted)]">
          {agg.reportCount} report{agg.reportCount === 1 ? "" : "s"}
          {agg.reporters.length > 0 && <> · {agg.reporters.join(", ")}</>}
          {agg.policyTypes.length > 0 && <> · policy: {agg.policyTypes.join(", ")}</>} ·{" "}
      ...
      {agg.reportCount === 0 ? (
        ...
        <ul className="divide-y divide-[var(--edh-border)]">
          {findings.map((f) => (
            <ReportFindingRow key={f.id} finding={f} detailTable={<TlsRptDayTable agg={agg} />} />
          ))}
        </ul>
      )}
    </section>
            ...
            >
              ...
              <td className="px-2 py-1">
                {r.failureDetails.length === 0
                  ? "—"
                  : r.failureDetails.map((d) => `${d.resultType} ×${d.count}`).join(", ")}
              </td>
            </tr>
          ))}
        </tbody>
