...
import { useAuditRun, useAuditRuns, useAuditResults } from "@/api/audit"
...
export function DmarcCheckPage() {
  ...
  const indexInRail = domainRuns.findIndex((r) => r.runId && r.runId === run?.runId)
  ...
  const backToDmarc = (): void => {
    if (runId && run?.runId) {
      navigate({ to: "/domains/$id/runs/$runId/dmarc", params: { id, runId: run.runId } })
    } else {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mt-1 text-sm text-[var(--edh-muted)]">
        <span className="font-medium text-slate-900">{name}</span> · DMARC › sub-test
      </div>
      ...
      {run && (
        <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-[var(--edh-muted)]">
          <span className="tabular-nums">Run {fmtStamp(run.startedAt ?? run.ranAt)}</span>
          <span>·</span>
          <button
            type="button"
      ...
      <Block title="What this is">
        {unit.whatItIs.map((p) => (
          <p key={p.slice(0, 32)} className="text-sm leading-relaxed text-slate-700">
      ...
      >
        {!run ? (
          <p className="text-sm text-slate-600">No audit yet — run one to measure this unit.</p>
        ) : result === null ? (
            ...
            {unitTests.map((t) => (
              ...
              >
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="font-mono text-xs text-[var(--edh-muted)]">{t.id}</span>
                  <span className="text-sm font-medium">{t.title}</span>
                </div>
                {t.detail && <p className="mt-1 text-sm text-slate-600">{t.detail}</p>}
                {t.evidence && (
        ...
        {run && record?.raw_record && (
          <div className="mt-3">
            <p className="mb-1 text-xs font-medium uppercase text-[var(--edh-muted)]">
              Raw record{unit.tags.length > 0 ? " — this unit's tags highlighted" : ""}
            </p>
      ...
      <Block title="What it means">
        <p className="text-sm leading-relaxed text-slate-700">
          {(result === "fail" || result === "warn" ? unit.meaningFail : unit.meaningPass).replaceAll(
            "<domain>",
            name,
          )}
        </p>
      ...
      <Block title="What you can do about it">
        {result === "pass" ? (
          <>
            <p className="text-sm text-slate-700">Nothing to do — keep it this way.</p>
            ...
            {unitTests
              ...
              .map((t) => (
                <li key={`fix-${t.id}-${t.title}`}>
                  ...
                  <CopyFixButton text={t.dns_value_expected ?? t.fix ?? ""} label="Copy" />
                </li>
              ))}
            {unit.fixSteps.map((s) => (
              <li key={s.slice(0, 32)}>{s.replaceAll("<domain>", name)}</li>
      ...
      <Block title="Run this check now">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            onClick={onRunNow}
            disabled={busy}
            title={isReportsUnit ? "Check for new reports" : `Re-runs all checks for ${name}`}
        ...
        {isReportsUnit && ingest.isError && (
          <p className="mt-2 text-sm text-red-600">The ingestion poll failed — try again.</p>
        )}
      </Block>
        ...
        <Block title="History">
          <div className="flex items-center gap-2 text-xs text-[var(--edh-muted)]">
            <UnitHistoryStrip
              ...
              domainId={id}
            />
            <span>this sub-test, last 10 runs — click a chip to view that run</span>
          </div>
        </Block>
      )}
...
function unitTestRows(
  unit: DmarcCheckUnit,
  tests: DmarcTestRow[],
  findings: { id: string; title: string; severity: string; detail?: string; evidence?: string; remediation?: string }[],
): DmarcTestRow[] {
  const owns = (id: string): boolean =>
    unit.findingIds.includes(id) ||
    (unit.prefixIds ?? []).some((p) => id === p.replace(/\.$/, "") || id.startsWith(p))
...
function ResultChip({ result, small }: { result?: UnitResult | null; small?: boolean }) {
  const style: Record<string, { cls: string; label: string; icon: React.ReactNode }> = {
    pass: {
      ...
      {raw.split(";").map((token, i) => {
        ...
        return (
          <span key={`${token}-${String(i)}`}>
            {i > 0 && ";"}
            <span className={hit ? "rounded bg-amber-100 font-semibold text-amber-900" : undefined}>
              {token}
            </span>
          </span>
        )
...
const FIELD_META: Record<string, { meaning: string; fallback?: (r: DmarcResults) => string }> = {
  ...
  t: { meaning: "Testing flag — t=y disables enforcement", fallback: () => "n (enforced)" },
  adkim: { meaning: "DKIM alignment: r relaxed / s strict", fallback: () => "r (relaxed)" },
  aspf: { meaning: "SPF alignment: r relaxed / s strict", fallback: () => "r (relaxed)" },
  pct: { meaning: "Percent of mail the policy applies to (obsolete)", fallback: () => "100" },
  rua: { meaning: "Where aggregate reports are sent" },
  ruf: { meaning: "Where failure reports are sent (optional)" },
  fo: { meaning: "Failure-report options (1 = either mechanism fails)", fallback: () => "0" },
  ri: { meaning: "Aggregate report interval, seconds (obsolete)", fallback: () => "86400" },
...
function ScopedFieldTable({ unit, record }: { unit: DmarcCheckUnit; record: DmarcResults }) {
  return (
        ...
        {unit.tags.map((tag) => {
          ...
          return (
            <tr key={tag} className="border-t border-[var(--edh-border)]">
              <td className="py-1.5 pr-3 align-top font-mono text-xs font-semibold">{tag}</td>
              <td
                className={cn(
