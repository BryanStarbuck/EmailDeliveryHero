...
export function DomainsPage() {
	...
	>(null);
	const [confirmRemove, setConfirmRemove] = useState<MonitoredDomain | null>(
		null,
	);
				...
				return (
					<div className="flex items-center justify-end gap-1 text-slate-500">
						...
						>
							<Play
								className={
									row.original.running ? "h-4 w-4 animate-pulse" : "h-4 w-4"
								}
							/>
						</IconBtn>
						<IconBtn
							label="Edit"
							onClick={() => setDialog({ mode: "edit", domain: d })}
						>
						...
						{table.getHeaderGroups().map((hg) => (
							<tr key={hg.id}>
								{hg.headers.map((h) => (
									<th
										key={h.id}
										className={TH_CLASS[h.column.id] ?? "px-2 py-2 text-center"}
									>
										{h.column.getCanSort() ? (
											<button
												type="button"
...
function ScheduleToggle({
	...
	return (
		<button
			...
			title={
				enabled ? "On the recurring schedule" : "Off the recurring schedule"
			}
			onClick={(e) => {
				e.stopPropagation();
				onToggle();
		...
		>
			<div className="w-full max-w-md rounded-lg border border-[var(--edh-border)] bg-white p-5 shadow-xl">
				...
				<p className="mt-2 text-sm text-[var(--edh-muted)]">
					This also removes its audit history.
				</p>
...
function validateSelectors(raw: string): string | null {
	...
	return bad
		? `“${bad}” is not a valid DKIM selector (letters, digits, hyphens)`
		: null;
}
...
function validateAlignedLinkDomains(raw: string): string | null {
	...
	return bad
		? `“${bad}” is not a valid domain (enter bare domains, e.g. clicks.example.net)`
		: null;
}
...
function validateExpectedMxHosts(raw: string): string | null {
	const bad = splitList(raw).find(
		(h) => !DOMAIN_RE.test(h.toLowerCase().replace(/\.$/, "")),
	);
	return bad
		? `“${bad}” is not a valid MX hostname (e.g. aspmx.l.google.com)`
		: null;
}
...
function DomainDialog({
	...
	const [usesForwarding, setUsesForwarding] = useState(
		domain?.arc?.usesForwarding ?? false,
	);
	const [forwarders, setForwarders] = useState<ArcForwarderConfig[]>(
		domain?.arc?.forwarders ?? [],
	);
	...
	const [bimiSelectors, setBimiSelectors] = useState(
		(domain?.bimi?.selectors ?? []).join(", "),
	);
	const [bimiSample, setBimiSample] = useState(
		domain?.bimi?.sampleMessage ?? "",
	);
	...
	const [dnsSkipAxfr, setDnsSkipAxfr] = useState(
		domain?.dnsHealth?.skipAxfrProbe ?? false,
	);
	...
	const [mxReceivesMail, setMxReceivesMail] = useState(
		domain?.mx?.receivesMail ?? true,
	);
	const [mxExpectedHosts, setMxExpectedHosts] = useState(
		(domain?.mx?.expectedHosts ?? []).join(", "),
	);
	const [mxSkipSmtpProbe, setMxSkipSmtpProbe] = useState(
		domain?.mx?.skipSmtpProbe ?? false,
	);
	...
	const [repBrands, setRepBrands] = useState(
		(domain?.domainReputation?.brands ?? []).join(", "),
	);
	...
	const [repCousinScan, setRepCousinScan] = useState(
		domain?.domainReputation?.cousinScan ?? false,
	);
	...
	const [unsubBulkSender, setUnsubBulkSender] = useState(
		domain?.listUnsub?.isBulkSender ?? false,
	);
	...
	const expiryDaysError = validateThresholdDays(
		repExpiryDays,
		"Expiry warning threshold",
	);
	const ageDaysError = validateThresholdDays(
		repAgeDays,
	...
	const onSubmit = (e: React.FormEvent) => {
		...
		const bimi = {
			selectors: splitList(bimiSelectors),
			sampleMessage: bimiSample.trim(),
		};
		const dnsHealth = {
		...
		const domainReputation = {
			...
			...(repExpiryDays.trim() !== ""
				? { expiryWarnDays: Number(repExpiryDays) }
				: {}),
		...
		const mx = {
			...
			expectedHosts: splitList(mxExpectedHosts).map((h) =>
				h.toLowerCase().replace(/\.$/, ""),
			),
			skipSmtpProbe: mxSkipSmtpProbe,
		};
		const mxConfigured =
			!mxReceivesMail || mx.expectedHosts.length > 0 || mxSkipSmtpProbe;
		...
		const listUnsub = {
			isBulkSender: unsubBulkSender,
			probeUnsubEndpoint: unsubProbeEndpoint,
		};
		...
		const input: CreateDomainInput = {
			...
			...(bimi.selectors.length > 0 || bimi.sampleMessage !== ""
				? { bimi }
				: {}),
						...
						<div className="grid gap-3">
							...
							<label className="flex items-center gap-2 text-sm">
								...
								<span className="font-medium">
									Include in recurring scheduled checks
								</span>
							</label>
                  ...
                  capture probe. */}
							<fieldset className="rounded-md border border-[var(--edh-border)] p-3">
								...
								<label className="flex items-center gap-2 text-sm">
									...
									/>
									<span className="font-medium">
										Sends through forwarders / mailing lists (DMARC-breaking
										hops)
									</span>
								</label>
								{usesForwarding && (
											...
											>
												...
												<div className="grid grid-cols-2 gap-2">
													<input
														value={f.label}
														onChange={(e) =>
															patchForwarder(setForwarders, i, {
                  ...
                  expected-NS allow-list (drift alerts), and the skip-AXFR-probe toggle. */}
							<fieldset className="rounded-md border border-[var(--edh-border)] p-3">
								...
								<label className="mt-3 flex items-center gap-2 text-sm">
									...
									/>
									<span className="font-medium">
										Skip the AXFR zone-transfer probe
									</span>
								</label>
							</fieldset>
                  ...
                  egress blocks port 25. */}
							<fieldset className="rounded-md border border-[var(--edh-border)] p-3">
								...
								<label className="flex items-center gap-2 text-sm">
									...
									/>
									<span className="font-medium">
										This domain receives mail (a missing or null MX is then
										critical)
									</span>
								</label>
								{!mxReceivesMail && (
									<p className="mt-1 pl-6 text-xs text-[var(--edh-muted)]">
										Send-only domain: an absent MX is accepted and an RFC 7505
										null MX (<code className="font-mono">MX 0 "."</code>) is
										reported as correct.
									</p>
								)}
								<label className="mt-3 block text-sm">
								...
								<label className="mt-3 flex items-center gap-2 text-sm">
									...
									/>
									<span className="font-medium">
										Skip the SMTP reachability probe (port-25 egress blocked
										here)
									</span>
								</label>
							</fieldset>
                  ...
                  registrant-intentionally-public silencer, and the future cousin-scan toggle. */}
							<fieldset className="rounded-md border border-[var(--edh-border)] p-3">
								...
								<label className="mt-3 flex items-center gap-2 text-sm">
									...
									/>
									<span className="font-medium">
										Registrant contact is intentionally public (silences the
										privacy note)
									</span>
								</label>
							...
							<fieldset className="rounded-md border border-[var(--edh-border)] p-3">
								...
								<label className="block text-sm">
									...
									<span className="mt-1 block text-xs text-[var(--edh-muted)]">
										Tracking/click/CDN domains your org controls; links to them
										are not flagged off-brand by the body-link alignment check.
									</span>
								</label>
							</fieldset>
                  ...
                  unsubscribe the sampled recipient). */}
							<fieldset className="rounded-md border border-[var(--edh-border)] p-3">
								...
								<label className="flex items-center gap-2 text-sm">
									...
									/>
									<span className="font-medium">
										Bulk sender (&gt; 5,000 msgs/day) — escalates a missing
										one-click header to critical
									</span>
								</label>
								<label className="mt-3 flex items-center gap-2 text-sm">
									<input
									...
									/>
									<span className="font-medium">
										Probe the one-click unsubscribe endpoint (live RFC 8058
										POST)
									</span>
								</label>
								{unsubProbeEndpoint && (
									<p className="mt-1 pl-6 text-xs text-amber-700">
										Warning: a live one-click POST may unsubscribe the sampled
										recipient. Use a seed/test recipient's sample where
										possible. Requires the probe to be globally permitted in
										Settings → Admin.
									</p>
								)}
							</fieldset>
