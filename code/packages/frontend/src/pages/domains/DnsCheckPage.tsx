...
export function DnsCheckPage() {
	...
	const toolRuns =
		(run?.results?.["infra.tool_runs"] as InfraToolRun[] | undefined) ?? [];
	...
	return (
		<div className="mx-auto max-w-3xl">
			...
			>
				<ArrowLeft className="h-4 w-4" /> Back to DNS & Infrastructure for{" "}
				{name}
			</button>
			...
			{!explainer || !familyDef ? (
                ...
                a fresh observation, never persisted into the immutable run history. */}
					...
					<Section
						title={
							run
								? `Current state — run ${fmtStamp(run.startedAt ?? run.ranAt)}`
								: "Current state"
						}
					>
						{!run ? (
					...
					{spot.data && (
						<Section
							title={`Spot check — ${fmtStamp(spot.data.finishedAt)} (live, not saved)`}
						>
							<p className="mb-2 text-xs text-[var(--edh-muted)]">
								A fresh observation of just this family. It is never written
								into the run history — re-run the full audit to record a new
								run.
							</p>
							...
							<ul className="space-y-1">
								{toolRuns.map((tr) => (
									...
									>
										...
										{tr.error ? (
											<span className="text-red-600"> · {tr.error}</span>
										) : null}
									</li>
								))}
							</ul>
