...
import type { ArcForwarderObservation, ArcResults, AuditResult, Finding, Severity } from "@/api/types"
...
function chipFor(
  ...
  if (worst === "critical") return { label: "✗ signer key", className: "bg-red-100 text-red-700" }
  if (worst === "warning")
    return { label: "⚠ unverified", className: "bg-amber-100 text-amber-700" }
  if (worst === "info") {
    if (arc?.applicable === false) return { label: "ⓘ N/A", className: "bg-slate-100 text-slate-600" }
    if (arc?.applicable === null || arc === undefined)
      return { label: "ⓘ undetermined", className: "bg-slate-100 text-slate-600" }
...
const REFERENCES: { label: string; href: string }[] = [
  ...
  { label: "RFC 8301 (key strength)", href: "https://www.rfc-editor.org/rfc/rfc8301" },
  { label: "RFC 6376 (DKIM signatures)", href: "https://www.rfc-editor.org/rfc/rfc6376" },
  { label: "DMARCbis (RFC 9989)", href: "https://www.rfc-editor.org/rfc/rfc9989" },
  { label: "dmarc.org — ARC overview", href: "https://dmarc.org/presentations/ARC-Overview.pdf" },
...
export function ArcCheckPage() {
  ...
  const onRunNow = (): void => {
    ...
    runDomains([{ id, name }])
    if (runId)
      navigate({ to: "/domains/$id/dmarc/check/$checkKey", params: { id, checkKey: "arc" } })
  }
  ...
  const failingSelectors = forwarders.filter((f) => f.selectorResolves === false)
  const weakKeys = arcFindings.filter(
    (f) => f.id.startsWith("arc.signature_algorithm") && f.severity === "warning",
  )
    ...
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mb-4 flex items-center justify-between">
        <button
          ...
          onClick={() =>
            runId && result?.runId
              ? navigate({ to: "/domains/$id/runs/$runId/dmarc", params: { id, runId: result.runId } })
      ...
      {result && (
        <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-[var(--edh-muted)]">
          <span className="tabular-nums">Run {fmtStamp(result.startedAt ?? result.ranAt)}</span>
          <span>·</span>
          <button
            type="button"
      ...
      <Block title="What this is">
        <p className="text-sm leading-relaxed text-slate-700">
          ARC (Authenticated Received Chain, RFC 8617) lets a forwarder or mailing list record
          "this message passed SPF/DKIM/DMARC when it reached me," sign that record, and pass it
          downstream. Three headers cooperate:{" "}
          <span className="font-mono text-xs">ARC-Authentication-Results</span> (the snapshot of
          the results the intermediary observed),{" "}
          <span className="font-mono text-xs">ARC-Message-Signature</span> (a DKIM-style signature
          over the message as received), and <span className="font-mono text-xs">ARC-Seal</span>{" "}
          (a signature over the whole chain so far, carrying{" "}
          <span className="font-mono text-xs">cv=</span>).
        </p>
        <p className="mt-2 text-sm leading-relaxed text-slate-700">
          It exists because forwarding breaks DMARC: the envelope changes (SPF alignment lost) and
          the body or subject often change —{" "}
          <Link
            to={runId && result?.runId ? "/domains/$id/runs/$runId/dkim" : "/domains/$id/dkim"}
            params={runId && result?.runId ? { id, runId: result.runId } : { id }}
            className="text-[var(--edh-primary)] underline"
          >
            breaking the DKIM signature
          </Link>
          . Concretely: mail from billing@{name} to a Google Group arrives at Gmail failing both,
          and under p=reject it is rejected even though it is legitimate.
        </p>
        <p className="mt-2 text-sm leading-relaxed text-slate-700">
          The crucial caveat: ARC is <em>not a DNS record you publish</em> and <em>not a
          guarantee</em> — intermediaries add it in transit, and receivers <em>may</em> honor a
          valid chain (cv=pass) from a forwarder they trust. That is why this unit is advisory:
          EmailDeliveryHero verifies the pieces that live in DNS/config today, and the chain itself
          only from a captured forwarded message.
        </p>
      </Block>
      ...
      <Block
        ...
        right={
          <span className={cn("rounded-full px-3 py-1 text-sm font-semibold", chip.className)}>
            {chip.label}
          </span>
        }
      >
        {!result ? (
          <p className="text-sm text-slate-600">No audit yet — run one below.</p>
        ) : notMeasured ? (
          <p className="text-sm text-slate-600">
            The ARC checker did not run in this audit (disabled, errored before emitting, or the
            run predates ARC). Re-run below to measure it.
          </p>
        ) : (
          <>
              ...
              <table className="w-full text-sm">
                <tbody>
                  <tr className="border-b border-[var(--edh-border)]">
                    <td className="w-44 px-3 py-1.5 text-xs font-semibold">DMARC policy</td>
                    <td className="px-3 py-1.5">
                      {policy ? (
                        ...
                        <span className="font-mono text-xs text-slate-400">— not read</span>
                      )}
                      <span className="ml-2 text-xs text-slate-600">
                        → enforcing: {policy ? (enforcing ? "yes" : "no") : "unknown"}
                      </span>
                    </td>
                  ...
                  <tr className="border-b border-[var(--edh-border)]">
                    <td className="px-3 py-1.5 text-xs font-semibold">Forwarding declared</td>
                    <td className="px-3 py-1.5 text-xs text-slate-700">
                      {usesForwarding ? "declared" : "not declared"} · {declaredCount} forwarder
                      {declaredCount === 1 ? "" : "s"}
                    </td>
                  ...
                  <tr>
                    <td className="px-3 py-1.5 text-xs font-semibold">Verdict</td>
                    <td className="px-3 py-1.5 font-mono text-xs text-slate-700" colSpan={2}>
                      applicable ={" "}
                      {arc?.applicable === null || arc?.applicable === undefined
                        ? "— not determined"
                        : String(arc.applicable)}{" "}
                      · forwardingRisk ={" "}
                      {arc?.forwardingRisk === null || arc?.forwardingRisk === undefined
                        ? "—"
            ...
            <ul className="mt-3 space-y-2">
              {arcFindings.map((f) => (
                <ArcFindingRow key={f.id} finding={f} id={id} runId={result.runId} />
              ))}
            </ul>
      ...
      <Block title="What it means">
        {!result || notMeasured ? (
          <p className="text-sm text-slate-700">
            Nothing measured yet — the concept above still applies; re-run the audit to see where
            this domain stands.
          </p>
        ) : arc?.applicable === false && !enforcing ? (
          <p className="text-sm text-slate-700">
            DMARC is not enforcing (p={policy ?? "none/absent"}), so nothing is being rejected and
            there is nothing for ARC to rescue; the real work is on the DMARC policy ladder.
            Deliverability impact today from ARC: none.
          </p>
        ) : arc?.applicable === false ? (
          <p className="text-sm text-slate-700">
            No forwarding is declared, and directly-sent mail never needs ARC. Impact: none —{" "}
            <em>unless</em> this domain actually does send through lists it hasn't declared, in
            which case losses are invisible. If that is possible, declare the forwarders in
            settings.
          </p>
        ) : (
          <div className="space-y-2 text-sm text-slate-700">
            {failingSelectors.length > 0 && (
              <p>
                <strong>Signer key broken:</strong> the forwarder may be sealing, but receivers
                cannot verify the seal — cryptographically equivalent to no ARC at all. The ARC
                evidence is discarded and mail through{" "}
                {failingSelectors.map((f) => f.label).join(", ")} falls back to the raw DMARC
                failure under p={policy ?? "?"}.
              </p>
            )}
            {weakKeys.length > 0 && (
              <p>
                <strong>Weak/legacy signer key:</strong> verifiers increasingly refuse &lt;1024-bit
                or non-rsa/ed25519 keys (RFC 8301), so the chain's protection erodes receiver by
                receiver.
              </p>
            )}
            {arc?.forwardingRisk && (
              <p>
                <strong>Unverified forwarding path:</strong> the classic silent failure — "some
                recipients never get our mail, but only via{" "}
                {forwarders.length > 0
                  ? forwarders.map((f) => f.label).join(" / ")
                  : "the declared paths"}
                ." Under p={policy ?? "quarantine/reject"}, forwarded legitimate mail is being
                spam-foldered or rejected at strict receivers until each forwarder demonstrably
                seals a valid chain (cv=pass).
              </p>
            )}
            {worst === "ok" && (
              <p>
                A cv=pass chain was verified on every declared path — forwarded mail survives
                p=reject on those hops. Residual caveat: honoring is per-receiver trust; ARC
                supplies evidence, it does not compel delivery.
              </p>
            )}
          </div>
        )}
      ...
      <Block title="What you can do about it">
        {!result || notMeasured ? (
          <p className="text-sm text-slate-700">
            Re-run the audit (below) — that is exactly the fix for an unmeasured unit.
          </p>
        ) : arc?.applicable === false ? (
          <div className="text-sm text-slate-700">
            <p className="font-medium">Nothing to do for ARC right now.</p>
            <p className="mt-1">
              Revisit when moving to p=quarantine/reject or when mail starts flowing through a
              list/forwarder. If forwarding exists but was never declared,{" "}
              <Link
                to="/domains"
                search={{ edit: id }}
                className="text-[var(--edh-primary)] underline"
          ...
          <ol className="list-decimal space-y-2 pl-5 text-sm text-slate-700">
            <li>Retry the audit (the button below) — the applicability lookup failed transiently.</li>
          </ol>
        ) : (
          <ol className="list-decimal space-y-3 pl-5 text-sm text-slate-700">
            {forwarders.length === 0 && (
              <li>
                Register each forwarder / mailing list (label + forwarding address + ARC signer
                d=/s= if known) in the{" "}
                <Link
                  to="/domains"
                  search={{ edit: id }}
            ...
            {(failingSelectors.length > 0 ? failingSelectors : forwarders).map((fw) => {
              const qname =
              ...
              return (
                <li key={`fix-${fw.label}`}>
                  ...
                  <div className="mt-1 flex items-center justify-between gap-2 rounded-md bg-slate-50 p-2">
                    <code className="break-all font-mono text-xs text-slate-700">{txt}</code>
                    <CopyFixButton text={txt} label="Copy" />
                  </div>
                  <div className="mt-1 flex items-center justify-between gap-2 rounded-md bg-slate-50 p-2">
                    <code className="break-all font-mono text-xs text-slate-700">{verify}</code>
                    <CopyFixButton text={verify} label="Copy" />
                  </div>
                  <p className="mt-1 text-xs text-[var(--edh-muted)]">
                    DNS changes take up to the record's TTL to propagate before receivers see them.
                  </p>
                </li>
              )
            })}
            <li>
              Third-party forwarder (Google Groups, a listserv you don't run)? Ask them to enable
              ARC sealing and tell you the d=/s= they sign with; record those in settings so the
              key check runs here. Google Groups and Microsoft 365 seal by default — record their
              observed signer once a sample is captured.
            </li>
            <li>
              Verify with the capture probe once available, or manually: forward a test message
              through the path and inspect its headers for{" "}
      ...
      <Block title="Run this check now">
        ...
        <p className="mt-2 text-xs text-[var(--edh-muted)]">
          Re-runs the DMARC + ARC checks for {name} and refreshes this page. Keyboard:{" "}
          <kbd className="rounded border border-[var(--edh-border)] px-1">r</kbd>
        </p>
      </Block>
...
function ArcFindingRow({
  ...
  const [open, setOpen] = useState(f.severity === "critical" || f.severity === "warning")
...
function SignerKeyCard({
  ...
  if (!fw.signerDomain || !fw.signerSelector) {
    return (
      <div className="rounded-md border border-dashed border-[var(--edh-border)] p-3 text-sm text-[var(--edh-muted)]">
        ARC signer not yet known for <span className="font-medium">{fw.label}</span> —{" "}
        <Link
          to="/domains"
  ...
  const raw =
    fw.rawKeyRecord ??
    findings.find((f) => f.id.startsWith("arc.selector_dns") && f.evidence?.includes("p="))
      ?.evidence ??
    null
  const badge =
    fw.selectorResolves === true ? (
      ...
      <span className="text-xs font-medium text-slate-500">ⓘ lookup failed</span>
    )
  return (
    <div className="rounded-md border border-[var(--edh-border)] p-3">
      <div className="flex items-center justify-between gap-2">
        <span className="break-all font-mono text-xs font-semibold">
          SIGNER KEY — {qname}{" "}
          <span className="font-sans font-normal text-[var(--edh-muted)]">({fw.label})</span>
        </span>
        {badge}
      </div>
...
function HighlightTags({ record }: { record: string }) {
  ...
  return (
    <>
      {parts.map((part, i) =>
        /^(?:;)?\s*(?:p|k)\s*=$/i.test(part.trim()) || /(?:p|k)\s*=$/i.test(part) ? (
...
function ParsedFieldTable({ arc }: { arc: ArcResults | undefined }) {
  ...
  const val = (v: boolean | number | string | null | undefined): React.ReactNode =>
        ...
        <tbody>
          {rows.slice(0, 2).map((r) => (
            <tr key={r.field} className="border-t border-[var(--edh-border)]">
              <td className="px-3 py-1.5 align-top font-mono text-xs font-semibold">{r.field}</td>
              <td className="px-3 py-1.5 align-top font-mono text-xs">{r.value}</td>
              <td className="px-3 py-1.5 align-top text-xs text-slate-500">{r.meaning}</td>
            </tr>
          ))}
                  ...
                  <tbody>
                    {forwarders.map((fw) => (
                      <tr key={fw.label + fw.forwardAddress}>
                        ...
                        <td className="pr-2">
                          {fw.selectorResolves === null ? "—" : fw.selectorResolves ? "✓" : "✗"}
                        </td>
                        <td>
                          {fw.keyType ?? "—"}
