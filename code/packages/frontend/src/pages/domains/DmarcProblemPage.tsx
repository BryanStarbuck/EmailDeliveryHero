...
function ArcRunData({ arc }: { arc: ArcResults | undefined }) {
	...
	const rows: [string, string][] = [
		[
			"applicable",
			arc.applicable === null ? "— not determined" : String(arc.applicable),
		],
		[
			"forwardingRisk",
			arc.forwardingRisk === null ? "—" : String(arc.forwardingRisk),
		],
		["dmarcPolicy", arc.dmarcPolicy ? `p=${arc.dmarcPolicy}` : "—"],
		["policySource", arc.policySource ?? "—"],
		["messageSampleId", sampled(arc.messageSampleId)],
		[
			"chainPresent / chainLength",
			`${sampled(arc.chainPresent)} / ${sampled(arc.chainLength)}`,
		],
		[
			"cvResult / sealValid / amsValid",
			`${sampled(arc.cvResult)} / ${sampled(arc.sealValid)} / ${sampled(arc.amsValid)}`,
		],
		[
			"instancesOk / oldestPass",
			`${sampled(arc.instancesOk)} / ${sampled(arc.oldestPass)}`,
		],
						...
						<tbody>
							{arc.forwarders.map((fw) => (
								...
								>
									...
									<td className="px-3 py-1.5 font-mono text-xs">
										{fw.keyType
											? `${fw.keyType}${fw.keyBits ? ` ~${fw.keyBits}-bit` : ""}`
											: "—"}
									</td>
								</tr>
							))}
