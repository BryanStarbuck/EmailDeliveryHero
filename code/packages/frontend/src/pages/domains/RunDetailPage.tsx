 ...
 */
const RunDiffContext = createContext<{
	diff: RunDiff;
	mode: RegressionMode;
} | null>(null);
 ...
 */
function categoryPageLinkProps(
	catKey: CategoryKey,
	id: string,
	runId?: string,
) {
	if (runId) {
		switch (catKey) {
			case "spf":
				return {
					to: "/domains/$id/runs/$runId/spf" as const,
					params: { id, runId },
				};
			case "dkim":
				return {
					to: "/domains/$id/runs/$runId/dkim" as const,
					params: { id, runId },
				};
			case "dmarc":
				return {
					to: "/domains/$id/runs/$runId/dmarc" as const,
					params: { id, runId },
				};
			case "blacklists":
				return {
					to: "/domains/$id/runs/$runId/blacklists" as const,
					params: { id, runId },
				};
			case "dnsInfra":
				return {
					to: "/domains/$id/runs/$runId/dns" as const,
					params: { id, runId },
				};
			case "spamContent":
...
export function RunDetailPage() {
	...
	const openProblems = result
		? result.counts.warning + result.counts.critical
		: 0;
			...
			<div className="mx-auto max-w-4xl">
				...
				<div className="mt-6 rounded-lg border border-dashed border-[var(--edh-border)] p-10 text-center">
					...
					<p className="mt-1 text-sm text-slate-500">
						This health-check run no longer exists — it may have been pruned or
						deleted.
					</p>
					<button
						type="button"
			...
			{!result ? (
				...
				<>
					<div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-[var(--edh-muted)]">
						...
						<span>
							· started{" "}
							{new Date(result.startedAt ?? result.ranAt).toLocaleString()}
							{result.finishedAt && (
								<>
									{" "}
					...
					<div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
						{CATEGORIES.map((c) => {
							...
							const chipLink = categoryPageLinkProps(
								c.key,
								id,
								runId ?? result.runId,
							);
									...
									>
										...
										{m === "new" && diff.annotationById.size > 0 && (
											<>
												...
												(
												{
													[...diff.annotationById.values()].filter(
														(a) => a === "new",
													).length
												}
												)
										...
										{m === "resolved" && diff.resolved.length > 0 && (
							...
							{mode !== "resolved" &&
								CATEGORIES.map((c) => {
									...
									if (
										mode === "new" &&
										!findings.some(
											(f) => diff.annotationById.get(f.id) === "new",
										)
									) {
										return null;
									}
									return (
										<CategorySection
											...
											contentScore={
												c.key === "spamContent"
													? (result.results?.["content.scoring"] as
															| ContentScoreResults
															| undefined)
													: undefined
											}
								...
								<button
									...
									className={`flex w-full items-center gap-2 px-3 py-1.5 text-left text-xs hover:bg-slate-50 disabled:opacity-40 ${
										r.runId === current.runId
											? "font-semibold text-[var(--edh-primary)]"
											: ""
									}`}
								>
			...
			<ul className="space-y-2">
				{findings.map((f) => (
					...
					>
						...
						<span className="text-xs uppercase text-[var(--edh-muted)]">
							{f.checkId}
						</span>
					</li>
				))}
...
function CategorySection({
	...
	const arcFindings =
		catKey === "dmarc" ? findings.filter((f) => f.checkId === "arc") : [];
	...
	const bimiFindings =
		catKey === "spamContent"
			? findings.filter((f) => f.checkId === "content.bimi")
			: [];
	...
	const dnssecBogus =
		...
		findings.some(
			(f) =>
				f.checkId === "infra.dnssec_validates" && f.severity === "critical",
		);
          ...
          triage list (§5). Omitted when all pass. */}
			{topProblem && (
				<p className="mb-2 text-xs text-[var(--edh-muted)]">
					top problem:{" "}
					<span className="font-medium text-slate-700">{topProblem.title}</span>
				</p>
			)}
			{open && (
				<>
					{dnssecBogus && (
						...
						>
							Domain may be unresolvable — the DNSSEC chain is bogus, so
							validating resolvers (including most large mailbox providers)
							SERVFAIL every lookup for this domain.
						</div>
					)}
					{catKey === "dnsInfra" ? (
						<DnsInfraSubGroup
...
function DnsInfraSubGroup({
	...
	const families = rollupFamilies(findings).filter(
		(f) => f.findings.length > 0,
	);
	const failFirst = [...findings].sort(
	...
	return (
		<div className="space-y-3">
				...
				{families.map((fam) => (
					...
					>
						...
						<span className="ml-auto shrink-0 text-xs text-[var(--edh-muted)]">
							{fam.failCount === 0
								? "healthy"
								: `${fam.failCount} of ${fam.findings.length} fail`}
						</span>
						<Link
			...
			<div className="flex flex-wrap items-center gap-3 pt-1">
				<button
					...
					onClick={() =>
						runDns.mutate(domainId, {
							onSuccess: () =>
								toast.success("DNS & Infrastructure re-run complete"),
							onError: (err) =>
								toast.error(
									`Re-run failed: ${err instanceof Error ? err.message : err}`,
								),
						})
					}
				...
				>
					<RefreshCw
						className={`h-3.5 w-3.5 ${runDns.isPending ? "animate-spin" : ""}`}
					/>
		...
		<div className="mt-3">
			<div className="mb-2 flex flex-wrap items-center justify-between gap-2">
				<h3 className="flex items-center gap-2 text-sm font-semibold text-slate-700">
					...
					{score && gaugeStyle && (
						<span
							className={`rounded-md px-2 py-0.5 text-xs font-bold ${gaugeStyle}`}
						>
			...
			<div className="mt-2 flex flex-wrap items-center gap-2">
				...
				>
					<RefreshCw
						className={`h-3.5 w-3.5 ${rescore.isPending ? "animate-spin" : ""}`}
					/>
	...
	return (
		<div className="mt-3">
			...
			<p className="mt-2 text-xs text-[var(--edh-muted)]">
				Logo &amp; certificate validation (SVG Tiny-PS preview, VMC
				issuer/expiry) is a future round.
			</p>
		</div>
	);
}
...
function UnitDot({ result }: { result: UnitResult | null }) {
	...
	return (
		<span
			className={`h-2.5 w-2.5 shrink-0 rounded-full ${cls[result ?? ""] ?? "bg-slate-300"}`}
		/>
	);
}
...
function FindingRow({
	...
	const annotation: RunAnnotation | undefined = ctx?.diff.annotationById.get(
		f.id,
	);
