...
export function DkimPage() {
	...
	const newest =
		(results ?? []).find((r) => r.domainId === id) ?? domainRuns[0];
	...
	const idx = currentRunId
		? domainRuns.findIndex((r) => r.runId === currentRunId)
		: -1;
	...
	useEffect(() => {
		const onKey = (e: KeyboardEvent) => {
			...
			if (e.key === "ArrowLeft" && prevRun?.runId) {
				navigate({
					to: "/domains/$id/runs/$runId/dkim",
					params: { id, runId: prevRun.runId },
				});
			} else if (e.key === "ArrowRight" && nextRun?.runId) {
				navigate({
					to: "/domains/$id/runs/$runId/dkim",
					params: { id, runId: nextRun.runId },
				});
			}
		};
	...
	return (
		<div className="mx-auto max-w-5xl">
			...
			<div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-[var(--edh-muted)]">
				...
				{runStarted && (
					<span>· run {new Date(runStarted).toLocaleString()}</span>
				)}
			</div>
			...
			{isStale && (
				<div className="mt-3 flex items-center justify-between gap-2 rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-800">
					<span className="flex items-center gap-2">
						...
						Viewing the run of{" "}
						{runStarted ? new Date(runStarted).toLocaleString() : runId} — a
						newer run exists
					</span>
					<Link
						to="/domains/$id/dkim"
...
function ToolRunRow({ run }: { run: DkimToolRun }) {
	...
	return (
		...
		>
			...
			{failed && (
				<p className="break-all px-3 pb-2 pl-9 font-mono text-xs text-amber-800">
					{run.error}
				</p>
			)}
			{open && (
...
function KeyHealthHero({
	...
	if (!dkim || (probed === 0 && !dkim.discovery_ran)) {
		...
		dkim.selectors_configured.length === 0 &&
		dkim.discovery_ran &&
		probed === 0
	) {
		verdict =
			"No selectors configured and discovery found nothing — mail is likely unsigned.";
		next =
		...
		next =
			"Import the discovered selector(s) below so every future run audits them.";
		...
		next =
			"Publish the exact TXT/CNAME from your provider (see the failing test below).";
	} else if (has("dkim.parses") || has("dkim.revoked")) {
		verdict =
			"A published record is unusable (unparseable or revoked) — signatures fail.";
		next =
			"Republish the key exactly as exported, or point the signer at a live selector.";
	} else if (has("dkim.algorithm")) {
		verdict =
			"A key is restricted to SHA-1 — such signatures permanently fail.";
		...
		next =
			"Rotate to 2048-bit RSA on a new selector, then retire the weak one.";
	} else if (has("dkim.testflag")) {
		verdict =
			"A selector is in t=y test mode — receivers treat your mail as unsigned.";
		next = "Remove the test flag.";
	} else if (has("dkim.ed25519_only")) {
		verdict =
			"Only Ed25519 keys are published — Gmail/Microsoft/Yahoo cannot verify them.";
		next = "Add an RSA-2048 selector and dual-sign.";
	} else if (
		has("dkim.single_record") ||
		has("dkim.record_size") ||
		has("dkim.underscore_label")
	) {
		verdict =
			"The DNS record shape is off (extra records, oversize strings, or a wildcard).";
		next =
			"Clean the DNS: one record per selector, ≤255-byte strings, no wildcard over _domainkey.";
		...
		next =
			"Stage a second selector / rotate per M3AAWG (new key ≥48h ahead → switch → revoke).";
	} else if (has("dkim.duplicate_key")) {
		verdict =
			"The same key is published on more than one domain — shared blast radius.";
		...
		next =
			"Keep monitoring; rotation reminders fire as keys approach 6 months.";
	}
...
function keyBadge(sel: DkimSelectorResult): {
	label: string;
	className: string;
} {
	if (!sel.present)
		return { label: "missing", className: "bg-red-100 text-red-700" };
	if (sel.is_revoked)
		return { label: "revoked", className: "bg-red-100 text-red-700" };
	if (!sel.parses)
		return { label: "unparseable", className: "bg-red-100 text-red-700" };
	if (sel.key_type === "ed25519")
		return { label: "ed25519", className: "bg-slate-200 text-slate-700" };
	if (sel.key_bits === null)
		return { label: "rsa", className: "bg-slate-200 text-slate-700" };
	if (sel.key_bits >= 2048)
		return {
			label: `RSA ${sel.key_bits}`,
...
function SelectorCard({
	...
	const discovered =
		sel.source === "discovered" && !configured.includes(sel.selector);
	...
	const onImport = () =>
		update.mutate({
			id: domainId,
			input: { dkimSelectors: [...configured, sel.selector] },
		});
	...
	return (
		...
		>
			<div className="flex items-center gap-2">
				...
				<span className="ml-auto text-[10px] uppercase text-[var(--edh-muted)]">
					{sel.source}
				</span>
			</div>
...
function SelectorsEditor({
	...
	const save = (next: string[]) =>
		update.mutate({ id: domainId, input: { dkimSelectors: next } });
	const add = () => {
	...
	const newHits = (outcome?.hits ?? []).filter(
		(h) => !selectors.includes(h.selector),
	);
		...
		<section className="mt-6 rounded-lg border border-[var(--edh-border)] bg-white p-4">
			...
			{selectors.length === 0 && (
				<p className="mt-2 rounded-md bg-sky-50 p-2 text-sm text-sky-800">
					No selectors configured — discovery probes common selectors on every
					run; add yours for precise auditing of {domainName}.
				</p>
			)}
			...
			{discovery.isError && (
				<p className="mt-3 rounded-md bg-amber-50 p-2 text-sm text-amber-800">
					Discovery failed — check that the backend is reachable, then try
					again.
				</p>
			)}
			{outcome?.wildcard_shadow && (
				<p className="mt-3 rounded-md bg-amber-50 p-2 text-sm text-amber-800">
					A wildcard TXT record answers every selector name for {domainName} —
					discovery results are unreliable and were suppressed. Remove or scope
					the wildcard, then retry.
				</p>
			)}
			{outcome && !outcome.wildcard_shadow && outcome.hits.length === 0 && (
				<p className="mt-3 text-sm text-[var(--edh-muted)]">
					No published keys found at {outcome.probed} common selector names. If
					mail signs with a custom selector, read the s= tag from a real message
					(Gmail "Show original") and add it above.
				</p>
			)}
			{outcome && !outcome.wildcard_shadow && outcome.hits.length > 0 && (
					...
					<div className="flex items-center justify-between">
						...
						{newHits.length > 1 && (
							<button
								type="button"
								onClick={() =>
									save([...selectors, ...newHits.map((h) => h.selector)])
								}
								disabled={update.isPending}
		...
		>
			...
			<div className="mt-4 overflow-hidden rounded-lg border border-[var(--edh-border)] bg-white">
				{["a", "b", "c", "d"].map((k) => (
					<div
						key={k}
						className="border-t border-[var(--edh-border)] px-3 py-3 first:border-t-0"
					>
			...
			<div className="overflow-hidden rounded-lg border border-[var(--edh-border)] bg-white">
				{sorted.length === 0 ? (
					<p className="p-4 text-sm text-slate-600">
						No DKIM tests in the latest run.
					</p>
				) : (
					<ul>
						{sorted.map((f) => (
