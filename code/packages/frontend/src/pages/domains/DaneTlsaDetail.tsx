...
function FieldRow({
	...
	return (
		<tr className="border-b border-slate-100 last:border-0">
			...
			<td className="break-all py-1 pr-3 font-mono text-xs text-slate-700">
				{value}
			</td>
...
function digestChip(mtype: number, data: string) {
	...
	return bad ? (
		<Chip tone="bad">✗ malformed</Chip>
	) : (
		<Chip tone="ok">✓ {data.length} hex</Chip>
	);
}
...
export function DaneTlsaDetail({
	...
	const sorted = [...hosts].sort(
		(a, b) =>
			(a.mxPreference ?? Number.MAX_SAFE_INTEGER) -
			(b.mxPreference ?? Number.MAX_SAFE_INTEGER),
	);
	const anyPending = sorted.some(
		(h) => h.certMatch === null || h.starttlsOffered === null,
	);
		...
		<div className="space-y-6">
			{sorted.length === 0 ? (
				...
				<>
								...
								{sorted.map((h) => (
									...
									>
										...
										<td className="px-2 py-1.5">
											{h.dnssecSigned ? (
												<Chip tone="ok">
													✓ {h.rrsigObserved ? "RRSIG" : "DS/DNSKEY"}
												</Chip>
											) : h.tlsaPresent ? (
					...
					{anyPending && (
						<p className="text-xs text-[var(--edh-muted)]">{PENDING_LINE}</p>
					)}
				</>
			)}
 ...
 */
export function TlsaGeneratorPanel({
	defaultMxHost,
}: {
	defaultMxHost: string;
}) {
	...
	return (
		<div className="rounded-md border border-[var(--edh-border)] p-3">
			...
			<p className="mt-1 text-xs text-[var(--edh-muted)]">
				Paste the MX host's TLS certificate (PEM) and confirm the hostname — the
				app computes the SPKI SHA-256 digest and the exact zone-file line to
				publish.
			</p>
			...
			{gen.isError && (
				<p className="mt-2 text-sm text-red-600">{errorMessage(gen.error)}</p>
			)}
			...
			{gen.data && (
				<div className="mt-3 space-y-1 rounded bg-slate-50 p-2">
					...
					<p className="text-xs text-[var(--edh-muted)]">
						owner <span className="font-mono">{gen.data.recordName}</span> ·
						SPKI SHA-256{" "}
						<span className="font-mono">
							{gen.data.spkiSha256.slice(0, 16)}…
						</span>
					</p>
					<p className="text-xs text-[var(--edh-muted)]">
						cert subject <span className="font-mono">{gen.data.subject}</span> ·
						valid to {gen.data.validTo}
					</p>
				</div>
			)}
		</div>
