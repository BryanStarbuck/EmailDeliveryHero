...
const OS_KIND_LABEL: Record<OsSchedulerKind, { os: string; artifact: string }> =
	{
		...
		systemd: {
			os: "Linux",
			artifact: "systemd user timer (crontab shown in the preview)",
		},
...
export function ScheduledChecksPage() {
	...
	const os = config.data?.os ?? {
		kind: "launchd" as OsSchedulerKind,
		installed: false,
		label: "",
	};
		...
		<div className="mx-auto max-w-3xl">
				...
				<div className="flex items-center justify-between">
					...
					<span className="text-sm text-[var(--edh-muted)]">
						...
						<span className="font-medium text-slate-700">
							{fmtNextRun(status.data?.nextRunAt)}
						</span>
					</span>
				</div>
				<p className="mt-2 text-xs text-[var(--edh-muted)]">
					Turning the schedule off never disables the manual Run checks button
					on the dashboard.
				</p>
			</Section>
					...
					{draft.times.map((t, i) => (
						<div key={t.id} className="flex items-center gap-2 py-1">
							...
							<button
								type="button"
								onClick={() =>
									set({ times: draft.times.filter((row) => row.id !== t.id) })
								}
								disabled={
									draft.cadence === "interval" || draft.times.length <= 1
								}
			...
			<Section title="Run even when the app is closed">
				<p className="mb-2 text-sm text-[var(--edh-muted)]">
					Detected OS:{" "}
					<span className="font-medium text-slate-700">{osLabel.os}</span> (
					{osLabel.artifact})
				</p>
				<label className="flex items-center gap-2 py-1 text-sm">
					<input
