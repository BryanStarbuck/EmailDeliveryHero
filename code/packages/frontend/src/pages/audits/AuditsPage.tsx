	...
	return (
		<div className="mx-auto max-w-4xl">
			...
			<p className="mb-6 text-sm text-[var(--edh-muted)]">
				21 deliverability checks across six categories — SPF, DKIM, DMARC,
				blacklists, DNS &amp; infrastructure (MX, PTR, DNSSEC, DANE, MTA-STS,
				and more), and spam &amp; content — each with the exact fix for every
				problem found.
			</p>
			...
			<div className="space-y-3">
				{list.map((d) => {
					...
					return (
						<div
							key={d.id}
							className="rounded-lg border border-[var(--edh-border)] bg-white"
						>
									...
									{r ? (
										<>
											<p className="mb-3 text-xs text-[var(--edh-muted)]">
												Last run {new Date(r.ranAt).toLocaleString()} ·{" "}
												{r.counts.critical} critical · {r.counts.warning}{" "}
												warning · {r.counts.ok} ok
											</p>
