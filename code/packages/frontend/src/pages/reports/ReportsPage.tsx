...
export function ReportsPage() {
	...
	useEffect(() => {
		if (isError)
			toast.error(errMsg(error, "Could not load the report history"));
	}, [isError, error]);
			...
			{isLoading ? (
				...
				<div className="rounded-lg border border-dashed border-[var(--edh-border)] p-10 text-center">
					<p className="text-slate-600">
						Could not load the report history — see the error toast.
					</p>
				</div>
			) : newest.length === 0 ? (
				<div className="rounded-lg border border-dashed border-[var(--edh-border)] p-10 text-center">
					<p className="text-slate-600">
						No reports yet. Run checks from the Dashboard and every run will
						file its report here.
					</p>
					<Link
						to="/"
		...
		<section className="mt-10">
			<header className="mb-3 flex items-center gap-2">
				...
				<h2 className="text-lg font-semibold">
					Report emails (DMARC rua &amp; TLS-RPT)
				</h2>
			</header>
			<p className="mb-3 text-sm text-[var(--edh-muted)]">
				The machine reports receivers send back to your domains — who sent mail
				as you and whether it authenticated, and whether inbound TLS worked.
				Open a domain to see the ingested reports, the problems they reveal, and
				the fixes.
			</p>
			{list.length === 0 ? (
				<div className="rounded-lg border border-dashed border-[var(--edh-border)] p-6 text-center text-sm text-slate-600">
					Add a domain first — then publish rua= on its DMARC / TLS-RPT records
					to receive reports.
				</div>
			) : (
...
function ReportMenu({
	...
	const onDelete = () => {
		if (
			!window.confirm(
				`Delete this report for ${run.domain}? The domain itself is unaffected.`,
			)
		) {
			return;
		}
		if (!run.runId) {
			toast.error(
				"This legacy report has no run id and cannot be deleted individually.",
			);
			return;
		}
