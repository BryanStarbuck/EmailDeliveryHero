...
import { useBlacklistRegistry, useBlacklistRuns, useUpdateBlacklistZone } from "@/api/blacklists"
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <p className="mb-4 mt-1 text-sm text-[var(--edh-muted)]">
        The effective zone catalog — the checked-in <code>blacklists.yaml</code> registry merged
        with your operator overrides. Toggling or re-weighting a zone writes an override to the
        state directory; the checked-in registry is never edited at runtime. Zones on the dead-zone
        registry are hard-blocked and never queried.
      </p>
        ...
        <>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-1.5">
                        ...
                        <span className="ml-2 text-xs text-[var(--edh-muted)]">{z.name}</span>
                      </td>
                      <td className="px-3 py-1.5">
                      ...
                      <td className="px-3 py-1.5">
                        <WeightCell
                          ...
                          onSave={(weight) => update.mutate({ zone: z.zone, kind: z.kind, weight })}
                        />
                      </td>
                      <td className="px-3 py-1.5">
                      ...
                      <td className="px-3 py-1.5">
                        <button
                          ...
                          onClick={() =>
                            update.mutate({ zone: z.zone, kind: z.kind, enabled: !z.enabled })
                          }
                          className={cn(
                            "relative h-5 w-9 rounded-full transition-colors disabled:opacity-50",
                            z.enabled ? "bg-[var(--edh-primary)]" : "bg-gray-300",
                          )}
                          title={
                            z.enabled ? "Enabled — click to disable" : "Disabled — click to enable"
                          }
                        >
                          <span
                            className={cn(
          ...
          <section className="mt-6 rounded-lg border border-[var(--edh-border)] bg-white p-4">
            ...
            <p className="mb-2 text-xs text-[var(--edh-muted)]">
              Dead lists sometimes wildcard on shutdown and "list the world". These zones are
              hard-blocked: they cannot be enabled from any override.
            </p>
