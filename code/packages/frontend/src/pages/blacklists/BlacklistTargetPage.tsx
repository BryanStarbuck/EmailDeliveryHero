...
const SOURCE_LABEL: Record<string, string> = {
  ...
  return_path: "a Return-Path (envelope) domain seen authenticating for this domain",
...
export function BlacklistTargetPage() {
    ...
    return (
      <div className="mx-auto max-w-3xl">
        ...
        <p className="mt-2 rounded-lg border border-dashed border-[var(--edh-border)] p-6 text-sm text-[var(--edh-muted)]">
          No blacklist run yet for this domain — run a check to sweep this target.
        </p>
      </div>
    )
  }
  ...
  const domainTarget = run.targets.domains.find((x) => x.domain.toLowerCase() === t.toLowerCase())
  ...
  if (!ipTarget && !domainTarget && rows.length === 0) {
    return (
      <div className="mx-auto max-w-3xl">
        ...
        <p className="mt-2 rounded-lg border border-dashed border-[var(--edh-border)] p-6 text-sm text-[var(--edh-muted)]">
          <span className="font-mono">{target}</span> was not part of this run's sweep. The run
          tested {run.targets.ips.length} IP(s) and {run.targets.domains.length} domain(s).
        </p>
      </div>
    )
  }
  ...
  const runTargetNow = async () => {
    try {
      const res = await recheck.mutateAsync({ domain: run.domain, targets: [t] })
      setLive(res)
    } catch (err) {
      toast.error(
  ...
  const identity: Array<[string, string]> = ipTarget
    ? [
        ...
        [
          "ASN / org",
          ipTarget.asn ? `AS${ipTarget.asn.number} — ${ipTarget.asn.org ?? "?"}` : "unknown",
        ],
      ]
    : [
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-3">
        ...
        <span
          className={cn(
            ...
            listed.length > 0 ? "bg-red-100 text-red-800" : "bg-emerald-100 text-emerald-800",
          )}
        >
      ...
      <section className="mt-4 rounded-lg border border-[var(--edh-border)] bg-white p-4">
        <h2 className="mb-1 font-semibold">How this target entered the sweep</h2>
        <p className="text-sm text-slate-700">{SOURCE_LABEL[source] ?? source}</p>
        {source === "email_report" && (
          <p className="mt-1 text-xs text-[var(--edh-muted)]">
            Email-derived targets are swept against every enabled IP zone exactly like configured
            sending IPs; a listing here is your delisting problem — the evidence trail is your own
            DMARC reports (see the domain's Reports page).
          </p>
        )}
      </section>
      ...
      <section className="mt-4 rounded-lg border border-[var(--edh-border)] bg-white p-4">
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {[...rows]
              .sort((a, b) => Number(b.listed) - Number(a.listed) || a.zone.localeCompare(b.zone))
              .map((r) => (
                <tr key={r.zone} className="border-b border-[var(--edh-border)] last:border-0">
      ...
      {scope.domainRuns.length > 1 && (
        <section className="mt-4">
          <h2 className="mb-2 text-sm font-semibold text-slate-600">
            This target over the last {Math.min(scope.domainRuns.length, 30)} runs
          </h2>
      ...
      <section className="mt-4 rounded-lg border border-[var(--edh-border)] bg-white p-4">
        ...
        <ul className="list-disc space-y-1 pl-5 text-xs text-[var(--edh-muted)]">
          <li>RFC 5782 — DNS blocklist query mechanics (reversed-IP A queries).</li>
          <li>multirbl.valli.org — cross-check this target against ~892 zones.</li>
          {ipTarget && <li>whois / mmdblookup — netblock owner and ASN context for this IP.</li>}
        </ul>
      </section>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-2">
        ...
        >
          ...
          {recheck.isPending ? "Re-querying this target…" : "Run this check now (this target)"}
        </button>
        <button
          type="button"
