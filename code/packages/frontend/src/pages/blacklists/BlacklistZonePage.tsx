				...
				{live.results.map((r) => {
					...
					return (
						...
						>
							...
							<span className={cn("rounded px-1.5 font-medium", chip.cls)}>
								{chip.label}
							</span>
						</li>
					);
...
export function BlacklistZonePage() {
	...
	const dead = registry?.dead_zones.find(
		(d) => zone === d.zone || zone.endsWith(`.${d.zone}`),
	);
		...
		return (
			<div className="mx-auto max-w-3xl">
				...
				<p className="mt-2 rounded-lg border border-dashed border-[var(--edh-border)] p-6 text-sm text-[var(--edh-muted)]">
					No zone named <span className="font-mono">{zoneId}</span> exists in
					the effective registry. See the Blocklist Zones panel for every zone
					we know.
				</p>
			</div>
		);
	...
	return (
		<div className="mx-auto max-w-3xl">
			...
			{entry?.notes && (
				<p className="mt-1 text-sm text-[var(--edh-muted)]">{entry.notes}</p>
			)}
			...
			{(decoderRows.length > 0 || bitmaskRows.length > 0) && (
				<section className="mt-4 rounded-lg border border-[var(--edh-border)] bg-white p-4">
					...
					<table className="w-full text-xs">
						...
						<tbody>
							{(decoderRows.length > 0 ? decoderRows : bitmaskRows).map(
								([code, meaning]) => (
									<tr
										key={code}
										className={cn(
									...
									>
										<td className="py-1 pr-2 font-mono">
											...
											{observedCodes.has(code) && (
												<span className="ml-1 rounded bg-amber-200 px-1 text-[10px]">
													this run
												</span>
											)}
										</td>
										<td className="py-1 pr-2">
											...
											{meaning.problem_state && (
												<span className="ml-1 text-slate-500">
													({meaning.problem_state}:{" "}
													{problemState(meaning.problem_state)?.name})
												</span>
											)}
										</td>
										<td className="py-1">
			...
			<section className="mt-4 rounded-lg border border-[var(--edh-border)] bg-white p-4">
				...
				<ol className="list-decimal space-y-1 pl-5 text-sm text-slate-700">
					<li>
						Fix the root cause first — a delist request with the cause live gets
						re-listed.
					</li>
					<li>
						Request removal via the operator's own portal
						{entry?.auto_expires && (
							<> — or wait it out (auto-expires {entry.auto_expires})</>
						)}
						.
					</li>
					{entry?.paid_delist_offered && (
						<li className="font-semibold text-red-700">
							Never pay for "express" delisting — these listings auto-expire and
							pay-to-delist is considered abusive (RFC 6471).
						</li>
					)}
					<li>
						Re-run this check after the operator's processing window to confirm
						removal.
					</li>
				</ol>
			</section>
				...
				<section className="mt-4 rounded-lg border border-[var(--edh-border)] bg-white p-4">
					...
					<ul className="space-y-1 text-sm">
						...
						<li className="text-xs text-[var(--edh-muted)]">
							RFC 5782 (DNS blocklist query mechanics) · multirbl.valli.org zone
							directory
						</li>
					</ul>
				</section>
			...
			<div className="mt-5 flex flex-wrap items-center gap-2">
				...
				>
					...
					/>
					{recheck.isPending
						? "Re-querying this zone…"
						: "Run this check now (this zone)"}
				</button>
				<button
					type="button"
