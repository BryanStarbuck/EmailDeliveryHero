...
const STATE_REFERENCES: Partial<Record<ProblemStateId, Array<{ label: string; url: string }>>> = {
  "PS-1": [{ label: "check.spamhaus.org (SBL case lookup)", url: "https://check.spamhaus.org" }],
  "PS-2": [{ label: "check.spamhaus.org (XBL self-delist)", url: "https://check.spamhaus.org" }],
  "PS-3": [
    { label: "Spamhaus PBL removal", url: "https://www.spamhaus.org/pbl/removal/" },
    { label: "SpamRATS removal (fix PTR first)", url: "https://www.spamrats.com/removal.php" },
  ],
  "PS-4": [
    { label: "Spamhaus DBL removal", url: "https://www.spamhaus.org/dbl/removal/" },
    { label: "Google Search Console (hacked-site check)", url: "https://search.google.com/search-console" },
  ],
  "PS-5": [{ label: "SpamCop SCBL lookup (report ages)", url: "https://www.spamcop.net/bl.shtml" }],
  "PS-6": [
    { label: "RFC 6471 — never pay for delisting", url: "https://www.rfc-editor.org/rfc/rfc6471" },
    { label: "UCEPROTECT rblcheck", url: "https://www.uceprotect.net/en/rblcheck.php" },
  ],
  "PS-8": [
    { label: "Google Postmaster Tools", url: "https://postmaster.google.com" },
    { label: "Microsoft SNDS", url: "https://sendersupport.olc.protection.outlook.com/snds/" },
    { label: "sender.office.com delist portal", url: "https://sender.office.com" },
  ],
  "PS-9": [{ label: "Spamhaus DQS (free key)", url: "https://www.spamhaus.com/free-trial/sign-up-for-a-free-data-query-service-account/" }],
  "PS-10": [
    { label: "dnsbl.com dead-DNSBL registry", url: "https://www.dnsbl.com" },
    { label: "multirbl.valli.org zone liveness", url: "https://multirbl.valli.org" },
  ],
  "PS-12": [{ label: "dnswl.org self-registration (free)", url: "https://www.dnswl.org" }],
  "PS-13": [{ label: "RFC 6471 — never pay for delisting", url: "https://www.rfc-editor.org/rfc/rfc6471" }],
}
...
const MANUAL_RERUN_NOTE: Partial<Record<ProblemStateId, string>> = {
  "PS-8": "Provider-side reputation has no DNS zone to re-query — check the portal ↗ instead.",
  "PS-11": "Domain age is a waiting game — nothing to re-run; the gates expire on their own.",
}
...
function evidenceRows(run: BlacklistRunResults, psId: ProblemStateId): BlacklistZoneResult[] {
  if (psId === "PS-9") return run.results.filter((r) => r.refusal_code !== null)
...
export function BlacklistStatePage() {
    ...
    return (
      <div className="mx-auto max-w-3xl">
        ...
        <p className="mt-2 rounded-lg border border-dashed border-[var(--edh-border)] p-6 text-sm text-[var(--edh-muted)]">
          No problem state named <span className="font-mono">{psId}</span> exists. The catalog is{" "}
          {Object.keys(PROBLEM_STATES).join(", ")}.
        </p>
      </div>
    )
  }
  ...
  const canRerun = !manualNote && rerunZones.length > 0 && rerunTargets.length > 0
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <p className="mt-1 text-sm text-[var(--edh-muted)]">Trigger: {ps.trigger}</p>
      ...
      <section className="mt-4 rounded-lg border border-[var(--edh-border)] bg-white p-4">
        ...
        {!run ? (
          <p className="text-sm text-[var(--edh-muted)]">
            No run to compare against — run a blacklist check first; this page still teaches the
            concept above.
          </p>
        ) : detected ? (
          ...
          <p className="text-sm text-emerald-700">{ps.id} was not detected in this run.</p>
        )}
        {evidence.length > 0 && (
                    ...
                    <tbody>
                      <tr className="border-b border-[var(--edh-border)]">
                        ...
                        <td className="py-0.5 font-mono">
                          ...
                          <span className="text-slate-400">({r.tier} tier)</span>
                        </td>
                      </tr>
                      <tr className="border-b border-[var(--edh-border)]">
                      ...
                      {r.listed && (
                        <tr>
                          ...
                          <td className="py-0.5">
                            ...
                            >
                              {r.delist_url} <ExternalLink className="h-3 w-3" />
                            </a>
                          </td>
                        </tr>
                      )}
      ...
      <section className="mt-4 rounded-lg border border-[var(--edh-border)] bg-white p-4">
        <h2 className="mb-1 font-semibold">What it means for your deliverability</h2>
      ...
      <section className="mt-4 rounded-lg border border-[var(--edh-border)] bg-white p-4">
        ...
        <button
          type="button"
          onClick={() =>
            navigator.clipboard?.writeText(ps.diagnose.map((c) => substitute(c)).join("\n"))
          }
          title="Click to copy"
        ...
        <h3 className="mt-3 mb-1 text-sm font-semibold">More health metrics to test</h3>
            ...
            {historyRuns.map((r) => {
              ...
              return (
                <Link
                  ...
                  params={{ id: scope.domainId ?? "", runId: r.runId ?? "", psId: ps.id }}
        ...
        <ul className="space-y-1 text-sm">
          <li>
            <a
              ...
              className="inline-flex items-center gap-1 text-[var(--edh-primary)] underline"
            >
              RFC 5782 — DNS blocklist query mechanics <ExternalLink className="h-3 w-3" />
            </a>
          </li>
          {references.map((ref) => (
            <li key={ref.url}>
        ...
        {manualNote ? (
          ...
          <>
            ...
            >
              ...
              {recheck.isPending ? "Re-checking evidence…" : "Re-check this evidence"}
            </button>
            {canRerun && (
