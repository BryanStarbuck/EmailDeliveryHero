...
function DebugDrawer({
	...
	const command =
		run.tool_runs?.find((t) => t.command.includes(query))?.command ?? null;
		...
		<div className="rounded-md bg-slate-50 p-3">
			...
			{row.listed && (
				<p className="mt-2 text-xs text-slate-700">
					...
					{row.auto_expires && (
						<> — or wait it out (auto-expires {row.auto_expires})</>
					)}
			...
			{ps && (
				...
				>
					Explain &amp; fix this ({ps.id}: {ps.name}){" "}
...
function ListingsMatrix({
	...
	const zones = useMemo(() => {
		...
		return [...seen.values()].sort(
			(a, b) =>
				tierOrder[a.tier] - tierOrder[b.tier] || a.zone.localeCompare(b.zone),
		);
	}, [run.results]);
...
function SubTestTable({
	...
	const newKeys = new Set(
		run.diff.new_listings.map((n) => `${n.zone}|${n.target}`),
	);
		...
		return (
			<section className="mt-6 rounded-lg border border-[var(--edh-border)] bg-white p-4">
				...
				<ol className="list-decimal space-y-1 pl-5 text-sm text-slate-700">
					<li>
						Keep FCrDNS valid on every sending IP and SPF/DKIM/DMARC green.
					</li>
					<li>
					...
					<li>
						Enroll in Google Postmaster Tools and Microsoft SNDS/JMRP (checklist
						below).
					</li>
					<li>
						Keep the scheduled re-check on so a new listing alerts within hours.
					</li>
			...
			<ol className="list-decimal space-y-2 pl-5 text-sm text-slate-700">
				{sorted.map((r) => (
					<li key={`${r.zone}|${r.target}`}>
						...
						{r.auto_expires && (
							<> — or wait it out (auto-expires {r.auto_expires})</>
						)}
						...
						{r.tier === "low" && (
							<span className="text-slate-500">
								{" "}
								Low real-world impact at Gmail/Outlook/Yahoo.
							</span>
						)}
					</li>
				))}
...
const PORTAL_PILL: Record<
	PortalUserState,
	{ label: string; cls: string; next: PortalUserState }
> = {
	unverified: {
		label: "Unverified",
	...
	return (
		<section className="mt-6 rounded-lg border border-[var(--edh-border)] bg-white p-4">
			...
			<p className="mb-3 text-xs text-[var(--edh-muted)]">
				The "invisible blacklists" — clean public zones ≠ clean at
				Gmail/Microsoft. Check each portal and mark its state (click the pill to
				cycle).
			</p>
					...
					return (
						<li key={p.provider} className="flex items-center gap-3 text-sm">
							...
							<button
								type="button"
								onClick={() =>
									setState.mutate({ provider: p.provider, state: pill.next })
								}
								className={cn(
...
function HistoryStrip({
	...
	const max = Math.max(
		1,
		...chronological.map((r) => r.results?.blacklist?.summary.listed ?? 0),
	);
	return (
		<section className="mt-6">
			<h2 className="mb-2 text-sm font-semibold text-slate-600">
				Listed count — last {chronological.length} runs (click a point to open
				that run)
			</h2>
				...
				{chronological.map((r) => {
					...
					return (
						<Link
							...
							className={cn(
								"w-3 rounded-t",
								listed > 0 ? "bg-red-600" : "bg-emerald-500",
							)}
...
function BlacklistRunView({
	...
	const domainRuns = useMemo(
		() =>
			(allRuns ?? []).filter(
				(r) =>
					(domainId ? r.domainId === domainId : r.domain === domainName) &&
					r.results?.blacklist,
			),
		[allRuns, domainId, domainName],
	);
	const currentIndex = runId
		? domainRuns.findIndex((r) => r.runId === runId)
		: 0;
	...
	const isHistorical = Boolean(
		runId && newestRun?.runId && runId !== newestRun.runId,
	);

	const run: BlacklistRunResults | undefined = runId
		...
		return (
			<div className="mx-auto max-w-4xl">
				...
				<p className="rounded-lg border border-dashed border-[var(--edh-border)] p-8 text-center text-[var(--edh-muted)]">
					Run not found — it may have been pruned or carries no Blacklists
					section.
					{domainId && (
						<Link
							to="/domains/$id/blacklists"
	...
	const allHighInconclusive =
		highRows.length > 0 && highRows.every((r) => r.inconclusive);
		...
		<div className="mx-auto max-w-[1100px]">
			...
			<div className="flex flex-wrap items-center justify-between gap-2">
				...
				{domainId && (
					<div className="flex items-center gap-2">
						...
						>
							<RefreshCw
								className={cn("h-3 w-3", runAudit.isPending && "animate-spin")}
							/>
							Re-run
						</button>
					</div>
			...
			{isHistorical && newestRun && (
				<div className="mt-2 rounded-md bg-slate-200 px-3 py-1.5 text-xs text-slate-700">
					historical run — newest is{" "}
					{new Date(newestRun.ranAt).toLocaleString()}{" "}
					{domainId && (
						<Link
							to="/domains/$id/blacklists"
							params={{ id: domainId }}
				...
				<div className="flex items-center justify-between gap-3">
					<div>
						...
						<p className="mt-1 text-sm">
							...
							· {run.summary.clean} clean · {run.summary.inconclusive} unknown
							across {run.summary.zones_enabled} zones
						...
						{worstListing && (
							<p className="mt-1 text-sm">
								...
								{worstListing.sub_list ? ` — ${worstListing.sub_list}` : ""}.
								Fix the cause first.
							</p>
						)}
						{(run.diff.new_listings.length > 0 ||
							run.diff.cleared.length > 0) && (
							<p className="mt-1 text-sm font-semibold">
								{run.diff.new_listings.length > 0 && (
									<>▲ {run.diff.new_listings.length} new </>
								)}
								{run.diff.cleared.length > 0 && (
									<>▼ {run.diff.cleared.length} resolved</>
								)}
							</p>
						)}
					</div>
			...
			{(run.resolver.refusals_detected || allHighInconclusive) && (
				<div className="mt-3 rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">
					Some blocklists refused our DNS queries — results are partially
					inconclusive. This says nothing bad about your domain.{" "}
					<Link
						to="/blacklists/$domain/state/$psId"
