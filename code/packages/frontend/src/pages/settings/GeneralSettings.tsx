	...
	return (
		<Panel title="Account & access">
			{isSignedIn ? (
				...
				<>
					...
					<p className="mt-3 text-sm text-slate-600">
						You are using the app as the <code>default</code> user — settings
						persist under that account until you sign in. Signing in is optional
						and never required.
					</p>
				</>
			)}
			...
			<Row
				...
				value="OpenAuthFederated session · cookie prefix oaf_edh"
			/>
			<p className="mt-3 text-sm text-[var(--edh-muted)]">
				Identity is asserted by OpenAuthFederated (Google Workspace SSO) — there
				is no password to manage here, and the sign-in allowlist is enforced on
				the OpenAuthFederated side. Admins edit it from the{" "}
				<Link
					to="/settings/$section"
	...
	return (
		<Panel title="Appearance">
			...
			<p className="mt-2 text-xs text-[var(--edh-muted)]">
				Per-user and cosmetic only — appearance never affects audits or other
				users.
			</p>
		</Panel>
	);
		...
		try {
			...
			if (result.desktop.attempted && "Notification" in window) {
				...
				if (permission === "granted") {
					new Notification("EmailDeliveryHero", {
	...
	return (
		<Panel title="Notifications">
			<p className="mb-3 text-sm text-[var(--edh-muted)]">
				How you learn about <em>new</em> problems found by scheduled runs. These
				are your own preferences; the shared webhook and SMTP channels are
				admin-only.
			</p>
			...
			<FieldRow label="Minimum severity">
				<select
					...
					disabled={!prefs}
					onChange={(e) =>
						onChange({ minSeverity: e.target.value as NotificationSeverity })
					}
			...
			<FieldRow label="Delivery">
				<select
					...
					disabled={!prefs}
					onChange={(e) =>
						onChange({ mode: e.target.value as NotificationMode })
					}
...
function MonitoredDomainsPanel() {
	...
	return (
		<Panel title="Monitored domains">
			<p className="mb-3 text-sm text-[var(--edh-muted)]">
				A compact view of the domains under audit — the full editor (sending
				IPs, DKIM selectors, per-domain schedule) lives in{" "}
			...
			{(domains ?? []).length === 0 ? (
				<p className="py-2 text-sm text-[var(--edh-muted)]">
						...
						>
							<span className="min-w-0">
								...
								<span className="ml-2 text-xs text-[var(--edh-muted)]">
									...
									{d.sendingIps.length > 0 &&
										` · IPs: ${d.sendingIps.join(", ")}`}
								</span>
							</span>
								...
								<button
									...
									onClick={() =>
										runAudit.mutate(d.id, {
											onSuccess: () =>
												toast.success(`Audit started for ${d.name}`),
			...
			<p className="mt-3 text-xs text-[var(--edh-muted)]">
				...
				<code>
					{settings
						? settings.config.checks.dkim.defaultSelectors.join(", ")
						: "…"}
				</code>{" "}
				(admin-only to edit).
			</p>
...
function ChecksReadOnlyPanel() {
	...
	return (
		<Panel title="Checks configuration">
			<p className="mb-3 text-sm text-[var(--edh-muted)]">
				What every scheduled run does and how every domain is scored. Everything
				here is <strong>admin-only to edit</strong> — shown read-only so you can
				see how your domains are judged.
				{isAdmin && (
					<>
						{" "}
						Edit it in the{" "}
			...
			<Row
				...
				value={
					checks
						? checks.enabled.map((c) => CATEGORY_LABEL[c] ?? c).join(", ") ||
							"none"
						: "…"
				}
			/>
			<Row
				label="DNSBL zones"
				value={checks ? checks.dnsbl.zones.join(", ") : "…"}
			/>
			<Row
			...
			<Row
				...
				value={checks ? String(checks.spf.maxLookups) : "…"}
			/>
		</Panel>
	);
}
	...
	return (
		<Panel title="Storage & data">
			...
			<Row
				...
				value={
					data
						? `${data.config.storage.retentionDays} days (admin-only to edit)`
						: "…"
				}
			/>
			...
			<p className="mt-3 text-xs text-[var(--edh-muted)]">
				Import and the destructive Reset actions are admin-only and live in the
				Admin section. There is no database — everything is YAML/JSON files
				under the state dir.
			</p>
		</Panel>
	);
...
function ToolsPanel() {
	...
	const toolValue = (
		tool: { found: boolean; version: string | null } | undefined,
	) => {
		if (!tool) return "…";
	...
	return (
		<Panel title="Tools & environment">
			...
			<Row
				label="swaks (SMTP tests / notification email)"
				value={toolValue(status?.swaks)}
			/>
			<Row
				label="DNS resolvers"
				value={
			...
			<Row
				label="Query timeout"
				value={data ? `${data.config.tools.timeoutMs} ms` : "…"}
			/>
			<Row
			...
			{/* U2 (pm/install_brew.mdx §9): the actionable companion to this read-only status —
          the Install page lists every missing Brew/OS tool and npm package for one-click install. */}
			<Link
				to="/install"
				search={{ manager: "all" }}
				className="mt-3 ml-2 inline-flex items-center gap-1 rounded-md bg-[var(--edh-primary)] px-3 py-1.5 text-sm font-medium text-white"
			>
