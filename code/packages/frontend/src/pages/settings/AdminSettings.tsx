...
function toDraft(view: SettingsView): Draft {
	...
	return {
		...
		fingerprints: (view.config.dns_health?.fingerprints ?? []).map((f) => ({
			...f,
		})),
		...
		repParkingNs: (
			view.config.domain_reputation?.parking_nameservers ?? []
		).join(", "),
		repHighAbuseTlds: (
			view.config.domain_reputation?.high_abuse_tlds ?? []
		).join(", "),
		repWatchlist: (
			view.config.domain_reputation?.registrar_reputation ?? []
		).map((r) => ({
			...r,
		})),
...
function fromDraft(d: Draft): UpdateAdminSettingsInput {
	return {
		...
		tools: {
			...
			paths: {
				spamassassin: d.spamassassinPath.trim(),
				spamc: d.spamcPath.trim(),
			},
		},
...
export function AdminSettings() {
		...
		return (
			<section className="rounded-lg border border-[var(--edh-border)] bg-white p-5">
				...
				<p className="text-sm text-slate-600">
					This section requires the <code>role:admin</code> claim. You can view
					every setting read-only from the{" "}
	...
	const onReset = async (scope: "audit_history" | "app") => {
		...
		try {
			...
			toast.success(
				scope === "app" ? "App reset to defaults" : "Audit history cleared",
			);
			setDraft(null);
		} catch {
		...
		<div className="space-y-4">
				...
				{draft.zones.map((zone, i) => (
					...
					<div key={i} className="flex items-center gap-2 py-1">
						...
						<button
							type="button"
							onClick={() =>
								set({ zones: draft.zones.filter((_, j) => j !== i) })
							}
            ...
            target, and the network-tests opt-in. Binary path override is env (EDH_TOOL_SPAMASSASSIN). */}
				...
				<label className="mt-2 flex items-center gap-2 py-1 text-sm">
					...
					/>
					Enable network content tests (URIBL/Razor/Pyzor/DCC — off keeps
            ...
            Admin-only — the backend refuses this write without role:admin. */}
				...
				<p className="mb-1 text-xs text-[var(--edh-muted)]">
					The future VMC certificate round matches each certificate&apos;s
					issuer DN against this allow-list; an unrecognized issuer makes{" "}
					<code>content.bimi_vmc_valid</code> critical.
				</p>
				{draft.mvaAllowList.map((mva, i) => {
					const setMva = (patch: Partial<BimiMvaEntry>) =>
						set({
							mvaAllowList: draft.mvaAllowList.map((m, j) =>
								j === i ? { ...m, ...patch } : m,
							),
						});
					...
					return (
						...
						<div key={i} className="flex flex-wrap items-center gap-2 py-1">
							...
							<button
								...
								onClick={() =>
									set({
										mvaAllowList: draft.mvaAllowList.filter((_, j) => j !== i),
									})
								}
            ...
            targets against. Admin-only — keep it fresh, new SaaS endpoints appear constantly. */}
				...
				<p className="mb-1 text-xs text-[var(--edh-muted)]">
					A CNAME whose final target matches one of these suffixes and no longer
					resolves is flagged as a critical <code>infra.dangling_cname</code>{" "}
					takeover risk.
				</p>
				{draft.fingerprints.map((fp, i) => {
					const setFp = (patch: Partial<TakeoverFingerprint>) =>
						set({
							fingerprints: draft.fingerprints.map((f, j) =>
								j === i ? { ...f, ...patch } : f,
							),
						});
					return (
						...
						<div key={i} className="flex flex-wrap items-center gap-2 py-1">
							...
							<button
								...
								onClick={() =>
									set({
										fingerprints: draft.fingerprints.filter((_, j) => j !== i),
									})
								}
            ...
            reference lists and the RDAP request budget + cache TTL. Admin-only. */}
				...
				<p className="mb-1 text-xs text-[var(--edh-muted)]">
					Registration data is stale-tolerant and RDAP endpoints rate-limit hard
					— snapshots are cached for the TTL below and each run spends at most
					the request budget. The reference lists feed{" "}
					<code>infra.parking_nameservers</code>, <code>infra.tld_risk</code>,
					and <code>infra.registrar_reputation</code>.
				</p>
				<div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
					<NumberInput
				...
				{draft.repWatchlist.map((entry, i) => {
					const setEntry = (patch: Partial<RegistrarReputationEntry>) =>
						set({
							repWatchlist: draft.repWatchlist.map((r, j) =>
								j === i ? { ...r, ...patch } : r,
							),
						});
					return (
						...
						<div key={i} className="flex flex-wrap items-center gap-2 py-1">
							<select
								...
								onChange={(e) =>
									setEntry({
										match_type: e.target
											.value as RegistrarReputationEntry["match_type"],
									})
								}
							...
							<button
								...
								onClick={() =>
									set({
										repWatchlist: draft.repWatchlist.filter((_, j) => j !== i),
									})
								}
			...
			<Panel title="Scheduling">
				<p className="text-sm text-[var(--edh-muted)]">
					Scheduling is an <strong>all-users</strong> setting (pm/settings.mdx
					§3.1) — configure it on the{" "}
					<Link
						to="/settings/$section"
			...
			<Panel title="Access">
				...
				<p className="mt-2 text-xs text-[var(--edh-muted)]">
					Enforcement happens on the OpenAuthFederated side
					(pm/authentication.mdx §2) — this value is displayed to users and
					applied to the OAF allowlist, never as a code change here.
				</p>
			</Panel>
	...
	return (
		<section className="rounded-lg border border-[var(--edh-border)] bg-white p-5">
			<h2 className="mb-3 font-semibold">
				{title}{" "}
				<span className="ml-1 align-middle text-xs text-[var(--edh-muted)]">
					🔒 admin-only
				</span>
			</h2>
			{children}
...
function LabeledInput({
	...
	return (
		<label className={`mt-2 block text-sm ${wide ? "w-full" : ""}`}>
			<span className="mb-1 block text-xs text-[var(--edh-muted)]">
				{label}
			</span>
			<input
				value={value}
