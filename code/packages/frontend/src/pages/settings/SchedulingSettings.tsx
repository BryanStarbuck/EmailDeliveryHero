...
function toDraft(cfg: ScheduleConfig): Draft {
  return {
    ...
    days: cfg.cadence === "weekly" && cfg.weekdays.length > 0 ? [...cfg.weekdays] : [...WEEKDAYS],
  }
}
...
function sameDraft(a: Draft, b: Draft): boolean {
  const times = (d: Draft) => [...new Set(d.times.map((t) => t.value))].sort().join(",")
  const days = (d: Draft) => WEEKDAYS.filter((w) => d.days.includes(w)).join(",")
...
export function SchedulingSettings() {
  ...
  const timesValid = draft.times.length > 0 && draft.times.every((t) => HHMM_RE.test(t.value))
  const canSave = dirty && timesValid && draft.days.length > 0 && !save.isPending
  ...
  const onRunNow = async () => {
    try {
      ...
      if (outcome.started) {
        toast.success(`Scheduled check ran (${outcome.domains ?? 0} domain(s))`)
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-lg border border-[var(--edh-border)] bg-white p-5">
        ...
        {!on && (
          <p className="mt-3 flex items-center gap-2 rounded-md bg-slate-50 px-3 py-2 text-sm text-slate-600">
            ...
            Scheduling is off — checks only run when you press "Run checks". Turning it on starts
            the schedule below.
          </p>
        )}
      </section>
      ...
      <section className="rounded-lg border border-[var(--edh-border)] bg-white p-5">
        ...
        {draft.times.map((row, i) => (
          <div key={row.id} className="flex items-center gap-2 py-1">
            ...
            <button
              type="button"
              onClick={() =>
                setDraft({ ...draft, times: draft.times.filter((t) => t.id !== row.id) })
              }
              disabled={draft.times.length <= 1}
              aria-label={`Remove time ${row.value}`}
              title={draft.times.length <= 1 ? "At least one time is required" : "Remove this time"}
        ...
        <button
          type="button"
          onClick={() => setDraft({ ...draft, times: [...draft.times, timeRow("18:00")] })}
        ...
        <p className="mt-2 text-xs text-[var(--edh-muted)]">
          Times use this machine's local timezone. Add more rows to run more than once per day.
        </p>
            ...
            return (
              <button
                ...
                onClick={() =>
                  setDraft({
                    ...draft,
                    days: active ? draft.days.filter((w) => w !== d) : [...draft.days, d],
                  })
                }
      ...
      <section className="rounded-lg border border-[var(--edh-border)] bg-white p-5">
            ...
            <>
              ...
              {status.data.lastTrigger && (
                <span className="text-[var(--edh-muted)]">
                  {" "}
                  · {TRIGGER_LABEL[status.data.lastTrigger] ?? status.data.lastTrigger}
                </span>
              )}{" "}
              ·{" "}
        ...
        <StatusRow label="Covers">
          {status.data?.domainsTotal != null ? (
            <>
              {status.data.domainsCovered ?? 0} of {status.data.domainsTotal} monitored domains ·{" "}
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-[var(--edh-border)] pt-3">
          ...
          <Link to="/scheduled-checks" className="text-sm text-[var(--edh-primary)] underline">
