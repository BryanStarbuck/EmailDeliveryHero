...
export const SPF_PROBLEM_STATES: SpfProblemState[] = [
  ...
  },
  {
    ...
    dataFields: ["record.void_count (limit 2)", "record.include_tree — nodes with is_void = true"],
    commands: ["dig +short TXT <include-target>", "checkdmarc <domain> -f json"],
  ...
  },
  {
    ...
    commands: ["dig +short TXT <domain>", "checkdmarc <domain>   # names the offending term"],
    tools: [
      "checkdmarc",
  ...
  },
  {
    ...
    dataFields: ["record.all_qualifier", "record.mechanisms (the CIDR terms)", "record.pass_set"],
    commands: ["dig +short TXT <domain>"],
    tools: ["dig — the last term tells the story", "any online checker flags +all instantly"],
    metrics: [
  ...
  },
  {
    ...
    severity: "warning",
    findingIds: ["spf.ptr", "spf.all_terminal", "spf.dup_mechanisms", "spf.redirect"],
    concept: [
    ...
    commands: ["dig +short TXT <domain>"],
    tools: ["none required — this is pure record hygiene; the mechanism table shows it all"],
    metrics: ["Lookup head-room reclaimed after cleanup (the budget gauge drops)."],
    pathForward: [
  ...
  },
  {
    ...
    commands: ["dig +short TXT <looping-target>   # follow each hop until the repeat appears"],
  ...
  },
  {
    ...
    dataFields: ["record.byte_length", "the spf.length / spf.dns_type findings"],
    commands: [
