...
export function matchProblemStates<T extends ProblemState>(states: T[], findings: Finding[]): T[] {
  const failing = new Set(
    findings.filter((f) => f.severity === "warning" || f.severity === "critical").map((f) => f.id),
  )
  return states.filter((ps) => ps.findingIds.some((id) => failing.has(id)))
}

export function problemStateById<T extends ProblemState>(states: T[], id: string): T | undefined {
