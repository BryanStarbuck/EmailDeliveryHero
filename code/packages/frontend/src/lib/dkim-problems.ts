...
export const DKIM_PROBLEM_STATES: DkimProblemState[] = [
	...
	},
	{
		...
		dataFields: [
			"selectors[].is_revoked = true",
			"selectors[].source",
			"selectors[].raw_record",
		],
		commands: [
			"dig +short TXT <selector>._domainkey.<domain>   # expect v=DKIM1; p= (empty)",
		],
		tools: [
			"dig",
	...
	},
	{
		...
		dataFields: [
			"selectors[].has_test_flag",
			"selectors[].has_strict_flag",
			"selectors[].flags.t",
		],
		commands: [
			"dig +short TXT <selector>._domainkey.<domain> | grep -o 't=[^;]*'",
		],
	...
	},
	{
		...
		tools: [
			"dig / doggo (follows chains)",
			"the ESP dashboard's current target value",
		],
		metrics: [
	...
	},
	{
		...
		metrics: [
			"Key age per selector — an info fires at ~150 days, the warning at 180.",
		],
		pathForward: [
	...
	},
	{
		...
		tools: [
			"the duplicate-key list on this page",
			"openssl for manual confirmation",
		],
		metrics: [
			"New cross-domain collisions are flagged by the regression diff.",
		],
		pathForward: [
	...
	},
	{
		...
		commands: [
			"dig +short TXT <selector>._domainkey.<domain> | grep -o 'k=[^;]*'",
		],
		tools: [
			"dig",
			"mailauth (npm) — shows both signatures' verdicts per message (future)",
		],
