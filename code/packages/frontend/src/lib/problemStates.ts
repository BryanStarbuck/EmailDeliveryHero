...
export const PROBLEM_STATES: Record<ProblemStateId, ProblemStateInfo> = {
	"PS-0": {
		...
		diagnose: [
			"dig +short 2.0.0.127.zen.spamhaus.org   # liveness probe — expect 127.0.0.2",
		],
	...
	"PS-3": {
		...
		severity: "warning",
		trigger:
			"PBL or SpamRATS-Dyna say this IP should not send direct-to-MX mail.",
		concept:
	...
	"PS-4": {
		...
		tools: [
			"dig",
			"dnstwist",
			"SURBL/URIBL lookup pages",
			"Google Search Console (security tab)",
		],
		furtherHealth: [
	...
	"PS-6": {
		...
		severity: "info",
		trigger:
			"UCEPROTECT L2/L3, SEM-netbl, or another range listing you didn't cause.",
		concept:
	...
	"PS-7": {
		...
		severity: "warning",
		trigger:
			"Missing or generic PTR; SpamRATS NoPtr/Dyna and PBL-class listings follow from it.",
		concept:
		...
		tools: [
			"dig",
			"swaks",
			"your hosting provider's reverse-DNS console/ticket",
		],
		furtherHealth: [
			"The full reverse-DNS check.",
			"HELO-name alignment (SMTP security check).",
		],
		progress: [
	...
	"PS-8": {
		...
		severity: "warning",
		trigger:
			"Bounces from Gmail/Microsoft/Yahoo while every public zone is green.",
		concept:
	...
	"PS-9": {
		...
		furtherHealth: [
			"Per-zone probe latency (rate-limiting shows as timeouts/127.255.255.255).",
		],
		progress: [
	...
	"PS-10": {
		...
		severity: "info",
		trigger:
			"An RFC 5782 probe failed; the zone lists 127.0.0.1 or answers nothing.",
		concept:
		...
		tools: [
			"dig",
			"multirbl.valli.org zone pages",
			"dnsbl.com dead-list registry",
		],
	...
	"PS-13": {
		...
		severity: "info",
		trigger:
			"The only open listings are on operators that sell 'express' removal.",
		concept:
