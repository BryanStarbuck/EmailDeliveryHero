...
export const DNS_PROBLEM_STATES: DnsProblemState[] = [
	...
	},
	{
		...
		metrics: [
			"Future probe adds port-25 reachability and SMTP banner per MX host.",
		],
		pathForward: [
	...
	},
	{
		...
		metrics: [
			"Future SMTP probe compares the MTA's HELO/EHLO name to the PTR hostname.",
		],
		pathForward: [
	...
	},
	{
		...
		metrics: [
			"Per-NS response-time trend; SOA-serial drift across secondaries.",
		],
		pathForward: [
	...
	},
	{
		...
		commands: [
			...
			"dig MX <domain>   # read the TTL column",
		],
		tools: [
			"dig",
			"intoDNS (flags out-of-range SOA values with RFC citations)",
		],
		metrics: [
	...
	},
	{
		...
		metrics: [
			"Re-probed every run — wildcards reappear with provider template resets.",
		],
		pathForward: [
	...
	},
	{
		...
		tools: [
			"dnsviz, delv (bind), drill (ldns), kdig +dnssec (brew install dnsviz bind ldns knot)",
		],
		metrics: [
	...
	},
	{
		...
		metrics: [
			"Blacklist category cross-check — open relays surface there first.",
		],
		pathForward: [
...
export function matchDnsProblemStates(findings: Finding[]): DnsProblemState[] {
	const failing = findings
		...
		.map((f) =>
			f.id.startsWith("infra.") ? f.id.slice("infra.".length) : f.id,
		);
	return DNS_PROBLEM_STATES.filter((ps) =>
		ps.findingPrefixes.some((prefix) =>
			failing.some((id) => id.startsWith(prefix)),
		),
	);
}
