...
export const DMARC_CHECK_UNITS: DmarcCheckUnit[] = [
  {
    ...
    oneLiner: "Is a v=DMARC1 TXT published at _dmarc.<domain> (or a covering parent)?",
  ...
  },
  {
    ...
    findingIds: ["dmarc.policy", "dmarc.no_policy", "dmarc.p_none", "dmarc.policy_ok"],
    tags: ["p"],
    whatItIs: [
  ...
  },
  {
    ...
    meaningPass: "No testing flag is weakening the published policy — what you see is what receivers enforce.",
    meaningFail:
  ...
  },
  {
    ...
    meaningPass: "No sampling tag — the policy applies to 100% of failing mail, deterministically.",
    meaningFail:
  ...
  },
  {
    ...
    oneLiner: "Third-party report destinations must authorize you or reports vanish.",
 ...
 */
export function dmarcUnitForFindingId(findingId: string): DmarcCheckUnit | undefined {
  return DMARC_CHECK_UNITS.find(
    (u) =>
      u.findingIds.includes(findingId) ||
      (u.prefixIds ?? []).some((p) => findingId === p.replace(/\.$/, "") || findingId.startsWith(p)),
  )
}
...
export function dmarcUnitResult(
  ...
  if (worst === null) {
    for (const f of findings) if (unitOwns(unit, f.id)) consider(RESULT_OF_SEVERITY[f.severity])
  }
  return worst
}
