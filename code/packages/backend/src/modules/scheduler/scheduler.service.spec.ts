...
import { defaultScheduleConfig, writeScheduleConfig } from "./schedule-config.store"
...
describe("SchedulerService.runNow", () => {
  ...
  const build = (config: Partial<ScheduleConfig>, domains: MonitoredDomain[]) => {
    ...
    process.env.EDH_STATE_DIR = mkdtempSync(join(tmpdir(), "edh-scheduler-spec-"))
  ...
  it("honors an explicit domain-id scope ANDed with each domain's own switch", async () => {
    build({ enabled: true, domains: ["b"] }, [domain("a", true), domain("b", false)])
    const outcome = await service.runNow("os")
  ...
  it("reports coverage and the configured slots in status()", () => {
    build({ enabled: true, times: ["06:00", "18:00"] }, [domain("a", true), domain("b", false)])
