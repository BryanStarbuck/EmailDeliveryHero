...
import type { OsArtifactPreview, ScheduleConfig, Weekday } from "./schedule.types"
...
export function renderSystemdUnits(cfg: ScheduleConfig): { service: string; timer: string } {
...
export function renderCrontab(cfg: ScheduleConfig): string {
  ...
  if (cfg.cadence === "interval") return `0 */${cfg.everyHours} * * *  ${cmd}\n`
  const dow =
    cfg.cadence === "weekly"
      ? activeWeekdays(cfg)
          .map((d) => CRON_DOW[d])
...
export async function installArtifact(cfg: ScheduleConfig): Promise<void> {
  ...
  if (kind === "systemd" || kind === "cron") {
    ...
    await execFileAsync("systemctl", ["--user", "enable", "--now", `${SYSTEMD_UNIT}.timer`])
  ...
  logInfo(`Installed schtasks task(s): ${schtasksTaskNames(cfg).join(", ")}`, "Scheduler")
}
