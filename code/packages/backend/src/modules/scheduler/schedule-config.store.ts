...
export function detectOsKind(platform: NodeJS.Platform = process.platform): OsSchedulerKind {
...
export function normalizeSchedule(raw: unknown): ScheduleConfig {
  ...
  const everyHours =
    Number.isFinite(everyHoursNum) && everyHoursNum >= 1 && everyHoursNum <= 24 * 7
      ? Math.round(everyHoursNum)
      : def.everyHours

  const times = Array.isArray(r.times)
    ? [...new Set(r.times.filter((t): t is string => typeof t === "string" && HHMM_RE.test(t)))]
    : []
  ...
  const timezone =
    typeof r.timezone === "string" && isValidTimezone(r.timezone) ? r.timezone : def.timezone
  ...
  if (Array.isArray(r.domains)) {
    const ids = r.domains.filter((d): d is string => typeof d === "string" && d.trim() !== "")
