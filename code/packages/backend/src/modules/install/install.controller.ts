...
import type { InstallJobAccepted, InstallJobStatus, PreflightResult } from "./install.types"
...
export class InstallController {
  ...
  @Get("catalog")
  @ApiOperation({ summary: "The tool catalog (brew/npm/all) with live installed status merged in." })
  catalog(@Query("manager") manager?: string): ToolStatus[] {
    return this.install.catalog(asManager(manager))
  }

  @Get("preflight")
  @ApiOperation({ summary: "Scope-aware missing / optional / installed split for a pending run." })
  preflight(
  ...
  @ApiOperation({ summary: "SSE stream of per-row install output (missing→installing→done/failed)." })
