...
describe("runFileTimestamp — the locked filename grammar (pm/storage.mdx §7.2)", () => {
	it("renders YYYY_MM_DD_hh_mm{AM|PM} in the given timezone, zero-padded", () => {
		...
		expect(runFileTimestamp("2026-07-01T16:14:02.000Z", TZ)).toBe(
			"2026_07_01_09_14AM",
		);
	});
	...
	it("renders midnight as 12_xxAM and noon as 12_xxPM", () => {
		...
		expect(runFileTimestamp("2026-07-01T07:05:00.000Z", TZ)).toBe(
			"2026_07_01_12_05AM",
		);
		expect(runFileTimestamp("2026-07-01T19:00:00.000Z", TZ)).toBe(
			"2026_07_01_12_00PM",
		);
	});
...
describe("saveRun / listRuns / getRun — one YAML file per run", () => {
	...
	it("sorts newest startedAt first and filters by domainId", () => {
		saveRun(
			makeRun({ runId: "old", startedAt: "2026-06-30T10:00:00.000Z" }),
			TZ,
		);
		saveRun(
			makeRun({ runId: "new", startedAt: "2026-07-01T16:14:02.000Z" }),
			TZ,
		);
		saveRun(
			makeRun({
...
describe("deleteRun / deleteDomainRuns — whole-file deletes only (§7.4)", () => {
	...
	it("deleting a domain removes its whole runs/<domain>/ directory", () => {
		saveRun(makeRun(), TZ);
		saveRun(
			makeRun({ runId: "other", domainId: "dom-2", domain: "act3ai.com" }),
			TZ,
		);
...
describe("pruneRuns — retention window + per-domain cap (§7.4)", () => {
	...
	it("deletes runs older than the retention window", () => {
		saveRun(
			makeRun({ runId: "ancient", startedAt: "2020-01-01T00:00:00.000Z" }),
			TZ,
		);
		saveRun(
			makeRun({ runId: "fresh", startedAt: new Date().toISOString() }),
			TZ,
		);
		pruneRuns(90, 50);
