...
import {
	type DkimDiscoveryOutcome,
	discoverDkimSelectors,
} from "./checks/dkim/dkim.check";
import {
	type TaggedToolRun,
	type ToolRunRecord,
	withCheckTag,
...
function ownsContentFinding(f: Finding, checkerId: string): boolean {
	...
	switch (checkerId) {
		...
		case "content.reputation":
			return REPUTATION_FINDING_STEMS.some((stem) =>
				f.id.startsWith(`content.${stem}`),
			);
...
export class AuditService {
	...
	runForDomain(
		...
		const run = (async () => {
			...
			logInfo(
				`Audited ${domain.name}: score ${result.score} (${result.status})`,
				"AuditService",
			);
			return result;
	...
	async runForAll(trigger: AuditTrigger = "manual"): Promise<AuditResult[]> {
		const scheduled =
			trigger === "scheduled-inprocess" || trigger === "scheduled-os";
		...
		if (scheduled) {
			logInfo(
				`Scheduled check run started (${domains.length} domain(s))`,
				"AuditScheduler",
			);
		} else {
		...
		logInfo(
			`Audited ${results.length}/${domains.length} domain(s)`,
			"AuditService",
		);
		return results;
	}
	...
	async runBlacklistsForDomain(
		...
		const run = (async () => {
			...
			try {
				...
				logError(
					`Blacklists-scoped check failed for ${domain.name}`,
					err,
					"AuditService",
				);
				findings.push({
					...
					remediation:
						"Re-run the check. If it keeps failing, this may be a transient DNS issue.",
				});
			}
	...
	async runDkimForDomain(
		...
		const run = (async () => {
			...
			try {
				...
				logError(
					`DKIM-scoped check failed for ${domain.name}`,
					err,
					"AuditService",
				);
				findings.push({
			...
			merged.sort(
				(a, b) =>
					(registryIndex.get(a.checkId) ?? 99) -
					(registryIndex.get(b.checkId) ?? 99),
			);
			const { score, status, counts } = summarize(
				merged,
				readAppConfig().checks.weights,
			);
			const finishedAt = new Date().toISOString();
			const result: AuditResult = {
				runId,
			...
			await this.persistResult(result);
			logInfo(
				`DKIM-scoped check finished for ${domain.name}: ${result.status}`,
				"AuditService",
			);
			return result;
		})().finally(() => this.inFlight.delete(domainId));
	...
	async runSpfForDomain(
		...
		const run = (async () => {
			...
			try {
				...
				logError(
					`SPF-scoped check failed for ${domain.name}`,
					err,
					"AuditService",
				);
				findings.push({
			...
			await this.persistResult(result);
			logInfo(
				`SPF-scoped check finished for ${domain.name}: ${result.status}`,
				"AuditService",
			);
			return result;
		})().finally(() => this.inFlight.delete(domainId));
	...
	async runDnsForDomain(
		...
		const familyKey =
			checkKey === undefined ? undefined : dnsFamilyKeyForCheckKey(checkKey);
		...
		if (this.inFlight.has(domainId)) {
			throw new RunInFlightError(
				`A run is already in flight for domain ${domainId}`,
			);
		}
		const scope = (
			familyKey ? `dns.${familyKey}` : "dns"
		) as AuditResult["scope"];
		const scoped = familyKey
			? CHECKERS.filter((c) => c.id === `infra.${familyKey}`)
			: CHECKERS.filter((c) => c.id.startsWith("infra."));
		if (scoped.length === 0)
			throw new Error(`No checker registered for infra.${familyKey}`);
		const run = (async () => {
			const domain = this.domains.get(domainId);
			logInfo(
			...
			const runOne = async (checker: Checker): Promise<void> => {
				...
				try {
					const outcome = await withCheckTag(checker.id, () =>
						Promise.resolve(checker.run(ctx)),
					);
					if (Array.isArray(outcome)) {
						findings.push(...outcome);
					} else {
					...
					logError(
						`Checker ${checker.id} failed for ${domain.name}`,
						err,
						"AuditService",
					);
					findings.push({
			...
			const toolRunLog: TaggedToolRun[] = [];
			await withToolRunLog(toolRunLog, () =>
				withDnsMemo(() => runCheckerGraph(scoped, runOne)),
			);
	...
	async rerunChecker(
		...
		logInfo(
			`Single-check re-run ${checkerId} started for ${domain.name}`,
			"AuditService",
		);
		...
		try {
			...
			logError(
				`Single-check re-run ${checkerId} failed for ${domain.name}`,
				err,
				"AuditService",
			);
			findings.push({
				...
				detail: `The ${checker.label} check could not complete: ${err instanceof Error ? err.message : String(err)}`,
				remediation:
					"Run this check again. If it keeps failing, check the tool installation.",
			});
		}
	...
	async spotCheck(
		...
		const latest = this.latest(domainId);
		logInfo(
			`Spot check ${checkerId} started for ${domain.name}`,
			"AuditService",
		);
		...
		try {
			...
			logError(
				`Spot check ${checkerId} failed for ${domain.name}`,
				err,
				"AuditService",
			);
			findings.push({
				...
				remediation:
					"Run it again. If it keeps failing, this may be a transient DNS issue.",
			});
		} finally {
			clearTimeout(deadlineTimer);
		}
	...
	async dkimDiscovery(domainId: string): Promise<DkimDiscoveryOutcome> {
		const domain = this.domains.get(domainId);
		logInfo(
			`DKIM selector discovery started for ${domain.name}`,
			"AuditService",
		);
	...
	private async auditDomain(
		...
		const runOne = async (checker: Checker): Promise<void> => {
			const didNotComplete = (): void => {
				findings.push({
					...
					remediation:
						"Re-run the audit. If it keeps timing out, check network connectivity.",
				});
			};
			...
			try {
				...
				findings.push({
					...
					detail: `The ${checker.label} check could not complete: ${err instanceof Error ? err.message : String(err)}`,
					remediation:
						"Re-run the audit. If it keeps failing, this may be a transient DNS issue.",
				});
			}
		};
		...
		const toolRunLog: TaggedToolRun[] = [];
		await withToolRunLog(toolRunLog, () =>
			withDnsMemo(() => runCheckerGraph(CHECKERS, runOne)),
		);
		clearTimeout(deadlineTimer);
		...
		const contentToolRuns: ToolRunRecord[] = toolRunLog
			...
			.map(({ check_id: _checkId, ...rest }) => rest);
		if (contentToolRuns.length > 0)
			results["content.tool_runs"] = contentToolRuns;
