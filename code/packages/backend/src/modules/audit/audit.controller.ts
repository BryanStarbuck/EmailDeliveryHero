...
import { AuditService, RunInFlightError, type SpotCheckResult } from "./audit.service"
...
export class AuditController {
  ...
  private async runDnsScoped(domainId: string, checkKey?: string): Promise<AuditResult> {
    try {
      return await this.audit.runDnsForDomain(domainId, checkKey, "manual")
    } catch (err) {
      if (err instanceof RunInFlightError) throw new ConflictException(err.message)
  ...
  async runContentCheck(
    ...
    try {
      ...
      if (err instanceof RunInFlightError) {
        ...
        throw new ConflictException({ reason: "audit_in_flight", message: err.message })
      }
      const message = err instanceof Error ? err.message : String(err)
