...
function dnsInfraFamilyOf(findingId: string): string | null {
	const bare = findingId.startsWith("infra.")
		? findingId.slice("infra.".length)
		: findingId;
...
const DNS_INFRA_DERIVED_KEYS = new Set([
	"status",
	"tests",
	"families",
	"problem_states",
...
const SPAM_CONTENT_DERIVED_KEYS = new Set([
	"status",
	"tests",
	"families",
	"problem_states",
...
function contentFamilyOf(finding: Finding): string | null {
	...
	if (checkId === "content.scoring" || CONTENT_SCORING_FINDING_IDS.has(id))
		return "content_score";
	if (checkId === "content.bimi" || id.startsWith("content.bimi"))
		return "bimi";
	if (checkId === "content.report_emails" || id.startsWith("content.report_"))
	...
	if (
		CONTENT_REPUTATION_STEMS.some((stem) => id.startsWith(`content.${stem}`))
	) {
 ...
 */
export function runFileTimestamp(
	startedAtIso: string,
	timezone: string,
): string {
	...
	const dayPeriod = get("dayPeriod").toUpperCase().startsWith("P")
		? "PM"
		: "AM";
...
function encodeRunFile(result: AuditResult): RunFile {
	...
	const infraFindings = (result.findings ?? []).filter((f) =>
		f.checkId.startsWith("infra."),
	);
	...
	const contentFindings = (result.findings ?? []).filter((f) =>
	...
	if (contentFindings.length > 0) {
		...
		for (const f of contentFindings) {
			...
			if (family) {
				...
				row.tests += 1;
				if (SEVERITY_RANK[f.severity] > SEVERITY_RANK[row.status])
					row.status = f.severity;
				families[family] = row;
			}
		}
	...
	for (const [id, payload] of Object.entries(results)) {
		if (id.startsWith("infra.")) dnsInfra[id.slice("infra.".length)] = payload;
			...
			spamContent[id.slice("content.".length)] = payload;
	}
	if (infraFindings.length > 0)
		dnsInfra.tests = infraFindings.map(encodeDnsInfraTest);
	if (contentFindings.length > 0)
		spamContent.tests = contentFindings.map(encodeContentTest);
...
function decodeRunFile(doc: unknown): AuditResult | null {
	...
	if (typeof run.run_id !== "string" || typeof run.domain !== "string")
		return null;
	...
	if (
		isPlainObject(doc.dmarc) &&
		("arc" in doc.dmarc || "reports" in doc.dmarc)
	) {
	...
	return {
		...
		...(run.new_problem_count !== undefined
			? { newProblemCount: run.new_problem_count }
			: {}),
...
export function listRuns(domainId?: string): AuditResult[] {
	...
	const filtered = domainId
		? runs.filter((r) => r.domainId === domainId)
		: runs;
...
function findRunFile(runId: string): string | null {
	for (const file of listRunFiles()) {
		...
		if (
			isPlainObject(doc) &&
			isPlainObject(doc.run) &&
			doc.run.run_id === runId
		)
			return file;
	}
	return null;
	...
	for (const entry of readdirSync(root, { withFileTypes: true })) {
		...
		for (const file of listRunFiles(entry.name)) {
			...
			const startedAt =
				isPlainObject(doc) &&
				isPlainObject(doc.run) &&
				typeof doc.run.started_at === "string"
					? doc.run.started_at
					: "";
...
export function migrateLegacyRunsJson(timezone: string): void {
	...
	try {
		...
		logError(
			"Failed to remove legacy runs.json after migration",
			err,
			"RunsStore",
		);
		return;
	...
	logInfo(
		`Migrated ${migrated} run(s) from runs.json to the runs/ YAML tree`,
		"RunsStore",
	);
}
