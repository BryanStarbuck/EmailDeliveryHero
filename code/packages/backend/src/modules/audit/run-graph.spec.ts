...
describe("runCheckerGraph", () => {
  ...
  function harness() {
    const checkers: Checker[] = IDS.map((id) => ({ id, label: id, run: async () => [] }))
  ...
  it("starts every foundation check together and gates dependents on their own prerequisites", async () => {
    ...
    expect(started).toEqual(
      expect.arrayContaining(["spf", "dkim", "infra.mx_routing", "infra.dnssec"]),
    )
    // Stage 2: nothing dependency-gated has started yet.
    for (const id of ["dmarc", "content.bimi", "infra.dane_tlsa", "infra.mta_sts"]) {
  ...
  it("declares the spec §2 dependency edges", () => {
    ...
    expect(CHECK_DEPENDENCIES["infra.dane_tlsa"]).toEqual(["infra.mx_routing", "infra.dnssec"])
  })
})
