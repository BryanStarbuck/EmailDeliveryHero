...
const analyze = (raw: string) => analyzeSpfTerms("example.com", raw, parseSpfRecord(raw))
const byId = (findings: ReturnType<typeof analyze>, id: string) => findings.find((f) => f.id === id)

describe("parseSpfRecord", () => {
  it("tokenizes qualifiers, mechanisms, values, and modifiers", () => {
    const p = parseSpfRecord("v=spf1 ip4:203.0.113.0/24 -include:_spf.example.net ~all")
  ...
  it("parses redirect= and exp= modifiers and ignores unknown modifiers", () => {
    const p = parseSpfRecord("v=spf1 redirect=_spf.example.com unknown-mod=whatever")
  ...
  it("flags unknown mechanisms, bad CIDR ranges, and values on all", () => {
    expect(parseSpfRecord("v=spf1 inlcude:foo.com -all").syntaxErrors.join()).toContain(
      "unknown mechanism",
    )
    expect(parseSpfRecord("v=spf1 ip4:1.2.3.4/33 -all").syntaxErrors.join()).toContain(
      "prefix out of range",
    )
    expect(parseSpfRecord("v=spf1 ip6:2001:db8::/129 -all").syntaxErrors.join()).toContain(
      "prefix out of range",
    )
    expect(parseSpfRecord("v=spf1 ip4:not-an-ip -all").syntaxErrors.join()).toContain(
      "malformed ip4",
    )
    expect(parseSpfRecord("v=spf1 all:everything").syntaxErrors.join()).toContain(
      '"all" takes no value',
    )
    expect(parseSpfRecord("v=spf1 include: -all").syntaxErrors.join()).toContain(
      "requires a domain",
    )
  })

  it("accepts a and mx with optional domain and dual-cidr suffixes", () => {
    const p = parseSpfRecord("v=spf1 a mx:mail.example.com a:web.example.com/24 -all")
    expect(p.syntaxErrors).toEqual([])
...
describe("analyzeSpfTerms", () => {
  it("passes a healthy record with ok findings", () => {
    ...
    expect(f.every((x) => x.severity !== "critical" && x.severity !== "warning")).toBe(true)
  })

  it("flags +all as critical and ?all / missing all as warnings", () => {
    expect(byId(analyze("v=spf1 ip4:1.2.3.4 +all"), "spf.all")?.severity).toBe("critical")
    expect(byId(analyze("v=spf1 ip4:1.2.3.4 ?all"), "spf.all")?.severity).toBe("warning")
    expect(byId(analyze("v=spf1 ip4:1.2.3.4"), "spf.all")?.severity).toBe("warning")
  })

  it("does not warn about a missing all when redirect= is present", () => {
    expect(byId(analyze("v=spf1 redirect=_spf.example.org"), "spf.all")).toBeUndefined()
  })
  ...
  it("flags /0 ranges as critical and very broad ranges as warnings", () => {
    expect(byId(analyze("v=spf1 ip4:0.0.0.0/0 -all"), "spf.cidr_scope")?.severity).toBe("critical")
    expect(byId(analyze("v=spf1 ip4:10.0.0.0/7 -all"), "spf.cidr_scope")?.severity).toBe("warning")
    expect(byId(analyze("v=spf1 ip4:10.0.0.0/24 -all"), "spf.cidr_scope")).toBeUndefined()
  })

  it("reports syntax and macro errors as criticals", () => {
    expect(byId(analyze("v=spf1 ip4:1.2.3.4/33 -all"), "spf.syntax")?.severity).toBe("critical")
    expect(byId(analyze("v=spf1 exists:%{x}.example.com -all"), "spf.macro")?.severity).toBe(
      "critical",
    )
  })
