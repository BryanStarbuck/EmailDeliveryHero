	...
	return new Promise<HttpResult>((resolve) => {
		...
		try {
			const req = httpsRequest(
				url,
				{ method: "GET", headers, timeout: HTTP_TIMEOUT_MS },
				(res) => {
					const chunks: Buffer[] = [];
...
async function rdapGet(
	...
	while (true) {
		if (budget.n <= 0)
			return { status: 0, body: "", headers: {}, error: "budget_exhausted" };
		budget.n--;
...
async function bootstrapServices(): Promise<[string[], string[]][] | null> {
	...
	const res = await httpGet("https://data.iana.org/rdap/dns.json", {
		Accept: "application/json",
	});
	if (res.status === 200) {
		try {
			const services = (JSON.parse(res.body).services ?? []) as [
				string[],
				string[],
			][];
...
export function relatedRdapUrl(rdap: Rdap, baseUrl: string): string | null {
	for (const link of Array.isArray(rdap?.links) ? rdap.links : []) {
		...
		const isRdap =
			type.includes("rdap") || (type === "" && link.href.includes("/domain/"));
		if (!isRdap) continue;
		try {
...
export function mergeRdapDocs(registry: Rdap, registrar: Rdap): Rdap {
	...
	return {
		...
		nameservers:
			arr(registry?.nameservers).length > 0
...
export function parseRdap(rdap: Rdap): RegData {
	...
	return {
		...
		...(piiExposed
			? { registrantContact: registrantName ?? registrantEmail }
			: {}),
...
function registrarChanged(
	...
	if (previous.name !== null && reg.registrar !== undefined) {
		return (
			previous.name.trim().toLowerCase() !== reg.registrar.trim().toLowerCase()
		);
	}
	return false;
}
...
export function deriveRegistrationFindings(
	...
	// infra.update_lock
	if (
		statusSet.has("clientUpdateProhibited") ||
		statusSet.has("serverUpdateProhibited")
	) {
		findings.push({
	...
	const holds = reg.statuses.filter(
		(s) => s === "serverHold" || s === "clientHold",
	);
	if (holds.length > 0) {
		findings.push({
	...
	const lifecycle = reg.statuses.filter(
		(s) => s === "pendingDelete" || s === "redemptionPeriod",
	);
	...
	if (registrarChanged(reg, opts.previousRegistrar)) {
		const before =
			opts.previousRegistrar?.name ?? `IANA ${opts.previousRegistrar?.ianaId}`;
...
export function buildSnapshot(
	...
	return {
		...
		age_days: Number.isFinite(created)
			? Math.floor((now - created) / DAY)
			: null,
		days_to_expiry: Number.isFinite(expiry)
			? Math.floor((expiry - now) / DAY)
			: null,
		source: "rdap",
...
function previousSnapshot(previous: unknown): DomainRegistrationResults | null {
	...
	if (typeof snap.checked_at !== "string" || snap.raw_record == null)
		return null;
...
export const domainReputationCheck: Checker = {
	...
	async run(ctx): Promise<CheckOutcome> {
		...
		const budget = {
			n: cfg.rdap_request_budget || DEFAULT_RDAP_REQUEST_BUDGET,
		};
		...
		const hasNonAscii = [...decoded].some(
			(ch) => (ch.codePointAt(0) ?? 0) > 127,
		);
		...
		const parkingList = (cfg.parking_nameservers ?? []).map((p) =>
			p.toLowerCase(),
		);
		...
		if (ns.error) {
			...
			const parkNs = ns.records.filter((h) =>
				parkingList.some(
					(p) => h.toLowerCase() === p || h.toLowerCase().endsWith(`.${p}`),
				),
			);
		...
		const prev = previousSnapshot(
			(ctx.previousResults as Record<string, unknown> | undefined)?.[
				"infra.domain_reputation"
			],
		);
		...
		deriveOpts.previousRegistrar = prev
			...
			: null;
		const ttlMs =
			(cfg.cache_ttl_hours || DEFAULT_CACHE_TTL_HOURS) * 60 * 60 * 1000;
		const prevAge = prev
			? Date.now() - Date.parse(prev.checked_at)
