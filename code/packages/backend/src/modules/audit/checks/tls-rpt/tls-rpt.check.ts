...
function isTlsRptCandidate(rec: string): boolean {
	...
	return (
		lower.includes("tlsrptv1") ||
		/^\s*v\s*=\s*tlsrpt/.test(lower) ||
		/(^|;)\s*rua\s*=/.test(lower)
	);
}
...
function splitSizeLimit(uri: string): {
	base: string;
	sizeLimit: string | null;
} {
...
export const tlsRptCheck: Checker = {
	...
	async run(ctx): Promise<CheckOutcome> {
		...
		const future = [
			...futureFindings(domain),
			...deriveTlsRptFindings(ctx.domainId ?? "", domain),
		];
		...
		const unknown = tags
			.filter((t) => !KNOWN_TAGS.has(t.key.toLowerCase()))
			.map((t) => t.key);
