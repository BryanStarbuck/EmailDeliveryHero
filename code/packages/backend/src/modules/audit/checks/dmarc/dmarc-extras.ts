...
function tokenize(raw: string): HygieneTag[] {
  return raw
    ...
    .map((token) => {
      ...
      if (eq === -1) return { name: token.toLowerCase(), value: "", raw: token }
      return {
...
export function analyzeRecordHygiene(raw: string): RecordHygiene {
  ...
  const duplicateTags = [...counts.entries()].filter(([, n]) => n > 1).map(([name]) => name)
  ...
  if (psd !== null) {
    if (!PSD_VALUES.has(psd)) {
      findings.push({
        ...
        severity: "info",
        detail: "psd must be y, n, or u (RFC 9989); anything else is ignored by receivers.",
        remediation: "Remove the psd tag — ordinary organizational domains never need it.",
        evidence: raw,
      })
    } else if (psd === "y") {
      findings.push({
...
function isNullMx(records: { exchange: string; priority: number }[]): boolean {
  return records.length === 1 && (records[0].exchange === "" || records[0].exchange === ".")
}
...
export async function probeReportDestinations(
  ...
  const domains = [
    ...new Set([...ruaUris, ...rufUris].map(mailDomainOf).filter((d): d is string => d !== null)),
  ]
...
export function deriveAlignmentBasis(
  ...
  const spf = upstream?.spf as { record_found?: boolean; eval_result?: string } | undefined
  const dkim = upstream?.dkim as { working_selectors?: number } | undefined
  const spf_ready =
    spf && typeof spf.record_found === "boolean"
      ? spf.record_found && (spf.eval_result === undefined || spf.eval_result === "valid")
      : null
  const dkim_ready =
    dkim && typeof dkim.working_selectors === "number" ? dkim.working_selectors > 0 : null

  if (spf_ready === null || dkim_ready === null) {
    return {
      sim: { spf_ready, dkim_ready, basis: "unknown", forwarding_fragile: false },
      finding: null,
    }
  }
  ...
  if (dkim_ready && spf_ready) {
    ...
    finding = {
      ...
      remediation: "Repair the SPF record (see the SPF category) so both mechanisms carry DMARC.",
    }
  } else if (spf_ready) {
    basis = "spf-only"
    finding = {
  ...
  return {
    sim: { spf_ready, dkim_ready, basis, forwarding_fragile: basis === "spf-only" },
    finding,
  }
}
