...
const POLICY_RANK: Record<Policy, number> = {
	none: 0,
	quarantine: 1,
	reject: 2,
};
...
function splitReportUris(
	value: string,
): { uri: string; size: string | null; raw: string }[] {
	return value
		.split(",")
		.map((u) => u.trim())
		.filter(Boolean)
		.map((raw) => {
			...
			if (bang > 0)
				return { uri: raw.slice(0, bang), size: raw.slice(bang + 1), raw };
 ...
 */
export function analyzeDmarcRecord(
	domain: string,
	raw: string,
	foundAt: string,
): Analysis {
	...
	if (
		tags.length === 0 ||
		tags[0].name !== "v" ||
		tags[0].value.toUpperCase() !== "DMARC1"
	) {
		findings.push({
			id: "dmarc.syntax",
			checkId: "dmarc",
			...
			remediation:
				"Rewrite the record so it begins exactly with v=DMARC1; followed by p=.",
			evidence: raw,
		});
	...
	if (!policyRaw) {
		findings.push({
			...
			remediation:
				'Add a policy, e.g. "; p=quarantine" (or p=none to start monitoring).',
			evidence: raw,
		});
	} else if (!policy) {
		findings.push({
			...
			severity: "critical",
			detail:
				"p= must be none, quarantine, or reject; anything else invalidates the record.",
	...
	const sp = (POLICIES as readonly string[]).includes(spRaw ?? "")
		? (spRaw as Policy)
		: null;
	...
	if (map.pct !== undefined) {
		...
		if (pct === null || pct > 100) {
			findings.push({
				...
				severity: "warning",
				detail:
					"pct must be an integer 0–100 — and is deprecated in DMARCbis anyway.",
			...
			findings.push({
				...
				severity: "info",
				detail:
					"pct=100 is the default; the tag is deprecated in DMARCbis and can be removed.",
	...
	if (map.fo !== undefined) {
		...
		if (tokens.some((t) => !["0", "1", "d", "s"].includes(t))) {
			findings.push({
				...
				remediation:
					"Set fo=1 to get a report when either SPF or DKIM fails alignment.",
				evidence: raw,
			});
		} else if (
			rufEntries.length > 0 &&
	...
	const unknown = tags
		.filter((t) => !KNOWN_TAGS.has(t.name))
		.map((t) => t.name);
	if (unknown.length > 0) {
		findings.push({
	...
	if (obsolete.length > 0) {
		findings.push({
			...
			severity: "info",
			detail:
				"These tags were removed in DMARCbis (RFC 9989); receivers ignore them.",
	...
	if (map.t?.toLowerCase() === "y") {
		findings.push({
			...
			remediation:
				"Remove t=y (or set t=n) once you are ready to enforce the published policy.",
			evidence: raw,
		});
	}
...
export function deriveProblemStates(
	findings: Finding[],
	opts: {
		misplacedHit?: boolean;
		enforcing?: boolean;
		dkimUnhealthy?: boolean;
	} = {},
): string[] {
	...
	for (const f of findings) {
		...
		if (!seen || SEVERITY_RANK[f.severity] > SEVERITY_RANK[seen])
			failing.set(f.id, f.severity);
	}
	...
	if (
		has("dmarc.syntax", "dmarc.no_policy") ||
		hasAt("dmarc.policy", "critical")
	)
		out.push("PS-04");
	...
	const unhealthy = [...failing.values()].some(
...
export function extractDoggoAnswers(
	parsed: unknown,
...
export function doggoTxtValues(answers: Record<string, unknown>[]): string[] {
	...
	for (const a of answers) {
		...
		if (typeof v === "string")
			vals.push(v.replace(/"\s+"/g, "").replace(/^"|"$/g, "").trim());
	}
	return vals;
}
...
async function probeMisplacedRecord(domain: string): Promise<string[]> {
	const dmarcish = (r: string): boolean =>
	...
	if (cname.records.length > 0) {
		hits.push(
			`_dmarc.${domain} is a CNAME to ${cname.records[0]} (target answers no DMARC record)`,
		);
	}
	return hits;
}
...
export const dmarcCheck: Checker = {
	...
	async run(ctx): Promise<CheckOutcome> {
		...
		if (lookup.error) {
			...
			let dmarc = lookup.records.filter((r) =>
			...
			if (dmarc.length === 0) {
				...
				for (const ext of externalReports) {
					...
					const authorized = probe.records.some((r) =>
						r.toLowerCase().startsWith("v=dmarc1"),
					);
					...
					if (probe.error && !authorized) {
						...
						testExtras.set(failure, {
							dns_value_expected: `${authName} TXT "v=DMARC1"`,
						});
					}
				}
				if (externalReports.length > 0) {
					const allAuthorized = record.external_report_auth.every(
						(a) => a.authorized,
					);
		...
		if (!doggoPath) {
			...
			if (inv.entry.exit_code === 0 && inv.entry.parsed) {
				...
				const ownRecord =
					record.found_at === name && record.record_count === 1
						? record.raw_record
						: null;
		...
		if (!checkdmarcPath) {
			...
			toolRuns.push(inv.entry);
			if (
				inv.entry.exit_code === 0 &&
				inv.entry.parsed &&
				typeof inv.entry.parsed === "object"
			) {
		...
		if (missingTools.length > 0) {
			const formulas = [
				...new Set(missingTools.map((m) => TOOL_INSTALL[m] ?? m)),
			];
			findings.push({
		...
		const dkim = ctx.upstream?.dkim as
			| { working_selectors?: number }
			| undefined;
		const dkimUnhealthy =
			typeof dkim?.working_selectors === "number" &&
