...
function parseContentType(value: string | null): { type: string; params: Record<string, string> } {
...
function walkEntity(
  ...
  if (ct.type === "message/rfc822") {
    const inner = splitHeadBody(decodeTransfer(body, getHeader(head, "content-transfer-encoding")))
    walkEntity(inner.head, inner.body, out, depth + 1)
    return
  }
  const decoded = decodeTransfer(body, getHeader(head, "content-transfer-encoding"))
...
export function parseLink(raw: string, shorteners: ReadonlySet<string>): LinkUrl | null {
 ...
 */
export function extractLinks(parts: SampleParts, shorteners: ReadonlySet<string>): LinkUrl[] {
