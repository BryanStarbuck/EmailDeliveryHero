...
export function buildQueryName(target: string, zone: BlocklistZone): string | null {
  if (zone.kind === "ip") {
 ...
 */
export function classifyAnswer(zone: BlocklistZone, addresses: string[]): AnswerClassification {
  ...
  if (zone.bitmask && addresses.includes("127.0.0.1")) return refused("127.0.0.1")
...
export const SEVERITY_RANK: Record<Severity, number> = { ok: 0, info: 1, warning: 2, critical: 3 }

export function worstSeverity(severities: Array<Severity | null | undefined>): Severity {
...
export function decodeMailspikeRep(addresses: string[]): {
  ...
  for (const a of addresses) {
    ...
    if (octet !== null && LABELS[octet]) return { code: a, label: LABELS[octet] }
  }
...
export function classifyZoneHealth(args: {
  ...
  return { zone, status, positive_probe: positive, negative_probe: negative, probe_ms: probeMs }
}
...
export function detectProblemStates(args: {
  ...
  if (results.some((r) => r.refusal_code) || zoneHealth.some((z) => z.status === "blocked")) {
    states.add("PS-9")
  }
  ...
  if (listings.some((r) => r.zone.includes("zrd.") || r.zone.startsWith("fresh."))) {
    states.add("PS-11")
  }

  // PS-13: the ONLY open listings are on pay-to-delist operators (never-pay advisory).
  const realListings = listings.filter((r) => r.severity === "warning" || r.severity === "critical")
  if (realListings.length > 0 && realListings.every((r) => r.paid_delist_offered)) {
    states.add("PS-13")
  }
...
export function diffRuns(prev: BlacklistRunResults | null, curr: ZoneResult[]): BlacklistDiff {
  if (!prev) return { new_listings: [], cleared: [], escalated: [], first_run: true }
  ...
  const diff: BlacklistDiff = { new_listings: [], cleared: [], escalated: [], first_run: false }
  ...
  for (const before of prev.results) {
    ...
    if (now && !now.inconclusive && !now.listed) {
      diff.cleared.push({ zone: before.zone, target: before.target, sub_list: before.sub_list })
    }
  }
  return diff
}
