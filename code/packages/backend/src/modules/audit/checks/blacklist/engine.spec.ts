...
describe("buildQueryName", () => {
	it("reverses IPv4 octets for IP zones (spec §8 acceptance 1)", () => {
		expect(
			buildQueryName("203.0.113.24", zone({ zone: "zen.spamhaus.org" })),
		).toBe("24.113.0.203.zen.spamhaus.org");
	});
	...
	it("queries domains directly with no reversal", () => {
		expect(
			buildQueryName(
				"Example.COM",
				zone({ zone: "dbl.spamhaus.org", kind: "domain" }),
			),
		).toBe("example.com.dbl.spamhaus.org");
	});
...
describe("decoders", () => {
	it("decodes DNSWL category and trust", () => {
		expect(decodeDnswl(["127.0.4.2"])).toEqual({
			listed: true,
	...
	it("decodes Sender Score and flags <70 as warning", () => {
		expect(decodeSenderScore(["127.0.4.84"])).toEqual({
			score: 84,
			severity: "info",
		});
...
describe("detectProblemStates (spec §16)", () => {
	...
	it("PS-9 on refusals and PS-10 on dead zones", () => {
		const states = detectProblemStates({
			results: [
				result({ refusal_code: "127.255.255.254", inconclusive: true }),
			],
			zoneHealth: [
				{
					zone: "dead.example",
...
describe("diffRuns (spec §8 acceptance 9)", () => {
	...
	it("flags clean→listed as new and listed→clean as cleared", () => {
		...
		expect(diff.new_listings).toEqual([
			{ zone: "a", target: "203.0.113.24", sub_list: "XBL" },
		]);
	...
	it("flags warning→critical as escalated", () => {
		const prev = runWith([
			result({ zone: "a", listed: true, severity: "warning" }),
		]);
		const diff = diffRuns(prev, [
			result({ zone: "a", listed: true, severity: "critical" }),
		]);
...
describe("zone catalog", () => {
	...
	it("extracts single-host ip4 literals from SPF", () => {
		expect(
			spfLiteralIps(
				"v=spf1 ip4:203.0.113.24 ip4:198.51.100.0/24 include:_spf.google.com ~all",
			),
