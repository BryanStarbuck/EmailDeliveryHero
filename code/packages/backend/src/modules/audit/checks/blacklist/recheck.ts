...
export type RecheckChange = "now_listed" | "now_clean" | "unchanged" | "inconclusive" | "untracked"
...
function classifyChange(row: ZoneResult, storedListed: boolean | null): RecheckChange {
...
export async function liveRecheck(
  ...
  if (opts.zones && opts.zones.length > 0) {
    const wanted = new Set(opts.zones.map((z) => z.trim().toLowerCase()).filter(Boolean))
  ...
  if (opts.targets && opts.targets.length > 0) {
    const cleaned = [...new Set(opts.targets.map((t) => t.trim()).filter(Boolean))]
    ipTargets = cleaned.filter((t) => reverseIpv4(t) !== null)
    domainTargets = cleaned.filter((t) => reverseIpv4(t) === null).map((t) => t.toLowerCase())
  } else if (stored) {
  ...
  const results: RecheckRow[] = rows.map((row) => {
    ...
    return { ...row, stored_listed: storedListed, change: classifyChange(row, storedListed) }
  })
