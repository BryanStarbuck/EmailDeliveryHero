...
export function extractReportXml(eml: string): string[] {
  ...
  const push = (buf: Buffer) => {
    ...
    if (text.includes("<feedback") && text.includes("<record>")) xmls.push(text)
  }
...
export function parseRuaXml(xml: string): ParsedReport {
  const policy = /<policy_published>([\s\S]*?)<\/policy_published>/i.exec(xml)?.[1] ?? ""
  ...
  for (const record of xml.match(/<record>[\s\S]*?<\/record>/gi) ?? []) {
    ...
    if (!ip) continue
    const evaluated = /<policy_evaluated>([\s\S]*?)<\/policy_evaluated>/i.exec(record)?.[1] ?? ""
    const aligned =
 ...
 */
export function collectEmailReportIps(domain: string): { ips: EmailReportIp[]; truncated: number } {
  const dir = emailsDir()
  ...
  for (const file of files) {
    ...
    for (const xml of extractReportXml(eml)) {
      ...
      const seen = new Date((report.endEpoch ?? Date.now() / 1000) * 1000).toISOString()
