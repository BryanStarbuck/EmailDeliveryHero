...
export class DkimToolBench {
  ...
  constructor(
    ...
    if (tools) for (const [name, path] of Object.entries(tools)) this.paths.set(name, path)
  }
      ...
      if (res.timedOut) {
        ...
        try {
          ...
          entry.parsed = spec.stdinSource
            ? { stdin_source: spec.stdinSource, ...(parsed as Record<string, unknown>) }
            : parsed
        } catch (err) {
   ...
   */
  async kdigCrossCheck(name: string, localAnswerCount: number): Promise<boolean | null> {
    const entry = await this.capture({
      tool: "kdig",
      ...
      condense: (stdout) => {
        ...
        return { answerRRs: count, agrees_with_local: count === localAnswerCount }
      },
    })
  ...
  async dnsxSweep(candidateNames: string[]): Promise<void> {
    await this.capture({
      tool: "dnsx",
      args: ["-txt", "-cname", "-resp", "-json", "-silent", "-retry", "1", "-t", "6"],
      outputFormat: "json",
