...
const MX_SELECTOR_HINTS: { pattern: RegExp; selectors: string[] }[] = [
	...
	{
		pattern: /protection\.outlook\.com$/i,
	...
	if (keyType === "ed25519") {
		...
		try {
			...
			if (key.asymmetricKeyType === "ed25519")
				return { valid: true, keyBits: null, keySha256 };
		} catch {
 ...
 */
async function publicResolverTxtSet(
	server: string,
	name: string,
): Promise<string[] | null> {
...
async function assessResolverAgreement(
	...
	const split = reachable.filter(
		(v) => !recordSetsEqual(localRecords, v.records),
	);
	if (split.length === 0) {
	...
	analysis.findings.push({
		...
		evidence: `local: ${fmt(localRecords)}${views
			.map(
				(v) =>
					` · ${v.server}: ${v.records === null ? "(unreachable)" : fmt(v.records)}`,
			)
			.join("")}`,
	});
}
...
export function analyzeAdspLegacy(
	...
	if (adspRecords.length > 0) {
		...
		adsp.practice =
			practice === "unknown" || practice === "all" || practice === "discardable"
				? practice
				: null;
...
export const dkimCheck: Checker = {
	...
	async run(ctx): Promise<CheckOutcome> {
		...
		const analyses = await mapLimit(
			probeList,
			DISCOVERY_CONCURRENCY,
			({ selector, source }) => probeSelector(domain, selector, source, bench),
		);
		...
		const previous =
			(ctx.previousResults?.dkim as DkimResults | undefined)?.selectors ?? [];
		const nowIso = new Date().toISOString();
		const rotationWarnDays = rotationWindowDays();
		const rotationInfoDays = Math.max(
			1,
			rotationWarnDays - ROTATION_INFO_LEAD_DAYS,
		);
		for (const sel of results.selectors) {
			...
			const ageDays = Math.floor(
				(Date.now() - Date.parse(sel.first_seen_at)) / 86_400_000,
			);
		...
		const working = results.selectors.filter(
			(s) =>
				s.present &&
				s.parses &&
				(s.key_type === "ed25519" || (s.key_bits ?? 0) >= 1024),
		);
		...
		const adspOutcome = analyzeAdspLegacy(
			domain,
			adspLookup.records,
			legacyLookup.records,
		);
		...
		results.tests = buildDkimTests(
			findings,
			new Set(results.selectors.map((s) => s.selector)),
		);
 ...
 */
export function buildDkimTests(
	findings: Finding[],
 ...
 */
export function deriveDkimProblemStates(
	findings: Finding[],
	results: DkimResults,
): string[] {
	...
	const has = (...prefixes: string[]): boolean =>
		prefixes.some((p) =>
			failing.some((id) => id === p || id.startsWith(`${p}.`)),
		);
	const out: string[] = [];
	...
	if (has("dkim.single_record", "dkim.record_size", "dkim.underscore_label"))
		out.push("PS-08");
 ...
 */
export async function discoverDkimSelectors(
	rawDomain: string,
): Promise<DkimDiscoveryOutcome> {
	...
	if (wildcardProbe.records.length > 0)
		return { wildcard_shadow: true, probed: 0, hits: [] };
...
async function captureSelectorEvidence(
	...
	if (agrees === false) {
		const present = analysis.findings.find(
			(f) => f.id === `dkim.present.${selector}`,
		);
		if (present) {
...
export function invalidSelectorReason(selector: string): string | null {
	...
	for (const label of selector.split(".")) {
		if (!label)
			return "the selector has an empty DNS label (leading/trailing or doubled dot)";
		if (label.length > 63)
			return `the DNS label "${label.slice(0, 24)}…" exceeds 63 octets`;
...
async function probeSelector(
	...
	if (lookup.records.length > 0) {
		...
		const analysis = analyzeSelectorRecord(
			domain,
			selector,
			source,
			lookup.records,
...
async function rawResolveTxt(
	...
	try {
		...
		return {
			...
			chunkLengths: chunks.flatMap((parts) =>
