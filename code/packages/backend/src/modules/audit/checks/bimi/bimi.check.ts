...
function dmarcFromSibling(ctx: CheckContext): DmarcState | null {
  ...
  return {
    ...
    policy: typeof dmarc.policy === "string" ? dmarc.policy.toLowerCase() : null,
 ...
 */
export function parseBimiSelectorHeader(sampleMessage: string | undefined): string | null {
  ...
  const selector = /(?:^|;)\s*s\s*=\s*([a-z0-9-]{1,63})\s*(?:;|$)/i.exec(header)?.[1]
...
export const bimiCheck: Checker = {
  ...
  async run(ctx): Promise<CheckOutcome> {
    ...
    const extraSelectors = [
      ...new Set(
        [...(ctx.bimi?.selectors ?? []), ...(headerSelector ? [headerSelector] : [])]
    ...
    const probeExtras = async (): Promise<void> => {
      for (const selector of extraSelectors) {
        ...
        rows.push(row)
        findings.push(await probeSelector(ctx.domain, selector, selector === headerSelector, row))
      }
    }
    ...
    const bimiRecords = records.filter((r) => r.toLowerCase().includes("v=bimi1"))
