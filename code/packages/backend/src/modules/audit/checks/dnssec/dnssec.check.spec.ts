...
const mockReadAppConfig = readAppConfig as jest.MockedFunction<
	typeof readAppConfig
>;
...
const DS_MISMATCH =
	"9999 13 2 32996839A6D808AFE3EB4A795A0E6A7A39A76FC52FF228B22B76F6D63826F2BA";
...
async function run(): Promise<{
	findings: Finding[];
	results: DnssecResults | undefined;
}> {
	const outcome = (await dnssecCheck.run(ctx())) as CheckOutcome;
	return {
		findings: outcome.findings,
		results: outcome.results as DnssecResults | undefined,
	};
}
...
function rrsig(covered: string, exp: string, keyTag = 2371) {
	return rr(
		"RRSIG",
		`${covered} 13 2 3600 ${exp} 20190101000000 ${keyTag} ${DOMAIN}. c2ln`,
	);
}
...
describe("dnssecCheck (pm/checks/dnssec.mdx)", () => {
	it("registers as 'infra.dnssec' with a label and namespaces every finding infra.dnssec_*", async () => {
		...
		dnsAnswers({
			DNSKEY: { records: [KSK_13, ZSK_13] },
			DS: { records: [DS_SHA256] },
		});
	...
	it("matching SHA-256 DS → ds_present ok + ds_algo_match ok; DANE-ready (acceptance #9, #13)", async () => {
		...
		expect(byId(findings, "infra.dnssec_ds_algo_match.ok")?.severity).toBe(
			"ok",
		);
	...
	it("algorithm 13 passes; algorithm 7 (RSASHA1-NSEC3-SHA1) is flagged warning (acceptance #8)", async () => {
		...
		expect(
			byId((await run()).findings, "infra.dnssec_algorithm.ok")?.severity,
		).toBe("ok");

		dnsAnswers({
			DNSKEY: { records: [rsaDnskey(257, 7, 256)] },
			DS: { empty: true },
		});
		const deprecated = byId(
			(await run()).findings,
			"infra.dnssec_algorithm.deprecated",
		);
	...
	it("undersized RSA KSK (< 2048 bits) → key_rollover warning (spec §2 advisory)", async () => {
		dnsAnswers({
			DNSKEY: { records: [rsaDnskey(257, 8, 128)] },
			DS: { empty: true },
		});
		const { findings } = await run();
		expect(byId(findings, "infra.dnssec_key_rollover.weak")?.severity).toBe(
			"warning",
		);
	});
	...
	describe("deep validateViaDig path (spec §3 FUTURE, acceptance #5–#7)", () => {
		...
		it("AD=1 → validates ok; fresh RRSIGs ok; chain complete; DANE-ready (acceptance #13)", async () => {
			...
			deepAnswers({
				"SOA:nocd": {
					status: "NOERROR",
					adFlag: true,
					answers: [rr("SOA", "ns1 host 1 1 1 1 1")],
				},
				"DNSKEY:cd": DNSKEY_CD,
				"SOA:cd": {
					answers: [rr("SOA", "ns1 host 1 1 1 1 1"), rrsig("SOA", FRESH)],
				},
			});
			const { findings, results } = await run();
			expect(byId(findings, "infra.dnssec_validates.ok")?.severity).toBe("ok");
			expect(byId(findings, "infra.dnssec_rrsig_expiry.ok")?.severity).toBe(
				"ok",
			);
			expect(byId(findings, "infra.dnssec_chain_complete.ok")?.severity).toBe(
				"ok",
			);
		...
		it("expired apex RRSIG → critical; within the 72h lead time → warning (acceptance #7)", async () => {
			...
			deepAnswers({
				...
				"DNSKEY:cd": {
					answers: [rr("DNSKEY", KSK_13), rrsig("DNSKEY", EXPIRED)],
				},
			});
			expect(
				byId((await run()).findings, "infra.dnssec_rrsig_expiry.expired")
					?.severity,
			).toBe("critical");
			...
			const { findings, results } = await run();
			expect(byId(findings, "infra.dnssec_rrsig_expiry.near")?.severity).toBe(
				"warning",
			);
		...
		it("NSEC3 iterations > 0 → warning per RFC 9276; params land in results", async () => {
			...
			const { findings, results } = await run();
			expect(byId(findings, "infra.dnssec_nsec3.params")?.severity).toBe(
				"warning",
			);
