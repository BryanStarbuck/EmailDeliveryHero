      ...
      return {
        resolvers:
          Array.isArray(c.resolvers) && c.resolvers.length > 0
            ? c.resolvers.filter((r): r is string => typeof r === "string" && r.trim() !== "")
            : DEFAULT_RESOLVERS,
        rrsigLeadHours:
 ...
 */
function buildAlgorithmRegistry(seed: DnssecAlgorithmEntry[]): AlgorithmRegistry {
  const names = new Map<number, string>()
  const deprecated = new Map<number, boolean>()
  for (const e of seed) {
    if (typeof e?.algo_num !== "number") continue
    if (typeof e.name === "string" && e.name !== "") names.set(e.algo_num, e.name)
...
async function runDeepChecks(
  ...
  if (!probe) {
    ...
    if (!cdProbe.error && cdProbe.status === "NOERROR" && cdProbe.answers.length > 0) {
      out.validates = false
      out.bogus = true
      findings.push({
  ...
  const dnskeyProbe = await digDnssec(domain, "DNSKEY", { resolver: probeResolver, cd: true })
  ...
  const n3 = await digDnssec(domain, "NSEC3PARAM", { resolver: probeResolver, cd: true })
  if (n3.error) {
    findings.push({
    ...
    if (!rec) {
      ...
      out.nsec3Optout = Number.isInteger(flags) ? (flags & 0x01) === 0x01 : null
      const issues: string[] = []
      ...
      if (out.nsec3Optout === true) {
        issues.push("opt-out flag is set (only appropriate for large delegation-heavy zones)")
      }
      if (issues.length > 0) {
        findings.push({
  ...
  if (dsPresent !== true) {
    ...
    const dsTagsInKeys = dsRecords.filter((d) => keys.some((k) => k.keyTag === d.keyTag))
  ...
  for (const type of ["SOA", "MX", "TXT"]) {
    const r = await digDnssec(domain, type, { resolver: probeResolver, cd: true })
    if (r.error) {
    ...
    const covered = r.answers.some(
      (a) => a.type.toUpperCase() === "RRSIG" && parseRrsig(a.rdata)?.typeCovered === type,
    )
  ...
  if (uncovered.length > 0) {
    findings.push({
      ...
      evidence: coverage.map((c) => `${c.type}: ${c.covered ? "RRSIG ✓" : "no RRSIG"}`).join("; "),
    })
...
export const dnssecCheck: Checker = {
  ...
  async run(ctx): Promise<CheckOutcome> {
    ...
    findings.push({
      ...
      evidence: keys
        .map((k) => `${k.flags} ${k.algorithm} (${reg.name(k.algorithm)}) keyTag=${k.keyTag}`)
        .join("; "),
    })
