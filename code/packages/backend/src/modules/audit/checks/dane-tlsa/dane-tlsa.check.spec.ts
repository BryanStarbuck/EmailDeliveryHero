...
const byPrefix = <T extends { id: string }>(
	findings: T[],
	prefix: string,
): T[] => findings.filter((f) => f.id.startsWith(prefix));
...
describe("parseTlsa", () => {
	...
	it("joins dig's space-chunked digest and lower-cases it", () => {
		...
		const [t] = parseTlsa([
			rr(`3 1 1 ${upper.slice(0, 32)} ${upper.slice(32)}`),
		]);
...
describe("analyzeHost", () => {
	it("healthy 3 1 1 x2 on a signed zone: only ok findings and a fully-populated row (AC2)", () => {
		const { findings, row, summary } = analyzeHost(
			observe({
				tlsa: {
					records: [rr(`3 1 1 ${SHA256}`), rr(`3 1 1 ${"c".repeat(64)}`)],
				},
			}),
		);
	...
	it("usage 0/1 (PKIX) is a critical params finding (AC4)", () => {
		const { findings } = analyzeHost(
			observe({ tlsa: { records: [rr(`1 1 1 ${SHA256}`)] } }),
		);
	...
	it("usable-but-not-recommended params (2 1 2) is info", () => {
		const { findings } = analyzeHost(
			observe({ tlsa: { records: [rr(`2 1 2 ${SHA512}`)] } }),
		);
		expect(byPrefix(findings, "infra.dane_tlsa_params")[0].severity).toBe(
			"info",
		);
	...
	it("exactly one TLSA record is a rollover warning (AC6) and rolloverReady=false", () => {
		const { findings, row } = analyzeHost(
			observe({ tlsa: { records: [rr(`3 1 1 ${SHA256}`)] } }),
		);
	...
	it("evaluates TTL sanity from the RR TTL: >24h is a warning, ≤1h is ok", () => {
		const long = analyzeHost(
			observe({ tlsa: { records: [rr(`3 1 1 ${SHA256}`, 172800)] } }),
		);
		expect(byPrefix(long.findings, "infra.dane_ttl_sane")[0].severity).toBe(
			"warning",
		);
		const sane = analyzeHost(
			observe({ tlsa: { records: [rr(`3 1 1 ${SHA256}`, 300)] } }),
		);
		expect(byPrefix(sane.findings, "infra.dane_ttl_sane")[0].severity).toBe(
			"ok",
		);
		const zero = analyzeHost(
			observe({ tlsa: { records: [rr(`3 1 1 ${SHA256}`, 0)] } }),
		);
		expect(byPrefix(zero.findings, "infra.dane_ttl_sane")[0].severity).toBe(
			"warning",
		);
	});
	...
	it("a transient lookup error on an unsigned zone stays info, never a false critical", () => {
		...
		expect(byPrefix(findings, "infra.dane_tlsa_present")[0].severity).toBe(
			"info",
		);
	});
	...
	it("no TLSA on an unsigned zone is only a warning (DANE impossible until signed)", () => {
		const { findings, row } = analyzeHost(
			observe({ dnssec: { signed: false, error: false } }),
		);
		expect(byPrefix(findings, "infra.dane_dnssec_prereq")[0].severity).toBe(
			"warning",
		);
	...
	it("stays quiet when the pinned next-cert digest IS staged (case-insensitive)", () => {
		...
		expect(byPrefix(findings, "infra.dane_rollover.next_missing")).toHaveLength(
			0,
		);
		expect(findings.every((f) => f.severity === "ok")).toBe(true);
	});
...
describe("domainZonePrereqFindings", () => {
	const dnssec = (signed: boolean): DnssecResults =>
		({ signed }) as unknown as DnssecResults;

	it("returns nothing without an upstream dnssec result (standalone run)", () => {
		expect(
			domainZonePrereqFindings("example.com", undefined, true),
		).toHaveLength(0);
	});
	...
	it("suppresses the no-DANE domain-zone warning when all MX are a no-DANE provider (Google)", () => {
		expect(
			domainZonePrereqFindings("example.com", dnssec(false), false, true),
		).toHaveLength(0);
	});
