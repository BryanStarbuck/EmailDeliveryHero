...
function withTimeout<T extends { error?: string }>(p: Promise<T>, timeoutValue: T): Promise<T> {
  return Promise.race([
    p,
...
function expandIpv6(raw: string): string | null {
  ...
  const groups = [...head, ...new Array(tailPart === null ? 0 : missing).fill("0"), ...tail]
  return groups
...
function ipLiteralMatch(host: string, ip: string): string | null {
  ...
  if (isIPv6(ip)) {
    const groups = (expandIpv6(ip) ?? "").split(":").map((g) => g.replace(/^0+/, "") || "0")
    for (const sep of ["-", ".", ":"]) {
...
function genericMatch(host: string, ip: string): string | null {
  ...
  if (first.length >= 12 && !/[aeiou]/.test(first) && /^[0-9a-f-]+$/.test(first))
    return "random-label"
  return null
}
...
async function auditIp(ip: string, domain: string, source: "mx" | "sending_ip"): Promise<IpAudit> {
  ...
  if (!v6) {
    ...
    if (revName) {
      const cname = await withTimeout(resolveCname(`${revName}.in-addr.arpa`), TIMED_OUT)
...
async function mapPool<T, R>(items: T[], limit: number, fn: (t: T) => Promise<R>): Promise<R[]> {
  ...
  const workers = new Array(Math.min(limit, items.length || 1)).fill(0).map(worker)
