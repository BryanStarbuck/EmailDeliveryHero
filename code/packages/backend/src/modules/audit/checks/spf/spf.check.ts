...
function isIpv4(s: string): boolean {
	...
	return (
		parts.length === 4 &&
		parts.every((p) => /^\d{1,3}$/.test(p) && Number(p) <= 255 && p !== "")
	);
}
...
function isIpv6(s: string): boolean {
	return (
		/^[0-9a-f:.]+$/i.test(s) && s.includes(":") && ipv6ToBigInt(s) !== null
	);
}
...
export function findMacroError(value: string): string | null {
	for (let i = 0; i < value.length; i++) {
		...
		if (!m || !MACRO_LETTERS.has(m[1].toLowerCase()))
			return `invalid macro "%{${body}}"`;
		i = close;
	}
	return null;
}
...
export function parseSpfRecord(raw: string): ParsedSpf {
	...
	for (const term of terms.slice(1) /* skip v=spf1 */) {
		...
		let bareName = name;
		if (colon === -1 && name.includes("/"))
			bareName = name.slice(0, name.indexOf("/"));
		...
		switch (bareName) {
			case "ip4": {
				...
				if (!value || !isIpv4(ip) || extra !== undefined) {
					...
					prefix !== undefined &&
					(!/^\d+$/.test(prefix) || Number(prefix) > 32)
				) {
			...
			case "all": {
				if (value !== null)
					out.syntaxErrors.push(`"all" takes no value: "${term}"`);
				break;
			}
			case "ptr":
				break;
...
export function ipv6ToBigInt(ip: string): bigint | null {
	...
	return groups.reduce(
		(acc, g) => (acc << 16n) | BigInt(Number.parseInt(g, 16)),
		0n,
	);
}
...
async function expandTerms(
	...
	for (const mech of parsed.mechanisms) {
		...
		if (state.lookups > 10) {
			...
			child.resolved_to.push(
				"not expanded — 10-lookup budget already exceeded",
			);
			return;
		}

		const target =
			mech.value && !mech.value.startsWith("/") ? mech.value : domain;
		...
		switch (mech.type) {
			...
			case "redirect": {
				if (target.includes("%")) {
					child.resolved_to.push(
						"macro target — not expanded (needs a live message)",
					);
					break;
				}
				if (state.visited.has(target)) {
					state.cycle = true;
					child.resolved_to.push(
						"cycle — this domain is already being evaluated",
					);
					state.findings.push({
				...
				const spf = lookup.records.filter((r) =>
				...
				if (spf.length > 1) {
					child.resolved_to.push(
						`broken — ${spf.length} v=spf1 records at the target`,
					);
					state.findings.push({
						id: "spf.include_resolves",
						checkId: "spf",
				...
				if (
					childParsed.syntaxErrors.length > 0 ||
					childParsed.macroErrors.length > 0
				) {
					state.findings.push({
						id: "spf.include_resolves",
						checkId: "spf",
			...
			case "a": {
				...
				for (const ip of v4.records)
					state.passSet.push({
						cidr: cidr4 ? `${ip}/${cidr4}` : ip,
						source: mech.raw,
					});
				for (const ip of v6.records)
					state.passSet.push({
						cidr: cidr6 ? `${ip}/${cidr6}` : ip,
						source: mech.raw,
					});
			...
			case "mx": {
				...
				for (const rec of mx.records.slice(0, 10)) {
					...
					for (const ip of ips.records)
						state.passSet.push({ cidr: ip, source: mech.raw });
					child.resolved_to.push(
						`${rec.exchange} → ${ips.records.join(", ") || "(no A)"}`,
					);
				}
				break;
			}
			...
			case "ptr": {
				...
				child.resolved_to.push(
					"ptr — counted, not evaluated (deprecated mechanism)",
				);
				break;
			}
		}
...
export function analyzeSpfTerms(
	domain: string,
	raw: string,
	parsed: ParsedSpf,
): Finding[] {
	...
	if (parsed.syntaxErrors.length > 0) {
		findings.push({
			...
			remediation:
				"Fix the offending terms and re-run; the record must parse cleanly end to end.",
			evidence: raw,
		});
	}
	...
	if (parsed.syntaxErrors.length === 0 && parsed.macroErrors.length === 0) {
		findings.push({
			...
			severity: "ok",
			detail:
				"Every term matches the RFC 7208 grammar; macros (if any) are well-formed.",
			evidence: raw,
		});
	}
	...
	if (allQ === "+all") {
		...
		findings.push({
			...
			remediation:
				'Append "~all" (softfail) or "-all" (hardfail) as the last term.',
			evidence: raw,
		});
	} else if (allQ === "?all") {
		findings.push({
			...
			remediation:
				'Use "~all" during rollout and "-all" once every legitimate source is listed.',
			evidence: raw,
		});
	} else if (allQ) {
		findings.push({
	...
	const mechOnly = parsed.mechanisms.filter(
		(m) => m.type !== "redirect" && m.type !== "exp",
	);
	...
	if (dups.length > 0) {
		findings.push({
			...
			remediation:
				"Remove the duplicates; each network or vendor should appear exactly once.",
			evidence: raw,
		});
	}
	...
	if (nonPlusLookups.length > 0) {
		...
		if (hardFail.length > 0) {
			...
			findings.push({
				...
				remediation:
					"Use the plain (+) form unless the softfail/neutral is deliberate.",
	...
	async run(ctx): Promise<CheckOutcome> {
		...
		if (state.lookups > 10) {
			...
			findings.push({
				...
				severity: "ok",
				detail:
					"The recursive expansion stays under the RFC 7208 10-lookup limit.",
			});
		}
		...
		if (state.voids > 2) {
			...
			findings.push({
				...
				remediation:
					"Check the include tree for the void node and delete the stale term.",
				evidence: raw,
			});
		}
		...
		for (const ip of ctx.sendingIps) {
			...
			ipCoverage.push({
				ip,
				covered: Boolean(hit),
				matched_by: hit ? hit.source : null,
			});
			if (!hit) {
		...
		if (ctx.sendingIps.length > 0 && ipCoverage.every((c) => c.covered)) {
			findings.push({
				...
				severity: "ok",
				detail:
					"Every configured sending IP falls inside the record's expanded pass-set.",
			});
		}
		...
		if (!txtAnswer.error) {
			const spfRow = txtAnswer.records.find((r) =>
				r.rdata.toLowerCase().includes("v=spf1"),
			);
			if (spfRow) {
				ttl = spfRow.ttl;
				if (ttl > 86_400) {
					...
					findings.push({
						...
						remediation:
							"Once the record is stable, raise the TTL to ~3600s (1 hour).",
						evidence: `TTL ${ttl}s`,
					});
				} else {
					findings.push({
						...
						severity: "ok",
						detail:
							"The SPF TXT record's TTL is in the operationally sane range (~300s–86,400s).",
						evidence: `TTL ${ttl}s`,
					});
				}
			}
		...
		if (!publicView.error) {
			...
			const agrees =
				localSet.length === publicSet.length &&
				localSet.every((r, i) => r === publicSet[i]);
			crossResolver = {
				resolver: "8.8.8.8",
				agrees,
				public_records: publicSpf,
			};
			if (agrees) {
				findings.push({
					...
					severity: "ok",
					detail:
						"The local resolver and Google's 8.8.8.8 return the same v=spf1 record set.",
		...
		const nullMx =
			mx.records.length === 1 &&
			mx.records[0].priority === 0 &&
			isNullExchange(mx.records[0].exchange);
		...
		if (nonSendingRecord && nullMx) {
			...
			findings.push({
				...
				remediation:
					"If the send-only posture is intentional, document it; otherwise review the MX.",
				evidence: raw,
			});
		}

		// IPv6 completeness (spf.ipv6): infrastructure that receives on IPv6 usually sends on it too.
		const passSetV6Count = state.passSet.filter((p) =>
			p.cidr.includes(":"),
		).length;
		let mxHasAaaa = false;
		...
		const dmarcRec = dmarcLookup.records.find((r) =>
		...
		if (aspf === "s") {
			...
			findings.push({
				...
				remediation:
					"Publish a _dmarc TXT record (start at p=none with rua reporting).",
		...
		const permerror =
			...
			state.cycle ||
			state.findings.some(
				(f) => f.id === "spf.include_resolves" && f.severity === "critical",
			);
		...
		const results: SpfResults = {
			...
			mail_posture: {
				has_mx: hasMx,
				null_mx: nullMx,
				non_sending_record: nonSendingRecord,
			},
