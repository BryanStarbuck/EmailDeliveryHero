...
function loadFingerprints(): TakeoverFingerprint[] {
	try {
		const list = readAppConfig().dns_health.fingerprints.filter(
			(f) => f.enabled,
		);
...
const FUTURE_SUBCHECKS = [
	...
	},
	{
		...
		remediation:
			"Open TCP/53 on every nameserver; DNS requires TCP fallback (RFC 7766).",
	},
	{
		id: "infra.glue_records",
		what: "parent-supplied glue vs authoritative A/AAAA comparison for in-bailiwick NS",
		remediation:
			"Add/fix glue at the registrar so it matches each in-zone NS host's real A/AAAA.",
	},
	{
		id: "infra.ttl_sanity",
	...
	},
	{
		...
		remediation:
			"Add anycast/closer secondaries and investigate any slow nameserver.",
	},
	{
...
function netGroup(ip: string): string {
	if (ip.includes(":"))
		return (
			ip.toLowerCase().split(":").slice(0, 3).join(":") || ip.toLowerCase()
		);
...
function matchFingerprint(
	...
	for (const fp of fingerprints)
		if (t.endsWith(fp.cname_suffix.toLowerCase())) return fp.provider;
	return null;
}

function transientInfo(
	id: string,
	detail: string,
	remediation: string,
): Finding {
...
async function followCname(start: string): Promise<{
	final: string;
	chain: string[];
	status: "chain" | "transient" | "loop";
}> {
	const chain: string[] = [];
...
async function checkNs(
	...
	for (const host of hosts) {
		...
		nsInfos.push({
			host,
			ips,
			groups: unique(ips.map(netGroup)),
			isCname,
			dead,
		});
	}
	...
	snap.ns_asn_diverse =
		resolvedCount === 0 ? null : snap.network_count <= 1 ? false : null;
	snap.lame_ns = nsInfos
		.filter((n) => n.dead)
		.map((n) => ({
			host: n.host,
			reason: "resolves to no A/AAAA address (inferred lame)",
		}));
	...
	const evidence = nsInfos
		.map((n) => `${n.host}=${n.ips.join("/") || "?"}`)
		.join(", ");
		...
		findings.push({
			...
			title:
				hosts.length < 2
					...
					: "No nameserver resolves to an address",
			severity: "critical",
			detail:
				hosts.length < 2
...
async function checkSoa(
	...
	if (r.retry >= r.refresh)
		problems.push(`retry ${r.retry}s >= refresh ${r.refresh}s`);
	...
	if (problems.length > 0) {
		findings.push({
			...
			remediation:
				"Set sane SOA timers, e.g. refresh 7200 retry 3600 expire 1209600 minimum 3600.",
	...
	if (!rname?.includes(".")) {
		...
		const [rmx, ra] = await Promise.all([
			resolveMx(mailboxDomain),
			resolve4(mailboxDomain),
		]);
		if (
			!rmx.error &&
			!ra.error &&
			rmx.records.length === 0 &&
			ra.records.length === 0
		) {
			findings.push({
				id: "infra.soa_rname",
...
function zoneChanged(
	prev: Partial<DnsHealthResults>,
	cur: DnsHealthResults,
): boolean {
	...
	if ((prev.txt_record_count ?? cur.txt_record_count) !== cur.txt_record_count)
		return true;
	const p = prev.soa;
	const c = cur.soa;
	if (!p || !c) return false;
 ...
 */
async function checkParentChildDrift(
	domain: string,
	findings: Finding[],
): Promise<void> {
...
async function danglingLabels(
	...
	const extras = extraLabels
		...
		.filter(Boolean)
		.map((l) =>
			l === domain || l.endsWith(`.${domain}`) ? l : `${l}.${domain}`,
		);
	...
	if (!txt.error) {
		...
		if (spf) {
			for (const tok of spf.split(/\s+/)) {
				...
				const mech = m
					? m[1].replace(/[:=]/, "")
					: (am as RegExpExecArray)[1].toLowerCase();
...
async function checkTxt(
	...
	if (spfRecs.length >= 2) {
		findings.push({
			...
			remediation:
				'Merge into a single "v=spf1 ... -all" TXT record and delete the duplicate(s).',
	...
	const mailNames = unique(
		["_dmarc", "_mta-sts", ...selectors.map((s) => `${s}._domainkey`)].map(
			(l) => `${l}.${domain}`,
		),
	);
