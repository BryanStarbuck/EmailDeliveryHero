...
export type GmailTab = "primary" | "promotions" | "social" | "updates" | "forums"
...
const PROMO_TABS: ReadonlySet<string> = new Set(["promotions", "social", "updates", "forums"])
...
export function aggregateByProvider(
  ...
  for (const r of results) {
    ...
    if (r.folder !== "missing") {
      ...
      if (r.spfPass !== null || r.dkimPass !== null || r.dmarcPass !== null) agg.authParsed++
    }
  ...
  for (const agg of map.values()) {
    ...
    agg.inboxRatePct = agg.delivered > 0 ? (agg.inbox / agg.delivered) * 100 : null
  }
  return map
}
...
export function aggregateOverall(results: SeedPlacementResult[]): OverallAggregate {
  ...
  return {
    ...
    inboxOfDeliveredPct: delivered > 0 ? (counts.inbox / delivered) * 100 : null,
...
function bandOf(ratePct: number, t: PlacementThresholds): "ok" | "warning" | "critical" {
...
export function trendSeries(tests: InboxPlacementTest[]): TrendPoint[] {
  return [...tests]
    ...
    .map((t) => {
      ...
      for (const [key, agg] of aggregateByProvider(t.results)) byProvider[key] = agg.inboxRatePct
      return { sentAt: t.sentAt, overallPct: overall.inboxOfDeliveredPct, byProvider }
    })
}
...
function authSummary(agg: ProviderAggregate): string {
  ...
  const part = (fails: number): string => (fails > 0 ? `FAIL(${fails})` : "pass")
...
export function scorePlacementTest(
  ...
  for (const major of MAJOR_PROVIDERS) {
    ...
    const majorityPromo = major.key === "gmail" && agg.promotions * 2 > agg.total
  ...
  const bothFails = delivered.filter((r) => r.spfPass === false && r.dkimPass === false)
  ...
  if (authParsed.length === 0) {
    ...
    if (spfFailCount > 0)
      mechanisms.push(`spf (${spfFailCount} seed(s) — see content.seed_spf_receiver)`)
    if (dkimFailCount > 0)
      mechanisms.push(`dkim (${dkimFailCount} seed(s) — see content.seed_dkim_receiver)`)
    if (dmarcFails.length > 0)
      mechanisms.push(`dmarc (${dmarcFails.length} seed(s) — see content.seed_dmarc_receiver)`)
    const providers = [
      ...new Set([...dmarcFails, ...bothFails].map((r) => normalizeProvider(r.provider))),
    ]
    findings.push({
      id: "content.seed_auth_pass",
      checkId: CHECK_ID,
  ...
  if (!gmail || gmail.total === 0 || gmailDelivered.length === 0) {
    ...
    const promo = gmailDelivered.filter(
      (r) => r.folder === "promotions" || (r.gmailTab !== null && PROMO_TABS.has(r.gmailTab)),
    )
    if (promo.length > 0) {
      const tabs = [...new Set(promo.map((r) => r.gmailTab ?? "promotions"))].join("/")
      findings.push({
        id: "content.seed_tab_placement",
        checkId: CHECK_ID,
  ...
  if (missingSeeds.length === 0) {
    ...
    const bounced = missingSeeds.filter((r) => r.missingReason === "bounced").length
  ...
  if (!previous) {
    ...
    for (const [key, agg] of byProvider) {
      ...
      if (!before || before.inboxRatePct === null || agg.inboxRatePct === null) continue
      const d = before.inboxRatePct - agg.inboxRatePct
      if (d > 0.5) {
        providerDrops.push(`${key} ${pct(before.inboxRatePct)}→${pct(agg.inboxRatePct)}`)
        worstProviderDrop = Math.max(worstProviderDrop, d)
      }
    }
    const bandWorsened = bandOf(curRate, t) !== "ok" && bandOf(prevRate, t) === "ok"
  ...
  const longtail = [...byProvider.values()].filter((a) => !MAJOR_KEYS.has(a.provider))
  ...
  const withLatency = delivered.filter((r) => r.latencySecs !== null && r.latencySecs !== undefined)
  ...
  const expected = (opts.providers ?? MAJOR_PROVIDERS.map((m) => m.key)).map(normalizeProvider)
  const uncovered = expected.filter((p) => (byProvider.get(p)?.total ?? 0) === 0)
  if (uncovered.length > 0) {
    ...
    const minSeeds = Math.min(...expected.map((p) => byProvider.get(p)?.total ?? 0))
    findings.push({
      id: "content.seed_coverage",
      checkId: CHECK_ID,
  ...
  const slices: {
    id: string
    mech: string
    field: keyof Pick<SeedPlacementResult, "spfPass" | "dkimPass" | "dmarcPass">
    failWord: string
    fix: string
  }[] = [
    {
  ...
  for (const slice of slices) {
    ...
    if (observed.length === 0) {
      ...
      const providers = [...new Set(fails.map((r) => normalizeProvider(r.provider)))]
      findings.push({
        id: slice.id,
        checkId: CHECK_ID,
