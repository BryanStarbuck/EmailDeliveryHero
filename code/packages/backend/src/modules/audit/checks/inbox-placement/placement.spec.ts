...
const score = (
  ...
  new Map(scorePlacementTest(test, previous, { domain: "example.com" }).map((f) => [f.id, f]))
...
describe("aggregation", () => {
  it("computes per-provider and overall rates over delivered seeds", () => {
    ...
    expect(gmail).toMatchObject({ total: 3, inbox: 1, spam: 1, missing: 1, delivered: 2 })
    expect(gmail?.inboxRatePct).toBe(50)
    const overall = aggregateOverall(results)
    expect(overall).toMatchObject({ seedCount: 4, delivered: 3, inbox: 2, missing: 1 })
...
describe("scorePlacementTest — severity bands and majority rules", () => {
  ...
  it("marks a majority-Junk provider critical and majority-Promotions Gmail warning (criterion #5)", () => {
    ...
    expect(findings.get("content.placement_outlook")?.severity).toBe("critical")
    ...
    expect(findings.get("content.seed_tab_placement")?.severity).toBe("warning")
  ...
  it("reports an uncovered provider as unknown info + a coverage warning, never ok (seed_coverage)", () => {
    const findings = score(
      makeTest([seed("gmail", "inbox"), seed("outlook", "inbox"), seed("yahoo", "inbox")]),
    )
...
describe("scorePlacementTest — receiver auth attribution (criterion #7)", () => {
  it("marks receiver-observed DMARC fail critical and names the failing mechanism", () => {
    ...
    expect(findings.get("content.seed_dkim_receiver")?.severity).toBe("critical")
    expect(findings.get("content.seed_dmarc_receiver")?.severity).toBe("critical")
  ...
  it("warns when one mechanism fails while DMARC still passes (fragile single alignment)", () => {
    ...
    expect(findings.get("content.seed_spf_receiver")?.severity).toBe("critical")
