...
function ipv6Class(ip: string): IpClass {
	...
	if (addr === "::" || addr === "::0" || /^0(:0){7}$/.test(addr))
		return "unspecified";
...
export const mxRoutingCheck: Checker = {
	...
	async run(ctx): Promise<CheckOutcome> {
		...
		const expectedHosts = [
			...new Set(
				(mxConfig.expectedHosts ?? []).map(toAsciiName).filter(Boolean),
			),
		];
			...
			return {
				...
				results: snapshot(
					[],
					{
						mx_found: false,
						null_mx: false,
						implicit_a_fallback: a.records.length > 0,
					},
		...
		if (nullRecords.length > 0 && realRecords.length === 0) {
			...
			const priorityOk =
				nullRecords.length === 1 && nullRecords[0].priority === 0;
			if (!priorityOk) {
				findings.push({
					id: "infra.mx_null",
					checkId: CHECK_ID,
			...
			return {
				...
				results: snapshot(
					...
					{ host_count: 0, network_count: 0 },
					{
						...
						distinct_priorities: new Set(nullRecords.map((r) => r.priority))
							.size,
		...
		for (const h of hosts) {
			...
			if (h.nonPublic.length > 0) {
				...
				findings.push({
					id: isLocalhost
						? `infra.mx_localhost.${h.host}`
						: `infra.mx_public_ip.${h.host}`,
					checkId: CHECK_ID,
					title: isLocalhost
		...
		if (hosts.length > 0 && !anyCname) {
			findings.push({
				...
				severity: "ok",
				detail:
					"Every MX target points at an A/AAAA host, not a CNAME (RFC 5321 §5.1).",
			});
		}
		if (hosts.length > 0 && !anyDangling && !hosts.some((h) => h.transient)) {
			findings.push({
				...
				severity: "ok",
				detail:
					"Every MX target resolves to at least one A/AAAA address (no dangling MX).",
			});
		}
		...
		const dups = [...targetCounts.entries()]
			.filter(([, n]) => n > 1)
			.map(([h]) => h);
		if (dups.length > 0) {
			findings.push({
		...
		if (realRecords.length > 10) {
			findings.push({
				...
				remediation:
					"Trim the MX RRset to the hosts you actually operate (2–4 is typical).",
				evidence: realRecords
					.map((r) => `${r.priority} ${r.exchange}`)
					.join(" | "),
			});
		...
		const doubled = hosts.filter((h) =>
			h.host.endsWith(`.${domain}.${domain}`),
		);
		...
		for (const h of hosts) {
			...
			const v6NonPublic =
				v6.length > 0 && v6.every((ip) => ipClass(ip) !== "public");
		...
		return {
			...
			results: snapshot(
				...
				{
					mx_found: true,
					null_mx: nullRecords.length > 0,
					implicit_a_fallback: false,
				},
...
function pushExpectedDrift(
	...
	const parts: string[] = [];
	if (missing.length > 0)
		parts.push(`expected but not published: ${missing.join(", ")}`);
	if (unexpected.length > 0)
		parts.push(`published but not expected: ${unexpected.join(", ")}`);
	const fixes: string[] = [];
	if (missing.length > 0) {
		fixes.push(
