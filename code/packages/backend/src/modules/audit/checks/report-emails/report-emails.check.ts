...
function matchDomain(
  ...
  const parents = monitored.filter((d) => wanted.endsWith(`.${d.name.toLowerCase()}`))
...
function scanCorpus(
  ...
  for (const file of files) {
    ...
    try {
      ...
      tally.decodeErrors.push(`${file}: ${err instanceof Error ? err.message : String(err)}`)
      continue
    }
    ...
    for (const payload of payloads) {
      ...
      if (kind === "dmarc") {
        ...
        if (!match) {
          ...
          else tally.otherDomains[match.name] = (tally.otherDomains[match.name] ?? 0) + 1
        }
      } else if (kind === "tlsrpt") {
  ...
  for (const policy of report.policies) {
    ...
    if (!match) {
      if (!seenDomains.has(policy.policyDomain)) {
        ...
        tally.orphans[policy.policyDomain] = (tally.orphans[policy.policyDomain] ?? 0) + 1
      }
      continue
    }
...
function buildSnapshot(
  ...
  return {
    ...
    attribution: {
      ...
      orphans: Object.entries(tally.orphans).map(([domain, count]) => ({ domain, count })),
    },
...
function deriveCorpusFindings(
  ...
  if (!hasDmarcReports) {
    ...
    const failSources = new Set(
      dmarc.rows.filter((r) => !(r.spfAligned && r.dkimAligned)).map((r) => r.sourceIp),
    ).size
    findings.push({
      id: "content.report_pass_rate",
      checkId: CHECK_ID,
    ...
    const bothFail = dmarc.rows.filter((r) => !r.spfAligned && !r.dkimAligned && r.count > 0)
    if (bothFail.length === 0) {
      ...
      const ownOnly = bothFail.every(
        (r) => r.envelopeSpfDomain !== "" && underDomain(r.envelopeSpfDomain, domain),
      )
    ...
    if (fragile.size === 0) {
      findings.push({
        ...
        severity: "ok",
        detail: "Every own stream that passes DMARC aligns on both SPF and DKIM.",
        source: "report",
      })
    } else {
  ...
  if (!hasTlsReports) {
    ...
    const types = [...new Set(tlsrpt.rows.flatMap((r) => r.failureDetails.map((d) => d.resultType)))]
    findings.push({
      id: "content.report_tls",
      checkId: CHECK_ID,
...
export const reportEmailsCheck: Checker = {
  ...
  async run(ctx): Promise<CheckOutcome> {
    ...
    if (!config.enabled) {
      ...
      return { findings: [ingestionDisabledFinding("content.report_corpus", CHECK_ID)] }
    }
