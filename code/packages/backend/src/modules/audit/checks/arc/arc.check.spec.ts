...
async function run(
	c: CheckContext,
): Promise<{ findings: Finding[]; results: ArcResults }> {
...
describe("arcCheck (pm/checks/arc.mdx)", () => {
	...
	it("warns on forwarding risk when usesForwarding is declared but no forwarders are registered", async () => {
		dnsAnswers({
			"_dmarc.example.com": { records: ["v=DMARC1; p=quarantine"] },
		});
		const { findings, results } = await run(
			ctx({ usesForwarding: true, forwarders: [] }),
		);
	...
	it("verifies each declared forwarder's signer selector in DNS (resolving key → ok)", async () => {
		dnsAnswers({
			...
			"arc1._domainkey.lists.acme.org": {
				records: [`v=DKIM1; k=rsa; p=${GOOD_P}`],
			},
		});
		const { findings, results } = await run(
			ctx({
		...
		expect(byId(findings, "arc.forwarding_risk.acme-users")?.severity).toBe(
			"warning",
		);
		...
		expect(
			byId(findings, "arc.signature_algorithm.acme-users"),
		).toBeUndefined();
	...
	it("flags an empty/revoked p= as critical and a weak RSA key as warning", async () => {
		dnsAnswers({
			...
			"arc2._domainkey.lists.beta.org": {
				records: [`v=DKIM1; k=rsa; p=${"A".repeat(120)}`],
			},
		});
		...
		expect(byId(findings, "arc.signature_algorithm.beta")?.severity).toBe(
			"warning",
		);
	});
	...
	it("emits an info (not critical) when a forwarder's signer is not yet known", async () => {
		...
		const { findings } = await run(
			ctx({
				usesForwarding: true,
				forwarders: [
					{ label: "Some List", forwardAddress: "list@example.org" },
				],
			}),
		);
