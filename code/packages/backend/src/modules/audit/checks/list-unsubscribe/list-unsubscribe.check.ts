...
import {
	DEFAULT_SHORTENERS,
	registrableDomain,
} from "../link-url-reputation/url-extract";
...
export function parseListUnsubscribe(value: string): ListUnsubParse {
	...
	if (tokens.length === 0)
		parse.syntaxIssues.push("header is present but empty");
	for (const token of tokens) {
		...
		const scheme =
			uri.match(/^([a-z][a-z0-9+.-]*):/i)?.[1]?.toLowerCase() ?? null;
		if (scheme === "https") {
			try {
			...
			parse.syntaxIssues.push(
				`"${uri}" has no URI scheme (expected https: or mailto:)`,
			);
		} else {
			parse.syntaxIssues.push(
...
function isTlsError(code: string): boolean {
	return /CERT|TLS|SSL|ALTNAME|SELF_SIGNED|UNABLE_TO_VERIFY|HANDSHAKE/i.test(
		code,
	);
}
		...
		try {
			...
			const req = httpsRequest(
				...
				(res) => {
					...
					const tlsValid =
						typeof socket.authorized === "boolean" ? socket.authorized : null;
					const chunks: Buffer[] = [];
					let bytes = 0;
...
async function probeEndpoint(
	...
	return withResource("http", async () => {
		...
		if (
			post.status !== null &&
			[301, 302, 303, 307, 308].includes(post.status) &&
			post.location
		) {
		...
		const endpointOk =
			post.status !== null && post.status >= 200 && post.status < 300;
		...
		if (post.error === null || post.error === "timeout") {
			...
			if (get.status !== null) {
				getSafe = !(
					get.status >= 200 &&
					get.status < 300 &&
					GET_UNSUBSCRIBED_RE.test(get.body)
				);
			}
		}
		return {
...
export function looksPerRecipient(
	httpsUri: string | null,
	mailtoUri: string | null,
): boolean {
	if (httpsUri) {
		try {
			...
			for (const value of url.searchParams.values()) {
				if (value.length >= 8 && /[a-z]/i.test(value) && /[0-9]/.test(value))
					return true;
...
export const listUnsubscribeCheck: Checker = {
	...
	async run(ctx): Promise<Finding[] | CheckOutcome> {
		...
		const bulkThreshold =
			listUnsubConfig?.bulkThresholdPerDay ?? DEFAULT_BULK_THRESHOLD_PER_DAY;
		const probeTimeoutMs =
			listUnsubConfig?.probeTimeoutMs ?? DEFAULT_PROBE_TIMEOUT_MS;
		const probeAllowedGlobally = listUnsubConfig?.probeAllowed ?? true;
		const probeCadenceMs =
			(listUnsubConfig?.probeCadenceHours ?? DEFAULT_PROBE_CADENCE_HOURS) *
			60 *
			60 *
			1000;
			...
			return [
				{
					...
					remediation:
						"Upload the sample .eml again to re-enable the list-management checks.",
		...
		const hasOneclick =
			listUnsubPost !== null &&
			listUnsubPost.replace(/\s+/g, "").toLowerCase() ===
		...
		const fromDkimAligned = dkimDomains.some((d) =>
			relaxedAligned(fromDomain, d),
		);
		...
		const precedenceBulk =
			...
			["bulk", "list", "junk"].includes(precedenceValue);
		const autoSubmittedSet =
		...
		const previous = ctx.previousResults?.[CHECK_ID] as
			| ListUnsubResults
			| undefined;
		...
		if (httpsUri) {
			...
			const upstreamUrl = ctx.upstream?.["content.url"] as
				| LinkUrlResults
				| undefined;
			const listedZones =
					...
					? [
							...new Set(
								upstreamUrl.links
									...
									.flatMap((l) =>
										l.listings.filter((z) => z.listed).map((z) => z.zone),
									),
							),
						]
					: [];
		...
		const unsubHost =
			httpsHost(httpsUri) ?? domainOfAddress(addressIn(mailtoUri));
		...
		if (fromOrg && (unsubOrg2 || listIdOrg)) {
			...
			if (unsubOrg2 && unsubOrg2 !== fromOrg) {
				mismatches.push(
					`unsubscribe host ${unsubOrg2} ≠ From org-domain ${fromOrg}`,
				);
			}
			if (listIdOrg && listIdOrg !== fromOrg) {
				mismatches.push(
					`List-Id domain ${listIdOrg} ≠ From org-domain ${fromOrg}`,
				);
			}
