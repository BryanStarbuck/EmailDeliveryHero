		...
		if (!result.timedOut && result.code === 0) {
			...
			if (report) {
				return {
					report,
					engine: "spamc",
					saVersion: avail.spamassassin
						? await readSaVersion(avail.spamassassin)
						: null,
				};
			}
		}
...
export function subCheckForRule(rule: string): ContentSubCheckId | null {
	...
	if (r.includes("CHARSET_FARAWAY") || r === "UNWANTED_LANGUAGE_BODY")
		return "content.charset";
	if (r.includes("EXECUTABLE") || r.includes("ATTACH"))
		return "content.attachment_risk";
	if (
		r.startsWith("SUBJ_") ||
		r.startsWith("SUBJECT_") ||
		r.startsWith("FAKE_REPLY")
	) {
	...
	if (r === "MIME_BASE64_TEXT" || r.startsWith("MIME_QP"))
		return "content.encoding";
	if (r === "MIME_HTML_ONLY" || r.startsWith("MPART_"))
		return "content.multipart";
	if (
		r.startsWith("HTML_IMAGE") ||
		r.includes("IMAGE_ONLY") ||
		r.includes("IMAGE_RATIO")
	) {
		return "content.image_text_ratio";
	}
	if (r.startsWith("MIME_") || r.startsWith("MISSING_MIME"))
		return "content.mime_valid";
	if (
		r === "HIDDEN_TEXT" ||
