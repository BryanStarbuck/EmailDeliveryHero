...
export function readSampleRaw(sample: ContentSampleRecord): string | null {
  if (sample.rawText !== null && sample.rawText !== undefined) return sample.rawText
...
export function saveSample(domainId: string, raw: string): ContentSampleRecord {
  ...
  if (byteSize > MAX_SAMPLE_BYTES) {
    throw new Error(`Sample message is too large (${byteSize} bytes; max ${MAX_SAMPLE_BYTES})`)
  }
  const id = randomUUID()
  ...
  const rows = readJson<ContentSampleRecord[]>(indexPath(domainId), []).map((s) => ({
    ...s,
    active: false,
  }))
  rows.push(record)
