...
import { type ContentSampleRecord, getActiveSample, readSampleRaw } from "./sample-store"
import {
...
> = {
  ...
  "content.image_text_ratio": {
    ...
    okDetail: "The message is not image-heavy — real body copy accompanies any images.",
    remediation:
  ...
  "content.spammy_phrases": {
    ...
    okDetail: "The body avoids trigger phrases, gappy text, and filter-dodging obfuscation.",
    remediation:
  ...
  "content.short_body": {
    ...
    okDetail: "The message has meaningful body copy — not a one-line wrapper around a single link.",
    remediation: "Include meaningful body copy; do not send one-line 'click here' emails.",
    advisory: true,
  },
}
...
function bandFor(total: number, safeTarget: number, threshold: number): Severity {
...
export const contentScoringCheck: Checker = {
  ...
  async run(ctx): Promise<Finding[] | CheckOutcome> {
    ...
    const previous = ctx.previousResults?.["content.scoring"] as ContentScoreResults | undefined
    if (
      previous &&
      ...
      return [
        {
          ...
          remediation: "Upload the sample .eml again to re-enable content scoring.",
