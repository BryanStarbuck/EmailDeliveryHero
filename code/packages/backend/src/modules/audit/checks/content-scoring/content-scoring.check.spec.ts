...
import {
	contentScoringCheck,
	isContentScoringFinding,
} from "./content-scoring.check";
...
describe("subCheckForRule / isHighWeightRule", () => {
	it("maps rule families to the §2 sub-checks", () => {
		...
		expect(subCheckForRule("HTML_IMAGE_RATIO_02")).toBe(
		...
		expect(subCheckForRule("FORGED_OUTLOOK_TAGS")).toBe(
			"content.header_sanity",
		);
		expect(subCheckForRule("MICROSOFT_EXECUTABLE")).toBe(
			"content.attachment_risk",
		);
		expect(subCheckForRule("SUSPICIOUS_ATTACHMENT")).toBe(
			"content.attachment_risk",
		);
		...
		expect(subCheckForRule("OBFUSCATING_COMMENT")).toBe(
	...
	it("flags the high-weight families that force a critical sub-check (§8 AC 5)", () => {
		expect(
			isHighWeightRule({ rule: "HIDDEN_TEXT", score: 1.0, description: "" }),
		).toBe(true);
		expect(
			isHighWeightRule({ rule: "FORGED_MUA", score: 0.5, description: "" }),
		).toBe(true);
		expect(
			isHighWeightRule({
		...
		expect(
			isHighWeightRule({ rule: "SUBJ_ALL_CAPS", score: 1.2, description: "" }),
		).toBe(false);
		expect(
			isHighWeightRule({ rule: "ANY_RULE", score: 3.5, description: "" }),
		).toBe(true);
	});
});
...
describe("sample-store", () => {
	it("persists an upload, parses From/Subject, and deactivates the prior active (§8 AC 2)", () => {
		...
		expect(first.rawPath).toContain(
			join("samples", "dom-store", `${first.id}.eml`),
		);
	...
	it("rejects empty and oversized samples", () => {
		...
		expect(() =>
			saveSample("dom-store", "x".repeat(MAX_SAMPLE_BYTES + 1)),
		).toThrow(/too large/);
	});
});
...
describe("contentScoringCheck", () => {
	...
	it("scores a sample: warning band, per-rule sub-check attribution, ok rows, payload (§8 AC 4/6)", async () => {
			...
			report: {
				...
				rulesFired: [
					...
					},
					{
						rule: "MIME_HTML_ONLY",
						score: 1.1,
						description: "HTML-only message",
					},
					{
						rule: "HTML_IMAGE_RATIO_02",
						score: 0.9,
						description: "Low text-to-image ratio",
					},
				],
			},
			engine: "spamassassin",
		...
		expect(findings.find((f) => f.id === "content.multipart")?.severity).toBe(
			"warning",
		);
		expect(
			findings.find((f) => f.id === "content.image_text_ratio")?.severity,
		).toBe("warning");
		// Quiet core sub-checks are explicit passes.
		expect(
			findings.find((f) => f.id === "content.header_sanity")?.severity,
		).toBe("ok");
		expect(
			findings.find((f) => f.id === "content.attachment_risk")?.severity,
		).toBe("ok");
	...
	it("bands the total exactly and makes one high-weight rule critical (§8 AC 5)", async () => {
		...
		mockScore.mockResolvedValue({
			report: {
				...
				threshold: 5.0,
				rulesFired: [
					{ rule: "HIDDEN_TEXT", score: 4.0, description: "Hidden text" },
				],
			},
			engine: "spamc",
			saVersion: null,
		...
		expect(
			outcome.findings.find((f) => f.id === "content.spamassassin_score")
				?.severity,
		).toBe("warning");
		// …but the hidden-text sub-check is critical regardless of the moderate total.
		expect(
			outcome.findings.find((f) => f.id === "content.html_hygiene")?.severity,
		).toBe("critical");
	});
