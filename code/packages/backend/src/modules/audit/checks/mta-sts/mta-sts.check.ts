...
function parsePolicy(body: string): ParsedPolicy {
  ...
  return { keys, version, versionFirst: keys[0] === "version", modeRaw, maxAgeRaw, mx }
}
    ...
    const req = httpsRequest(
      {
        ...
        timeout: timeoutMs,
        ...(signal ? { signal } : {}),
        headers: { host, accept: "text/plain", "user-agent": "EmailDeliveryHero-audit/1.0" },
      },
      (res) => {
        const socket = res.socket as TLSSocket
        const cert = socket.getPeerCertificate?.()
        const hasCert = Boolean(cert && Object.keys(cert).length > 0)
        const nameErr = hasCert ? checkServerIdentity(host, cert) : new Error("no peer certificate")
        const certOk = socket.authorized === true && !nameErr
        const certDetail = certOk
          ? null
          : socket.authorized !== true
            ? String(socket.authorizationError ?? "certificate chain not trusted")
...
export const mtaStsCheck: Checker = {
  ...
  async run(ctx: CheckContext): Promise<CheckOutcome> {
    ...
    const modeValid = modeLower === "enforce" || modeLower === "testing" || modeLower === "none"
    results.mode = modeValid ? (modeLower as "enforce" | "testing" | "none") : null
    const maxAge =
      policy.maxAgeRaw !== null && /^\d+$/.test(policy.maxAgeRaw) ? Number(policy.maxAgeRaw) : null
    ...
    if (upstreamMx?.hosts) {
      ...
      liveMx = mx.records.map((r) => normalizeHost(r.exchange)).filter((h) => h !== "" && h !== ".")
    }
    ...
    if (policy.mx.length > 0 && liveMxKnown) {
      ...
      const unmatchedLive = liveMx.filter((h) => !patterns.some((p) => mxPatternMatches(p, h)))
      const deadPatterns = patterns.filter((p) => !liveMx.some((h) => mxPatternMatches(p, h)))
    ...
    const prev = ctx.previousResults?.[CHECK_ID] as Partial<MtaStsResults> | undefined
