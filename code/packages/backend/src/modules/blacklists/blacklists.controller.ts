...
export class BlacklistsController {
  ...
  updateZone(@Param("zone") zone: string, @Body() body: UpdateZoneDto): BlacklistRegistryInfo {
    if (isDeadZone(zone)) {
  ...
  async recheck(
    ...
    try {
      ...
      if (err instanceof RecheckInputError) throw new BadRequestException(err.message)
      throw err
    }
  }
