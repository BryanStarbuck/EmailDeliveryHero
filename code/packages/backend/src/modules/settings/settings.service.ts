...
export class SettingsService {
	...
	async updateUser(
		...
		updateUserConfig(user.email, (cfg) => {
			...
			if (dto.appearance?.density !== undefined)
				cfg.density = dto.appearance.density;
			if (dto.notifications)
				cfg.notifications = { ...cfg.notifications, ...dto.notifications };
		});
	...
	async updateAdmin(
		...
		updateAppConfig((cfg) => {
			if (dto.checks) {
				if (dto.checks.enabled)
					cfg.checks.enabled = [...new Set(dto.checks.enabled)];
				...
				if (dto.checks.dkim?.defaultSelectors)
					cfg.checks.dkim.defaultSelectors = cleanList(
						dto.checks.dkim.defaultSelectors,
					);
				if (dto.checks.dnsbl?.zones)
					cfg.checks.dnsbl.zones = cleanList(dto.checks.dnsbl.zones);
				...
				if (dto.checks.listUnsub)
					cfg.checks.listUnsub = {
						...cfg.checks.listUnsub,
						...dto.checks.listUnsub,
					};
				if (dto.checks.thresholds)
					cfg.checks.thresholds = {
						...cfg.checks.thresholds,
						...dto.checks.thresholds,
					};
			...
			if (dto.notifications?.webhook)
				cfg.notifications.webhook = {
					...cfg.notifications.webhook,
					...dto.notifications.webhook,
				};
			if (dto.notifications?.smtp)
				cfg.notifications.smtp = {
					...cfg.notifications.smtp,
					...dto.notifications.smtp,
				};
			if (dto.storage?.retentionDays !== undefined)
				cfg.storage.retentionDays = dto.storage.retentionDays;
			if (dto.tools) {
				if (dto.tools.preferCli !== undefined)
					cfg.tools.preferCli = dto.tools.preferCli;
				if (dto.tools.resolvers)
					cfg.tools.resolvers = cleanList(dto.tools.resolvers);
				if (dto.tools.timeoutMs !== undefined)
					cfg.tools.timeoutMs = dto.tools.timeoutMs;
			...
			if (dto.seedList) {
				...
				if (sl.service !== undefined)
					cfg.seedList.service = sl.service.trim().toLowerCase();
				if (sl.providers)
					cfg.seedList.providers = cleanList(
						sl.providers.map((p) => p.toLowerCase()),
					);
				if (sl.cadence !== undefined) cfg.seedList.cadence = sl.cadence;
				if (sl.thresholds) {
					cfg.seedList.thresholds = {
						...cfg.seedList.thresholds,
						...sl.thresholds,
					};
					// Keep the bands ordered: critical must sit at or below warning (spec §3 mapping).
					if (
						cfg.seedList.thresholds.criticalBelowPct >
						cfg.seedList.thresholds.warnBelowPct
					) {
						cfg.seedList.thresholds.criticalBelowPct =
							cfg.seedList.thresholds.warnBelowPct;
					}
				...
				if (sl.monthlyBudget !== undefined)
					cfg.seedList.monthlyBudget = sl.monthlyBudget;
				if (sl.seeds) {
					const seen = new Set<string>();
		...
		if (dto.schedule) {
			...
			await this.scheduler.updateConfig(
				dto.schedule as Record<string, unknown>,
			);
		}
	...
	async openStateDir(): Promise<{ opened: boolean; stateDir: string }> {
		...
		try {
			...
			logError(
				"Could not open the state dir in the file manager",
				err,
				"Settings",
			);
	...
	async exportArchive(): Promise<{ fileName: string; data: Buffer }> {
		...
		if (!zip)
			throw new ServiceUnavailableException(
				"The `zip` tool is not installed on this host",
			);
		const stateDir = resolveStateDir();
		const entries = EXPORT_ENTRIES.filter((e) => existsSync(join(stateDir, e)));
		if (entries.length === 0)
			throw new BadRequestException("Nothing to export yet");
	...
	async importArchive(archiveBase64: string): Promise<{ imported: string[] }> {
		...
		if (!unzip)
			throw new ServiceUnavailableException(
				"The `unzip` tool is not installed on this host",
			);
		...
		try {
			...
			try {
				await execFileAsync(unzip, ["-o", "-q", zipFile, "-d", staging], {
					timeout: 60_000,
				});
			} catch (err) {
			...
			for (const yamlName of ["config.yaml", "domains.yaml"]) {
				...
				try {
					...
					throw new BadRequestException(
						`The archive's ${yamlName} is not valid YAML`,
					);
				}
			}
			...
			if (found.length === 0)
				throw new BadRequestException(
					"The archive contains none of the known state files",
				);
			const stateDir = resolveStateDir();
			for (const entry of found) {
				cpSync(join(staging, entry), join(stateDir, entry), {
					recursive: true,
					force: true,
				});
			}
