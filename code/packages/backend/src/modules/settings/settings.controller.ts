...
export class SettingsController {
  ...
  view(@CurrentUser() user: AuthUser): Promise<SettingsView> {
	...
	@Put()
	@ApiOperation({
		summary: "Update MY per-user fields (notification prefs, appearance)",
	})
	updateUser(
	...
	@ApiOperation({
		summary:
			"Update admin-only global fields (checks, channels, storage, tools, access)",
	})
	updateAdmin(
		@CurrentUser() user: AuthUser,
  ...
  testNotification(@CurrentUser() user: AuthUser): Promise<TestNotificationResult> {
	...
	@Post("tools/detect")
	@ApiOperation({
		summary: "Re-probe the environment for dig/swaks and return their status",
	})
	...
	@Post("open-state-dir")
	@ApiOperation({
		summary: "Reveal the state dir in the OS file manager (Finder on macOS)",
	})
	...
	@ApiOperation({
		summary: "Download a zip of config.yaml + domains.yaml + audit history",
	})
  ...
  importArchive(@Body() dto: ImportArchiveDto): Promise<{ imported: string[] }> {
	...
	@RequireRole("admin")
	@ApiOperation({
		summary: "Reset audit history or the whole app (body selects the scope)",
	})
	reset(
		@Body() dto: ResetDto,
		@CurrentUser() user: AuthUser,
