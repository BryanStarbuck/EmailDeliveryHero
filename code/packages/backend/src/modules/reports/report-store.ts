...
export function saveDmarcReport(
	domainId: string,
...
export function saveTlsRptReport(
	domainId: string,
...
export function hasAnyReports(domainId: string): boolean {
	return (
		listDmarcReports(domainId).length > 0 ||
		listTlsRptReports(domainId).length > 0
	);
}

export function readIngestState(domainId: string): IngestState {
	return readJson<IngestState>(ingestStatePath(domainId), {
		lastIngestAt: null,
		processedIds: [],
	});
}
