...
function unzipAll(buf: Buffer): Buffer[] {
	...
	while (offset + 30 <= buf.length && buf.readUInt32LE(offset) === 0x04034b50) {
		...
		if (method === 8) {
			out.push(
				inflateRawSync(data, { maxOutputLength: MAX_UNCOMPRESSED_BYTES }),
			);
		} else if (method === 0) {
...
export function decompressPayload(buf: Buffer): Buffer[] {
	if (isGzip(buf))
		return [gunzipSync(buf, { maxOutputLength: MAX_UNCOMPRESSED_BYTES })];
...
export function extractReportPayloads(raw: Buffer): ReportPayload[] {
	...
	if (isGzip(raw) || isZip(raw) || looksLikeBarePayload(raw)) {
		return decompressPayload(raw).map((content) => ({
			mediaType: "",
			filename: "",
			content,
		}));
	...
	for (const part of collectLeafParts(message)) {
		...
		const isOctetHint =
			...
			REPORT_EXTENSIONS.test(filename);
...
export function classifyPayload(
	...
	if (head.startsWith("{")) {
		return head.includes('"organization-name"') || head.includes('"policies"')
			? "tlsrpt"
			: null;
	}
	if (head.startsWith("<")) {
