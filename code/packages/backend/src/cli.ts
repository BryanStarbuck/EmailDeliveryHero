...
async function main(): Promise<number> {
  ...
  try {
    ...
    if (outcome.started) {
      console.log(`Scheduled check run finished (${outcome.domains ?? 0} domain(s))`)
    } else {
      console.log(`Scheduled check run skipped (${outcome.reason ?? "unknown"})`)
    }
    return 0
  } catch (err) {
    logError("Headless CLI run failed", err, "Cli")
    console.error(`edh scheduler run failed: ${err instanceof Error ? err.message : String(err)}`)
    return 1
  } finally {
    await app.close()
  }
...
main().then(
  ...
  (err) => {
    console.error(`edh failed: ${err instanceof Error ? err.message : String(err)}`)
    process.exit(1)
  },
)
